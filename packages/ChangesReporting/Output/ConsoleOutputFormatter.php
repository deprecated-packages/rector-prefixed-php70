...
final class ConsoleOutputFormatter implements \Rector\ChangesReporting\Contract\Output\OutputFormatterInterface
{
    ...
    private function normalizePathsToRelativeWithLine(string $errorMessage) : string
    {
        ...
        $errorMessage = \RectorPrefix20210616\Nette\Utils\Strings::replace($errorMessage, $regex, '');
        return \RectorPrefix20210616\Nette\Utils\Strings::replace($errorMessage, self::ON_LINE_REGEX, ':');
    }
    /**
     * @return void
     */
    ...
    private function createRectorChangelogLines(\Rector\Core\ValueObject\Reporting\FileDiff $fileDiff) : array
    {
        ...
        foreach ($rectorsChangelogs as $rectorClass => $changelog) {
            $rectorShortClass = (string) \RectorPrefix20210616\Nette\Utils\Strings::after($rectorClass, '\\', -1);
