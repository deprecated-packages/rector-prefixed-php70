...
final class Indent
{
     ...
     */
    const CHARACTERS = [self::SPACE => ' ', self::TAB => "\t"];
    /**
     * @var string
     */
    const SPACE = 'space';
     ...
     * @var string
     */
    const PARSE_INDENT_REGEX = '/^(?P<indent>( +|\\t+)).*/m';
    /**
     * @var string
     */
    private $string;
    ...
    {
        $match = \RectorPrefix20210526\Nette\Utils\Strings::match($content, self::VALID_INDENT_REGEX);
        if ($match === null) {
    ...
    {
        $match = \RectorPrefix20210526\Nette\Utils\Strings::match($content, self::PARSE_INDENT_REGEX);
    ...
    {
        return \RectorPrefix20210526\Nette\Utils\Strings::startsWith($this->string, ' ');
    }
}
