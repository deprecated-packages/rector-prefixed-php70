...
final class XmlFileFormatter implements \Rector\FileFormatter\Contract\Formatter\FileFormatterInterface
{
    ...
    {
        $withNewLines = \RectorPrefix20210526\Nette\Utils\Strings::replace(\trim($xml), self::XML_PARTS_REGEX, "\$1\n\$2\$3");
        return \explode("\n", $withNewLines);
    }
    ...
    {
        return (bool) \RectorPrefix20210526\Nette\Utils\Strings::match($part, self::IS_OPENING_TAG_REGEX);
    }
    private function isClosingTag(string $part) : bool
    {
        return (bool) \RectorPrefix20210526\Nette\Utils\Strings::match($part, self::IS_CLOSING_TAG_REGEX);
    }
    private function isOpeningCdataTag(string $part) : bool
    {
        return \RectorPrefix20210526\Nette\Utils\Strings::contains($part, '<![CDATA[');
    }
    private function isClosingCdataTag(string $part) : bool
    {
        return \RectorPrefix20210526\Nette\Utils\Strings::contains($part, ']]>');
    }
}
