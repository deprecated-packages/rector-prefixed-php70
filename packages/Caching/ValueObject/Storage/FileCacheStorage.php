...
use RectorPrefix20210620\Nette\Utils\Random;
use RectorPrefix20210620\Nette\Utils\Strings;
use Rector\Caching\ValueObject\CacheFilePaths;
use Rector\Caching\ValueObject\CacheItem;
use RectorPrefix20210620\Symplify\EasyCodingStandard\Caching\Exception\CachingException;
...
final class FileCacheStorage
{
    ...
    public function __construct(string $directory, \RectorPrefix20210620\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem)
    {
    ...
    public function save(string $key, string $variableKey, $data)
    {
        ...
        $tmpPath = \sprintf('%s/%s.tmp', $this->directory, \RectorPrefix20210620\Nette\Utils\Random::generate());
        ...
        if ($errorAfter !== null && $errorBefore !== $errorAfter) {
            ...
            throw new \RectorPrefix20210620\Symplify\EasyCodingStandard\Caching\Exception\CachingException($errorMessage);
        }
    ...
    {
        $keyHash = \sha1($key);
        $firstDirectory = \sprintf('%s/%s', $this->directory, \RectorPrefix20210620\Nette\Utils\Strings::substring($keyHash, 0, 2));
        $secondDirectory = \sprintf('%s/%s', $firstDirectory, \RectorPrefix20210620\Nette\Utils\Strings::substring($keyHash, 2, 2));
