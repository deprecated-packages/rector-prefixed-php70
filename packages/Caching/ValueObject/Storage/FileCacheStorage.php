...
use RectorPrefix20210616\Nette\Utils\Random;
...
final class FileCacheStorage
{
    ...
    public function save(string $key, string $variableKey, $data)
    {
        ...
        $tmpPath = \sprintf('%s/%s.tmp', $this->directory, \RectorPrefix20210616\Nette\Utils\Random::generate());
        ...
        if ($errorAfter !== null && $errorBefore !== $errorAfter) {
            ...
            throw new \RectorPrefix20210616\Symplify\EasyCodingStandard\Caching\Exception\CachingException($errorMessage);
        }
    ...
    {
        $keyHash = \sha1($key);
        $firstDirectory = \sprintf('%s/%s', $this->directory, \RectorPrefix20210616\Nette\Utils\Strings::substring($keyHash, 0, 2));
        $secondDirectory = \sprintf('%s/%s', $firstDirectory, \RectorPrefix20210616\Nette\Utils\Strings::substring($keyHash, 2, 2));
