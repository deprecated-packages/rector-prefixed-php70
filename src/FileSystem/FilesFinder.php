...
use RectorPrefix20210503\Symplify\Skipper\SkipCriteriaResolver\SkippedPathsResolver;
...
final class FilesFinder
{
    ...
    public function __construct(\Rector\Core\FileSystem\FilesystemTweaker $filesystemTweaker, \RectorPrefix20210503\Symplify\SmartFileSystem\Finder\FinderSanitizer $finderSanitizer, \RectorPrefix20210503\Symplify\SmartFileSystem\FileSystemFilter $fileSystemFilter, \RectorPrefix20210503\Symplify\Skipper\SkipCriteriaResolver\SkippedPathsResolver $skippedPathsResolver)
    {
    ...
    private function findInDirectories(array $directories, array $suffixes) : array
    {
        ...
        $finder = \RectorPrefix20210503\Symfony\Component\Finder\Finder::create()->followLinks()->files()->size('> 0')->in($absoluteDirectories)->name($suffixesPattern)->sortByName();
     ...
     * @return void
     */
    private function addFilterWithExcludedPaths(\RectorPrefix20210503\Symfony\Component\Finder\Finder $finder)
    {
        $excludePaths = $this->skippedPathsResolver->resolve();
        if ($excludePaths === []) {
            return;
        }
        $finder->filter(function (\RectorPrefix20210503\Symfony\Component\Finder\SplFileInfo $splFileInfo) use($excludePaths) : bool {
            ...
            foreach ($excludePaths as $excludePath) {
                ...
                if (\RectorPrefix20210503\Nette\Utils\Strings::match($realPath, '#' . \preg_quote($excludePath, '#') . '#')) {
                    return \false;
                }
    ...
    {
        // ends with *
        if (\RectorPrefix20210503\Nette\Utils\Strings::match($path, self::ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\RectorPrefix20210503\Nette\Utils\Strings::match($path, self::STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
        return $path;
    }
