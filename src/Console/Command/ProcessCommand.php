...
use RectorPrefix20210531\Symplify\PackageBuilder\Console\ShellCode;
final class ProcessCommand extends \RectorPrefix20210531\Symfony\Component\Console\Command\Command
{
    /**
    ...
    protected function configure()
    {
        ...
        $this->addArgument(\Rector\Core\Configuration\Option::SOURCE, \RectorPrefix20210531\Symfony\Component\Console\Input\InputArgument::OPTIONAL | \RectorPrefix20210531\Symfony\Component\Console\Input\InputArgument::IS_ARRAY, 'Files or directories to be upgraded.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_DRY_RUN, 'n', \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'See diff of changes, do not save them to files.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_AUTOLOAD_FILE, 'a', \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'File with extra autoload');
        $names = $this->outputFormatterCollector->getNames();
        $description = \sprintf('Select output format: "%s".', \implode('", "', $names));
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_OUTPUT_FORMAT, 'o', \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_OPTIONAL, $description, \Rector\ChangesReporting\Output\ConsoleOutputFormatter::NAME);
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_NO_PROGRESS_BAR, null, \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Hide progress bar. Useful e.g. for nicer CI output.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_NO_DIFFS, null, \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Hide diffs of changed files. Useful e.g. for nicer CI output.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_OUTPUT_FILE, null, \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Location for file to dump result in. Useful for Docker or automated processes');
        $this->addOption(\Rector\Core\Configuration\Option::CACHE_DEBUG, null, \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Debug changed file cache');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_CLEAR_CACHE, null, \RectorPrefix20210531\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Clear unchaged files cache');
    }
    protected function execute(\RectorPrefix20210531\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210531\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
     ...
     * @return void
     */
    protected function initialize(\RectorPrefix20210531\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210531\Symfony\Component\Console\Output\OutputInterface $output)
    {
        $application = $this->getApplication();
        if (!$application instanceof \RectorPrefix20210531\Symfony\Component\Console\Application) {
    ...
    private function resolveReturnCode(\Rector\Core\ValueObject\ProcessResult $processResult) : int
    {
        ...
        if ($processResult->getErrors() !== []) {
            return \RectorPrefix20210531\Symplify\PackageBuilder\Console\ShellCode::ERROR;
        }
        // inverse error code for CI dry-run
        if (!$this->configuration->isDryRun()) {
            return \RectorPrefix20210531\Symplify\PackageBuilder\Console\ShellCode::SUCCESS;
        }
        return $processResult->getFileDiffs() === [] ? \RectorPrefix20210531\Symplify\PackageBuilder\Console\ShellCode::SUCCESS : \RectorPrefix20210531\Symplify\PackageBuilder\Console\ShellCode::ERROR;
    }
    /**
     * @param File[] $files
     * @return void
