...
namespace RectorPrefix20210519\Symplify\ConsoleColorDiff\Console\Formatter;
...
final class ColorConsoleDiffFormatter
{
    ...
    {
        $escapedDiff = \RectorPrefix20210519\Symfony\Component\Console\Formatter\OutputFormatter::escape(\rtrim($diff));
        $escapedDiffLines = \RectorPrefix20210519\Nette\Utils\Strings::split($escapedDiff, self::NEWLINES_REGEX);
    ...
    {
        return \RectorPrefix20210519\Nette\Utils\Strings::replace($string, self::PLUS_START_REGEX, '<fg=green>$1</fg=green>');
    }
    private function makeMinusLinesRed(string $string) : string
    {
        return \RectorPrefix20210519\Nette\Utils\Strings::replace($string, self::MINUT_START_REGEX, '<fg=red>$1</fg=red>');
    }
    private function makeAtNoteCyan(string $string) : string
    {
        return \RectorPrefix20210519\Nette\Utils\Strings::replace($string, self::AT_START_REGEX, '<fg=cyan>$1</fg=cyan>');
    }
}
