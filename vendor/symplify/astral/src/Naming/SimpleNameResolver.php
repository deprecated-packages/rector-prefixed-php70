...
namespace RectorPrefix20210525\Symplify\Astral\Naming;
...
final class SimpleNameResolver
{
    ...
    public function isName($node, string $desiredName) : bool
    {
        ...
        if (\RectorPrefix20210525\Nette\Utils\Strings::contains($desiredName, '*')) {
    ...
    public function resolveShortNameFromNode(\PhpParser\Node\Stmt\ClassLike $classLike)
    {
        ...
        if (\RectorPrefix20210525\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_REGEX)) {
            return null;
        }
    ...
    public function isNameMatch(\PhpParser\Node $node, string $desiredNameRegex) : bool
    {
        ...
        return (bool) \RectorPrefix20210525\Nette\Utils\Strings::match($name, $desiredNameRegex);
    }
