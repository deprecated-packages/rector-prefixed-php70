 ...
 */
namespace RectorPrefix20210503\Composer\Json;
...
class JsonManipulator
{
    ...
    {
        $decoded = \RectorPrefix20210503\Composer\Json\JsonFile::parseJson($this->contents);
        // no link of that type yet
        if (!isset($decoded[$type])) {
            return $this->addMainKey($type, array($package => $constraint));
        }
        $regex = '{' . self::$DEFINES . '^(?P<start>\\s*\\{\\s*(?:(?&string)\\s*:\\s*(?&json)\\s*,\\s*)*?)' . '(?P<property>' . \preg_quote(\RectorPrefix20210503\Composer\Json\JsonFile::encode($type)) . '\\s*:\\s*)(?P<value>(?&json))(?P<end>.*)}sx';
        ...
        if ($this->pregMatch($regex, $links, $packageMatches)) {
            ...
            $links = \preg_replace_callback('{' . self::$DEFINES . '"' . $packageRegex . '"(?P<separator>\\s*:\\s*)(?&string)}ix', function ($m) use($existingPackage, $constraint) {
                return \RectorPrefix20210503\Composer\Json\JsonFile::encode(\str_replace('\\/', '/', $existingPackage)) . $m['separator'] . '"' . $constraint . '"';
            }, $links);
        } else {
            if ($this->pregMatch('#^\\s*\\{\\s*\\S+.*?(\\s*\\}\\s*)$#s', $links, $match)) {
                ...
                $links = \preg_replace(
                    ...
                    \addcslashes(',' . $this->newline . $this->indent . $this->indent . \RectorPrefix20210503\Composer\Json\JsonFile::encode($package) . ': ' . \RectorPrefix20210503\Composer\Json\JsonFile::encode($constraint) . $match[1], '\\$'),
                    $links
                );
            } else {
                // links empty
                $links = '{' . $this->newline . $this->indent . $this->indent . \RectorPrefix20210503\Composer\Json\JsonFile::encode($package) . ': ' . \RectorPrefix20210503\Composer\Json\JsonFile::encode($constraint) . $this->newline . $this->indent . '}';
            }
        }
        if (\true === $sortPackages) {
    ...
    private function sortPackages(array &$packages = array())
    {
        $prefix = function ($requirement) {
            if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($requirement)) {
                return \preg_replace(array('/^php/', '/^hhvm/', '/^ext/', '/^lib/', '/^\\D/'), array('0-$0', '1-$0', '2-$0', '3-$0', '4-$0'), $requirement);
            }
    ...
    public function addRepository($name, $config, $append = \true)
    {
    ...
    public function addSubNode($mainNode, $name, $value, $append = \true)
    {
        $decoded = \RectorPrefix20210503\Composer\Json\JsonFile::parseJson($this->contents);
        $subName = null;
        ...
        $nodeRegex = '{' . self::$DEFINES . '^(?P<start> \\s* \\{ \\s* (?: (?&string) \\s* : (?&json) \\s* , \\s* )*?' . \preg_quote(\RectorPrefix20210503\Composer\Json\JsonFile::encode($mainNode)) . '\\s*:\\s*)(?P<content>(?&object))(?P<end>.*)}sx';
        try {
        ...
        if ($this->pregMatch($childRegex, $children, $matches)) {
            ...
            if (!empty($match['content'])) {
                ...
                if ($append) {
                    $children = \preg_replace('#' . $whitespace . '}$#', \addcslashes(',' . $this->newline . $this->indent . $this->indent . \RectorPrefix20210503\Composer\Json\JsonFile::encode($name) . ': ' . $this->format($value, 1) . $whitespace . '}', '\\$'), $children);
                } else {
                    $whitespace = '';
                    if (!empty($match['leadingspace'])) {
                        $whitespace = $match['leadingspace'];
                    }
                    $children = \preg_replace('#^{' . $whitespace . '#', \addcslashes('{' . $whitespace . \RectorPrefix20210503\Composer\Json\JsonFile::encode($name) . ': ' . $this->format($value, 1) . ',' . $this->newline . $this->indent . $this->indent, '\\$'), $children);
                }
            } else {
                if ($subName !== null) {
                    $value = array($subName => $value);
                ...
                $children = '{' . $this->newline . $this->indent . $this->indent . \RectorPrefix20210503\Composer\Json\JsonFile::encode($name) . ': ' . $this->format($value, 1) . $whitespace . '}';
            }
        }
    ...
    public function addMainKey($key, $content)
    {
        ...
        // key exists already
        $regex = '{' . self::$DEFINES . '^(?P<start>\\s*\\{\\s*(?:(?&string)\\s*:\\s*(?&json)\\s*,\\s*)*?)' . '(?P<key>' . \preg_quote(\RectorPrefix20210503\Composer\Json\JsonFile::encode($key)) . '\\s*:\\s*(?&json))(?P<end>.*)}sx';
        if (isset($decoded[$key]) && $this->pregMatch($regex, $this->contents, $matches)) {
            // invalid match due to un-regexable content, abort
            if (!@\json_decode('{' . $matches['key'] . '}')) {
                return \false;
            }
            $this->contents = $matches['start'] . \RectorPrefix20210503\Composer\Json\JsonFile::encode($key) . ': ' . $content . $matches['end'];
            return \true;
        }
        // append at the end of the file and keep whitespace
        if ($this->pregMatch('#[^{\\s](\\s*)\\}$#', $this->contents, $match)) {
            $this->contents = \preg_replace('#' . $match[1] . '\\}$#', \addcslashes(',' . $this->newline . $this->indent . \RectorPrefix20210503\Composer\Json\JsonFile::encode($key) . ': ' . $content . $this->newline . '}', '\\$'), $this->contents);
            return \true;
        }
        // append at the end of the file
        $this->contents = \preg_replace('#\\}$#', \addcslashes($this->indent . \RectorPrefix20210503\Composer\Json\JsonFile::encode($key) . ': ' . $content . $this->newline . '}', '\\$'), $this->contents);
        return \true;
    }
    public function removeMainKey($key)
    {
        ...
        // key exists already
        $regex = '{' . self::$DEFINES . '^(?P<start>\\s*\\{\\s*(?:(?&string)\\s*:\\s*(?&json)\\s*,\\s*)*?)' . '(?P<removal>' . \preg_quote(\RectorPrefix20210503\Composer\Json\JsonFile::encode($key)) . '\\s*:\\s*(?&json))\\s*,?\\s*(?P<end>.*)}sx';
        if ($this->pregMatch($regex, $this->contents, $matches)) {
            // invalid match due to un-regexable content, abort
    ...
    public function format($data, $depth = 0)
    {
        if (\is_array($data)) {
            ...
            $elems = array();
            foreach ($data as $key => $val) {
                $elems[] = \str_repeat($this->indent, $depth + 2) . \RectorPrefix20210503\Composer\Json\JsonFile::encode($key) . ': ' . $this->format($val, $depth + 1);
            }
            return $out . \implode(',' . $this->newline, $elems) . $this->newline . \str_repeat($this->indent, $depth + 1) . '}';
        }
        return \RectorPrefix20210503\Composer\Json\JsonFile::encode($data);
    }
