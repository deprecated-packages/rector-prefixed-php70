...
use RectorPrefix20210503\Composer\Config\ConfigSourceInterface;
...
class Config
{
    ...
    public static $defaultRepositories = array('packagist.org' => array('type' => 'composer', 'url' => 'https://repo.packagist.org'));
    private $config;
    private $baseDir;
     ...
     */
    public function __construct($useEnvironment = \true, $baseDir = null)
    {
        // load defaults
    ...
    public function setConfigSource(\RectorPrefix20210503\Composer\Config\ConfigSourceInterface $source)
    {
    ...
    public function setAuthConfigSource(\RectorPrefix20210503\Composer\Config\ConfigSourceInterface $source)
    {
    ...
    public function get($key, $flags = 0)
    {
        switch ($key) {
            ...
            case 'capath':
                ...
                $val = \RectorPrefix20210503\Composer\Util\Platform::expandPath($val);
                if (\substr($key, -4) !== '-dir') {
                    return $val;
                }
            ...
            case 'discard-changes':
                if ($env = $this->getComposerEnv('COMPOSER_DISCARD_CHANGES')) {
                    if (!\in_array($env, array('stash', 'true', 'false', '1', '0'), \true)) {
                ...
                if (!\in_array($this->config[$key], array(\true, \false, 'stash'), \true)) {
    ...
    {
        $all = array('repositories' => $this->getRepositories());
    ...
    public function raw()
    {
        return array('repositories' => $this->getRepositories(), 'config' => $this->config);
    }
    /**
     ...
     */
    public function prohibitUrlByConfig($url, \RectorPrefix20210503\Composer\IO\IOInterface $io = null)
    {
        ...
        if (\in_array($scheme, array('http', 'git', 'ftp', 'svn'))) {
            if ($this->get('secure-http')) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException("Your configuration does not allow connections to {$url}. See https://getcomposer.org/doc/06-config.md#secure-http for details.");
            }
            if ($io) {
    ...
    public static function disableProcessTimeout()
    {
        ...
        \RectorPrefix20210503\Composer\Util\ProcessExecutor::setTimeout(0);
    }
}
