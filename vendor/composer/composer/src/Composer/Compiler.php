...
use RectorPrefix20210503\Seld\PharUtils\Timestamps;
use RectorPrefix20210503\Seld\PharUtils\Linter;
/**
...
class Compiler
{
    ...
    public function compile($pharFile = 'composer.phar')
    {
        ...
        $process = new \RectorPrefix20210503\Symfony\Component\Process\Process('git log --pretty="%H" -n1 HEAD', __DIR__);
        if ($process->run() != 0) {
            throw new \RuntimeException('Can\'t run git log. You must ensure to run compile from composer git repository clone and that git binary is available.');
        }
        $this->version = \trim($process->getOutput());
        $process = new \RectorPrefix20210503\Symfony\Component\Process\Process('git log -n1 --pretty=%ci HEAD', __DIR__);
        if ($process->run() != 0) {
            throw new \RuntimeException('Can\'t run git log. You must ensure to run compile from composer git repository clone and that git binary is available.');
        }
        $this->versionDate = new \DateTime(\trim($process->getOutput()));
        $this->versionDate->setTimezone(new \DateTimeZone('UTC'));
        $process = new \RectorPrefix20210503\Symfony\Component\Process\Process('git describe --tags --exact-match HEAD');
        if ($process->run() == 0) {
            $this->version = \trim($process->getOutput());
        } else {
            // get branch-alias defined in composer.json for dev-master (if any)
            $localConfig = __DIR__ . '/../../composer.json';
            $file = new \RectorPrefix20210503\Composer\Json\JsonFile($localConfig);
        ...
        $finder->files()->ignoreVCS(\true)->name('*.php')->notName('Compiler.php')->notName('ClassLoader.php')->in(__DIR__ . '/..')->sort($finderSort);
        ...
        // Add vendor files
        $finder = new \RectorPrefix20210503\Symfony\Component\Finder\Finder();
        $finder->files()->ignoreVCS(\true)->notPath('/\\/(composer\\.(json|lock)|[A-Z]+\\.md|\\.gitignore|appveyor.yml|phpunit\\.xml\\.dist|phpstan\\.neon\\.dist|phpstan-config\\.neon)$/')->notPath('/bin\\/(jsonlint|validate-json|simple-phpunit)(\\.bat)?$/')->notPath('symfony/debug/Resources/ext/')->notPath('justinrainbow/json-schema/demo/')->notPath('justinrainbow/json-schema/dist/')->notPath('composer/installed.json')->notPath('composer/LICENSE')->exclude('Tests')->exclude('tests')->exclude('docs')->in(__DIR__ . '/../../vendor/')->sort($finderSort);
        $extraFiles = array(\realpath(__DIR__ . '/../../vendor/composer/spdx-licenses/res/spdx-exceptions.json'), \realpath(__DIR__ . '/../../vendor/composer/spdx-licenses/res/spdx-licenses.json'), \realpath(\RectorPrefix20210503\Composer\CaBundle\CaBundle::getBundledCaBundlePath()), \realpath(__DIR__ . '/../../vendor/symfony/console/Resources/bin/hiddeninput.exe'), \realpath(__DIR__ . '/../../vendor/symfony/polyfill-mbstring/Resources/mb_convert_variables.php8'));
        $unexpectedFiles = array();
        foreach ($finder as $file) {
            if (\in_array(\realpath($file), $extraFiles, \true)) {
                unset($extraFiles[\array_search(\realpath($file), $extraFiles, \true)]);
        ...
        $util = new \RectorPrefix20210503\Seld\PharUtils\Timestamps($pharFile);
        $util->updateTimestamps($this->versionDate);
        $util->save($pharFile, \Phar::SHA1);
        \RectorPrefix20210503\Seld\PharUtils\Linter::lint($pharFile);
    }
    /**
    ...
    private function addFile($phar, $file, $strip = \true)
    {
        ...
        if ($path === 'src/Composer/Composer.php') {
            $content = \strtr($content, array('@package_version@' => $this->version, '@package_branch_alias_version@' => $this->branchAliasVersion, '@release_date@' => $this->versionDate->format('Y-m-d H:i:s')));
