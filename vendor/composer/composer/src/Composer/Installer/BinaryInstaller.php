...
class BinaryInstaller
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, $binDir, $binCompat, \RectorPrefix20210503\Composer\Util\Filesystem $filesystem = null)
    {
        $this->binDir = $binDir;
    ...
    public function installBinaries(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $installPath, $warnOnOverwrite = \true)
    {
        ...
        foreach ($binaries as $bin) {
            ...
            if (\file_exists($link)) {
                if (\is_link($link)) {
                    ...
                    \RectorPrefix20210503\Composer\Util\Silencer::call('chmod', $link, 0777 & ~\umask());
                }
                if ($warnOnOverwrite) {
    ...
    public function removeBinaries(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        $this->initializeBinDir();
        $binaries = $this->getBinaries($package);
        if (!$binaries) {
        ...
        if (\is_dir($this->binDir) && $this->filesystem->isDirEmpty($this->binDir)) {
            \RectorPrefix20210503\Composer\Util\Silencer::call('rmdir', $this->binDir);
        }
    }
    ...
    protected function getBinaries(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        return $package->getBinaries();
    }
    protected function installFullBinaries($binPath, $link, $bin, \RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
    ...
    protected function generateWindowsProxyCode($bin, $link)
    {
        ...
        return "@ECHO OFF\r\n" . "setlocal DISABLEDELAYEDEXPANSION\r\n" . "SET BIN_TARGET=%~dp0/" . \trim(\RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($binPath), '"\'') . "\r\n" . "{$caller} \"%BIN_TARGET%\" %*\r\n";
    }
    protected function generateUnixyProxyCode($bin, $link)
    {
        $binPath = $this->filesystem->findShortestPath($link, $bin);
        $binDir = \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape(\dirname($binPath));
