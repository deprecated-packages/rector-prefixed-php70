...
use RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasInstalledOperation;
use RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasUninstalledOperation;
...
class InstallationManager
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\Util\Loop $loop, \RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\EventDispatcher\EventDispatcher $eventDispatcher = null)
    {
        $this->loop = $loop;
        $this->io = $io;
     ...
     */
    public function addInstaller(\RectorPrefix20210503\Composer\Installer\InstallerInterface $installer)
    {
     ...
     * @param InstallerInterface $installer installer instance
     */
    public function removeInstaller(\RectorPrefix20210503\Composer\Installer\InstallerInterface $installer)
    {
    ...
    public function disablePlugins()
    {
        foreach ($this->installers as $i => $installer) {
            if (!$installer instanceof \RectorPrefix20210503\Composer\Installer\PluginInstaller) {
                continue;
            }
     ...
     * @return bool
     */
    public function isPackageInstalled(\RectorPrefix20210503\Composer\Repository\InstalledRepositoryInterface $repo, \RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
    ...
    public function ensureBinariesPresence(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        ...
        if ($installer instanceof \RectorPrefix20210503\Composer\Installer\BinaryPresenceInterface) {
     ...
     */
    public function execute(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, array $operations, $devMode = \true, $runScripts = \true)
    {
        ...
        $runCleanup = function () use(&$cleanupPromises, $loop) {
            ...
            foreach ($cleanupPromises as $cleanup) {
                $promises[] = new \RectorPrefix20210503\React\Promise\Promise(function ($resolve, $reject) use($cleanup) {
                    $promise = $cleanup();
        ...
        if ($handleInterruptsUnix) {
            ...
            \pcntl_signal(\SIGINT, function ($sig) use($runCleanup, $prevHandler, $io) {
                $io->writeError('Received SIGINT, aborting', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                $runCleanup();
                if (!\in_array($prevHandler, array(\SIG_DFL, \SIG_IGN), \true)) {
        ...
        if ($handleInterruptsWindows) {
            $windowsHandler = function ($event) use($runCleanup, $io) {
                ...
                $io->writeError('Received CTRL+C, aborting', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                $runCleanup();
                exit(130);
            };
        ...
        try {
            ...
            foreach ($operations as $index => $operation) {
                if (\in_array($operation->getOperationType(), array('update', 'install'), \true)) {
                    $package = $operation->getOperationType() === 'update' ? $operation->getTargetPackage() : $operation->getPackage();
                    if ($package->getType() === 'composer-plugin' && ($extra = $package->getExtra()) && isset($extra['plugin-modifies-downloads']) && $extra['plugin-modifies-downloads'] === \true) {
                        if ($batch) {
                            $batches[] = $batch;
                        }
     ...
     */
    private function downloadAndExecuteBatch(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, array $operations, array &$cleanupPromises, $devMode, $runScripts, array $allOperations)
    {
        $promises = array();
        foreach ($operations as $index => $operation) {
     ...
     * @param array $allOperations Complete list of operations to be executed in the install job, used for event listeners
     */
    private function executeBatch(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, array $operations, array $cleanupPromises, $devMode, $runScripts, array $allOperations)
    {
        $promises = array();
        ...
        foreach ($operations as $index => $operation) {
            ...
            })->then($cleanupPromises[$index])->then(function () use($installManager, $devMode, $repo) {
    ...
    {
        $progress = null;
        if ($this->outputProgress && $this->io instanceof \RectorPrefix20210503\Composer\IO\ConsoleIO && !$this->io->isDebug() && \count($promises) > 1) {
     ...
     */
    public function install(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, \RectorPrefix20210503\Composer\DependencyResolver\Operation\InstallOperation $operation)
    {
        $package = $operation->getPackage();
        $installer = $this->getInstaller($package->getType());
     ...
     */
    public function update(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, \RectorPrefix20210503\Composer\DependencyResolver\Operation\UpdateOperation $operation)
    {
     ...
     */
    public function uninstall(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, \RectorPrefix20210503\Composer\DependencyResolver\Operation\UninstallOperation $operation)
    {
        $package = $operation->getPackage();
        $installer = $this->getInstaller($package->getType());
     ...
     */
    public function markAliasInstalled(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, \RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasInstalledOperation $operation)
    {
        $package = $operation->getPackage();
     ...
     */
    public function markAliasUninstalled(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, \RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasUninstalledOperation $operation)
    {
        $package = $operation->getPackage();
    ...
    public function notifyInstalls(\RectorPrefix20210503\Composer\IO\IOInterface $io)
    {
        $promises = array();
        try {
            foreach ($this->notifiablePackages as $repoUrl => $packages) {
                ...
                if (\strpos($repoUrl, '%package%')) {
                    foreach ($packages as $package) {
                        ...
                        $opts = array('retry-auth-failure' => \false, 'http' => array('method' => 'POST', 'header' => array('Content-type: application/x-www-form-urlencoded'), 'content' => \http_build_query($params, '', '&'), 'timeout' => 3));
                ...
                $opts = array('retry-auth-failure' => \false, 'http' => array('method' => 'POST', 'header' => array('Content-Type: application/json'), 'content' => \json_encode($postData), 'timeout' => 6));
    ...
    private function markForNotification(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
