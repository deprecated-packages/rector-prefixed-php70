 ...
 */
namespace RectorPrefix20210503\Composer\Plugin;

use RectorPrefix20210503\Composer\Composer;
use RectorPrefix20210503\Composer\EventDispatcher\EventSubscriberInterface;
...
use RectorPrefix20210503\Composer\Plugin\Capability\Capability;
...
class PluginManager
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Composer $composer, \RectorPrefix20210503\Composer\Composer $globalComposer = null, $disablePlugins = \false)
    {
        $this->io = $io;
    ...
    public function loadInstalledPlugins()
    {
        ...
        if ($globalRepo) {
            $this->loadRepository($globalRepo, \true);
        }
    }
    /**
     ...
     */
    public function registerPackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $failOnMissingClasses = \false, $isGlobalPlugin = \false)
    {
        if ($this->disablePlugins) {
            return;
        }
        if ($package->getType() === 'composer-plugin') {
            ...
            $currentPluginApiConstraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $this->versionParser->normalize($currentPluginApiVersion));
        ...
        $rootPackageRepo = new \RectorPrefix20210503\Composer\Repository\RootPackageRepository($rootPackage);
        $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($localRepo, $rootPackageRepo));
        if ($globalRepo) {
        ...
        foreach ($classes as $class) {
            if (\class_exists($class, \false)) {
                ...
                $code = \strtr($code, array('__FILE__' => \var_export($path, \true), '__DIR__' => \var_export(\dirname($path), \true), '__CLASS__' => \var_export($class, \true)));
     ...
     * @throws \UnexpectedValueException
     */
    public function deactivatePackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        if ($this->disablePlugins) {
            return;
        }
     ...
     * @throws \UnexpectedValueException
     */
    public function uninstallPackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        if ($this->disablePlugins) {
            return;
        }
    ...
    {
        return \RectorPrefix20210503\Composer\Plugin\PluginInterface::PLUGIN_API_VERSION;
    }
    /**
     ...
     */
    public function addPlugin(\RectorPrefix20210503\Composer\Plugin\PluginInterface $plugin, $isGlobalPlugin = \false, \RectorPrefix20210503\Composer\Package\PackageInterface $sourcePackage = null)
    {
        $details = array();
        if ($sourcePackage) {
        ...
        $this->io->writeError('Loading plugin ' . \get_class($plugin) . ($details ? ' (' . \implode(', ', $details) . ')' : ''), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
        $this->plugins[] = $plugin;
        $plugin->activate($this->composer, $this->io);
        if ($plugin instanceof \RectorPrefix20210503\Composer\EventDispatcher\EventSubscriberInterface) {
     ...
     * @param PluginInterface $plugin plugin instance
     */
    public function removePlugin(\RectorPrefix20210503\Composer\Plugin\PluginInterface $plugin)
    {
        $index = \array_search($plugin, $this->plugins, \true);
        if ($index === \false) {
            return;
        }
        $this->io->writeError('Unloading plugin ' . \get_class($plugin), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
     ...
     * @param PluginInterface $plugin plugin instance
     */
    public function uninstallPlugin(\RectorPrefix20210503\Composer\Plugin\PluginInterface $plugin)
    {
        $this->io->writeError('Uninstalling plugin ' . \get_class($plugin), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
     ...
     */
    private function loadRepository(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, $isGlobalRepo)
    {
        $packages = $repo->getPackages();
        $sortedPackages = \RectorPrefix20210503\Composer\Util\PackageSorter::sortPackages($packages);
        foreach ($sortedPackages as $package) {
            if (!$package instanceof \RectorPrefix20210503\Composer\Package\CompletePackage) {
                continue;
            }
     ...
     */
    private function collectDependencies(\RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, array $collected, \RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
     ...
     */
    private function getInstallPath(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $global = \false)
    {
        if (!$global) {
     ...
     */
    protected function getCapabilityImplementationClassName(\RectorPrefix20210503\Composer\Plugin\PluginInterface $plugin, $capability)
    {
        if (!$plugin instanceof \RectorPrefix20210503\Composer\Plugin\Capable) {
            return null;
        }
        ...
        if (\array_key_exists($capability, $capabilities) && (empty($capabilities[$capability]) || !\is_string($capabilities[$capability]) || !\trim($capabilities[$capability]))) {
     ...
     */
    public function getPluginCapability(\RectorPrefix20210503\Composer\Plugin\PluginInterface $plugin, $capabilityClassName, array $ctorArgs = array())
    {
        if ($capabilityClass = $this->getCapabilityImplementationClassName($plugin, $capabilityClassName)) {
            ...
            if (!$capabilityObj instanceof \RectorPrefix20210503\Composer\Plugin\Capability\Capability || !$capabilityObj instanceof $capabilityClassName) {
