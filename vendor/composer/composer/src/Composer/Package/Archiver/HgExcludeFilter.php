...
use RectorPrefix20210503\Symfony\Component\Finder;
/**
 * An exclude filter that processes hgignore files
 *
 * @author Nils Adermann <naderman@naderman.de>
 */
class HgExcludeFilter extends \RectorPrefix20210503\Composer\Package\Archiver\BaseExcludeFilter
{
    ...
    public function __construct($sourcePath)
    {
        ...
        if (\file_exists($sourcePath . '/.hgignore')) {
            $this->excludePatterns = $this->parseLines(\file($sourcePath . '/.hgignore'), array($this, 'parseHgIgnoreLine'));
        }
    }
    /**
    ...
    {
        $pattern = '#' . \substr(\RectorPrefix20210503\Symfony\Component\Finder\Glob::toRegex($line), 2, -1) . '#';
        $pattern = \str_replace('[^/]*', '.*', $pattern);
        return array($pattern, \false, \true);
    }
    /**
