 ...
 */
class AliasPackage extends \RectorPrefix20210503\Composer\Package\BasePackage implements \RectorPrefix20210503\Composer\Package\CompletePackageInterface
{
    protected $version;
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\Package\PackageInterface $aliasOf, $version, $prettyVersion)
    {
        parent::__construct($aliasOf->getName());
        $this->version = $version;
        $this->prettyVersion = $prettyVersion;
        $this->aliasOf = $aliasOf;
        $this->stability = \RectorPrefix20210503\Composer\Package\Version\VersionParser::parseStability($version);
        $this->dev = $this->stability === 'dev';
        foreach (\RectorPrefix20210503\Composer\Package\Link::$TYPES as $type) {
    ...
    protected function replaceSelfVersionDependencies(array $links, $linkType)
    {
        ...
        if ($prettyVersion === \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
            $prettyVersion = $this->aliasOf->getPrettyVersion();
        }
        if (\in_array($linkType, array(\RectorPrefix20210503\Composer\Package\Link::TYPE_CONFLICT, \RectorPrefix20210503\Composer\Package\Link::TYPE_PROVIDE, \RectorPrefix20210503\Composer\Package\Link::TYPE_REPLACE), \true)) {
            $newLinks = array();
            foreach ($links as $link) {
                // link is self.version, but must be replacing also the replaced version
                if ('self.version' === $link->getPrettyConstraint()) {
                    $newLinks[] = new \RectorPrefix20210503\Composer\Package\Link($link->getSource(), $link->getTarget(), $constraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $this->version), $linkType, $prettyVersion);
            ...
            foreach ($links as $index => $link) {
                if ('self.version' === $link->getPrettyConstraint()) {
                    if ($linkType === \RectorPrefix20210503\Composer\Package\Link::TYPE_REQUIRE) {
                        $this->hasSelfVersionRequires = \true;
                    }
                    $links[$index] = new \RectorPrefix20210503\Composer\Package\Link($link->getSource(), $link->getTarget(), $constraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $this->version), $linkType, $prettyVersion);
                    $constraint->setPrettyString($prettyVersion);
                }
            }
        }
