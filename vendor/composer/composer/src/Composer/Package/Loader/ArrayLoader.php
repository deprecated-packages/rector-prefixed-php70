...
use RectorPrefix20210503\Composer\Package;
 ...
 */
class ArrayLoader implements \RectorPrefix20210503\Composer\Package\Loader\LoaderInterface
{
    protected $versionParser;
    protected $loadOptions;
    public function __construct(\RectorPrefix20210503\Composer\Package\Version\VersionParser $parser = null, $loadOptions = \false)
    {
        if (!$parser) {
    ...
    public function load(array $config, $class = 'RectorPrefix20210503\\Composer\\Package\\CompletePackage')
    {
        ...
        foreach (\RectorPrefix20210503\Composer\Package\BasePackage::$supportedLinkTypes as $type => $opts) {
    ...
    private function createObject(array $config, $class)
    {
        ...
        if (isset($config['version_normalized'])) {
            ...
            if ($version === \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
    ...
    private function configureObject($package, array $config)
    {
        ...
        if ($package instanceof \RectorPrefix20210503\Composer\Package\CompletePackageInterface) {
        ...
        if ($aliasNormalized = $this->getBranchAlias($config)) {
            ...
            if ($package instanceof \RectorPrefix20210503\Composer\Package\RootPackageInterface) {
                return new \RectorPrefix20210503\Composer\Package\RootAliasPackage($package, $aliasNormalized, $prettyAlias);
            }
            return new \RectorPrefix20210503\Composer\Package\AliasPackage($package, $aliasNormalized, $prettyAlias);
        }
        return $package;
    }
    ...
    private function createLink($source, $sourceVersion, $description, $target, $prettyConstraint)
    {
        if (!\is_string($prettyConstraint)) {
            throw new \UnexpectedValueException('Link constraint in ' . $source . ' ' . $description . ' > ' . $target . ' should be a string, got ' . \gettype($prettyConstraint) . ' (' . \var_export($prettyConstraint, \true) . ')');
        }
        ...
        return new \RectorPrefix20210503\Composer\Package\Link($source, $target, $parsedConstraint, $description, $prettyConstraint);
    }
    /**
    ...
    public function getBranchAlias(array $config)
    {
        ...
        if (isset($config['extra']['branch-alias']) && \is_array($config['extra']['branch-alias'])) {
            foreach ($config['extra']['branch-alias'] as $sourceBranch => $targetBranch) {
                ...
                if ($targetBranch === \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
                    $validatedTargetBranch = \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS;
                } else {
        ...
        if (isset($config['default-branch']) && $config['default-branch'] === \true && \false === $this->versionParser->parseNumericAliasPrefix($config['version'])) {
            return \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS;
        }
    }
}
