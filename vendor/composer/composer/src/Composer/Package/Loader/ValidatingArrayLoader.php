 ...
 */
class ValidatingArrayLoader implements \RectorPrefix20210503\Composer\Package\Loader\LoaderInterface
{
    const CHECK_ALL = 3;
    ...
    private $strictName;
    private $flags;
    public function __construct(\RectorPrefix20210503\Composer\Package\Loader\LoaderInterface $loader, $strictName = \true, \RectorPrefix20210503\Composer\Package\Version\VersionParser $parser = null, $flags = 0)
    {
        $this->loader = $loader;
        $this->versionParser = $parser ?: new \RectorPrefix20210503\Composer\Package\Version\VersionParser();
        $this->strictName = $strictName;
        $this->flags = $flags;
    }
    public function load(array $config, $class = 'RectorPrefix20210503\\Composer\\Package\\CompletePackage')
    {
        $this->errors = array();
        $this->warnings = array();
        $this->config = $config;
        ...
        if ($this->strictName) {
            $this->validateRegex('name', '[A-Za-z0-9][A-Za-z0-9_.-]*/[A-Za-z0-9][A-Za-z0-9_.-]*', \true);
        } else {
        ...
        if (isset($this->config['license']) && (!$releaseDate || $releaseDate->getTimestamp() >= \strtotime('-8days'))) {
            if (\is_array($this->config['license']) || \is_string($this->config['license'])) {
                ...
                $licenseValidator = new \RectorPrefix20210503\Composer\Spdx\SpdxLicenses();
                foreach ($licenses as $license) {
                    ...
                    if (!$licenseValidator->validate($licenseToValidate)) {
                        if ($licenseValidator->validate(\trim($licenseToValidate))) {
        ...
        $unboundConstraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', '10000000-dev');
        $stableConstraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', '1.0.0');
            ...
            if ($this->validateArray($linkType) && isset($this->config[$linkType])) {
                foreach ($this->config[$linkType] as $package => $constraint) {
                    ...
                    if (!\is_string($constraint)) {
                        ...
                        if ($this->flags & self::CHECK_UNBOUND_CONSTRAINTS && 'require' === $linkType && $linkConstraint->matches($unboundConstraint) && !\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($package)) {
                            $this->warnings[] = $linkType . '.' . $package . ' : unbound version constraints (' . $constraint . ') should be avoided';
                        } elseif ($this->flags & self::CHECK_STRICT_CONSTRAINTS && 'require' === $linkType && \strpos($linkConstraint, '=') === 0 && $stableConstraint->versionCompare($stableConstraint, $linkConstraint, '<=')) {
        ...
        if ($this->validateString('minimum-stability') && !empty($this->config['minimum-stability'])) {
            if (!isset(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities[$this->config['minimum-stability']])) {
                $this->errors[] = 'minimum-stability : invalid value (' . $this->config['minimum-stability'] . '), must be one of ' . \implode(', ', \array_keys(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities));
        ...
        if ($this->errors) {
            throw new \RectorPrefix20210503\Composer\Package\Loader\InvalidPackageException($this->errors, $this->warnings, $config);
        }
    ...
    public static function hasPackageNamingError($name, $isLink = \false)
    {
        ...
        if (\preg_match('{[A-Z]}', $name)) {
            ...
            $suggestName = \preg_replace('{(?:([a-z])([A-Z])|([A-Z])([A-Z][a-z]))}', 'RectorPrefix20210503\\1\\3-\\2\\4', $name);
    ...
    private function validateString($property, $mandatory = \false)
    {
        ...
        if (!isset($this->config[$property]) || \trim($this->config[$property]) === '') {
            if ($mandatory) {
