 ...
 */
namespace RectorPrefix20210503\Composer\Package\Version;
...
class VersionSelector
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\Repository\RepositorySet $repositorySet, \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo = null)
    {
        $this->repositorySet = $repositorySet;
        if ($platformRepo) {
            foreach ($platformRepo->getPackages() as $package) {
                $this->platformConstraints[$package->getName()][] = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $package->getVersion());
            }
        }
    }
    /**
    ...
    {
        if (!isset(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities[$preferredStability])) {
        ...
        if ($this->platformConstraints && \true !== $ignorePlatformReqs) {
            ...
            $candidates = \array_filter($candidates, function ($pkg) use($platformConstraints, $ignorePlatformReqs) {
                ...
                foreach ($reqs as $name => $link) {
                    if (!\in_array($name, $ignorePlatformReqs, \true) && isset($platformConstraints[$name])) {
        ...
        $minPriority = \RectorPrefix20210503\Composer\Package\BasePackage::$stabilities[$preferredStability];
        ...
        if ($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && $package->getVersion() === \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
     ...
     * @return string
     */
    public function findRecommendedRequireVersion(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        $version = $package->getVersion();
        if (!$package->isDev()) {
            return $this->transformVersion($version, $package->getPrettyVersion(), $package->getStability());
        }
        $loader = new \RectorPrefix20210503\Composer\Package\Loader\ArrayLoader($this->getParser());
        $dumper = new \RectorPrefix20210503\Composer\Package\Dumper\ArrayDumper();
        $extra = $loader->getBranchAlias($dumper->dump($package));
        if ($extra && $extra !== \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
    ...
    {
        if ($this->parser === null) {
            $this->parser = new \RectorPrefix20210503\Composer\Package\Version\VersionParser();
        }
        return $this->parser;
    }
}
