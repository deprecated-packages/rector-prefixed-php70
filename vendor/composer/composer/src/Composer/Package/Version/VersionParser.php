...
class VersionParser extends \RectorPrefix20210503\Composer\Semver\VersionParser
{
    ...
    public function parseNameVersionPairs(array $pairs)
    {
        ...
        for ($i = 0, $count = \count($pairs); $i < $count; $i++) {
            ...
            if (\false === \strpos($pair, ' ') && isset($pairs[$i + 1]) && \false === \strpos($pairs[$i + 1], '/') && !\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($pairs[$i + 1])) {
                $pair .= ' ' . $pairs[$i + 1];
                $i++;
            }
            if (\strpos($pair, ' ')) {
    ...
    public static function isUpgrade($normalizedFrom, $normalizedTo)
    {
        ...
        if (\in_array($normalizedFrom, array('dev-master', 'dev-trunk', 'dev-default'), \true)) {
            $normalizedFrom = \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS;
        }
        if (\in_array($normalizedTo, array('dev-master', 'dev-trunk', 'dev-default'), \true)) {
            $normalizedTo = \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS;
        }
        if (\strpos($normalizedFrom, 'dev-') === 0 || \strpos($normalizedTo, 'dev-') === 0) {
            return \true;
        }
        $sorted = \RectorPrefix20210503\Composer\Semver\Semver::sort(array($normalizedTo, $normalizedFrom));
