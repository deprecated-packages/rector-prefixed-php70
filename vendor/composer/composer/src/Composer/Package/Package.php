...
use RectorPrefix20210503\Composer\Util\ComposerMirror;
/**
 * Core package definitions that are needed to resolve dependencies and install packages
 *
 * @author Nils Adermann <naderman@naderman.de>
 */
class Package extends \RectorPrefix20210503\Composer\Package\BasePackage
{
    protected $type;
    protected $targetDir;
    ...
    protected function getUrls($url, $mirrors, $ref, $type, $urlType)
    {
        ...
        if ($mirrors) {
            foreach ($mirrors as $mirror) {
                if ($urlType === 'dist') {
                    $mirrorUrl = \RectorPrefix20210503\Composer\Util\ComposerMirror::processUrl($mirror['url'], $this->name, $this->version, $ref, $type);
                } elseif ($urlType === 'source' && $type === 'git') {
                    $mirrorUrl = \RectorPrefix20210503\Composer\Util\ComposerMirror::processGitUrl($mirror['url'], $this->name, $url, $type);
                } elseif ($urlType === 'source' && $type === 'hg') {
                    $mirrorUrl = \RectorPrefix20210503\Composer\Util\ComposerMirror::processHgUrl($mirror['url'], $this->name, $url, $type);
                } else {
                    continue;
                }
