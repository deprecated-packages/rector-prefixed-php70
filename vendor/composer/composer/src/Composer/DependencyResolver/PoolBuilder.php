...
use RectorPrefix20210503\Composer\Plugin\PrePoolCreateEvent;
...
use RectorPrefix20210503\Composer\Semver\Intervals;
/**
...
class PoolBuilder
{
     ...
     */
    public function __construct(array $acceptableStabilities, array $stabilityFlags, array $rootAliases, array $rootReferences, \RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\EventDispatcher\EventDispatcher $eventDispatcher = null)
    {
    ...
    public function buildPool(array $repositories, \RectorPrefix20210503\Composer\DependencyResolver\Request $request)
    {
        ...
        foreach ($request->getFixedOrLockedPackages() as $package) {
            ...
            $this->loadedPackages[$package->getName()] = new \RectorPrefix20210503\Composer\Semver\Constraint\MatchAllConstraint();
            ...
            foreach ($package->getReplaces() as $link) {
                $this->loadedPackages[$link->getTarget()] = new \RectorPrefix20210503\Composer\Semver\Constraint\MatchAllConstraint();
            }
            ...
            if ($package->getRepository() instanceof \RectorPrefix20210503\Composer\Repository\RootPackageRepository || $package->getRepository() instanceof \RectorPrefix20210503\Composer\Repository\PlatformRepository || \RectorPrefix20210503\Composer\Package\Version\StabilityFilter::isPackageAcceptable($this->acceptableStabilities, $this->stabilityFlags, $package->getNames(), $package->getStability())) {
        ...
        foreach ($this->packages as $i => $package) {
            ...
            if (!$package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
                ...
                foreach ($aliasedPackages as $packageOrAlias) {
                    if (\RectorPrefix20210503\Composer\Semver\CompilingMatcher::match($constraint, \RectorPrefix20210503\Composer\Semver\Constraint\Constraint::OP_EQ, $packageOrAlias->getVersion())) {
                        $found = \true;
                    }
                }
                if (!$found) {
        ...
        $pool = new \RectorPrefix20210503\Composer\DependencyResolver\Pool($this->packages, $this->unacceptableFixedOrLockedPackages);
        $this->aliasMap = array();
        ...
        $this->indexCounter = 0;
        \RectorPrefix20210503\Composer\Semver\Intervals::clear();
        return $pool;
    }
    private function markPackageNameForLoading(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, $name, \RectorPrefix20210503\Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
        ...
        if (isset($rootRequires[$name]) && !\RectorPrefix20210503\Composer\Semver\Intervals::isSubsetOf($constraint, $rootRequires[$name])) {
        ...
        if (!isset($this->loadedPackages[$name])) {
            ...
            if (isset($this->packagesToLoad[$name])) {
                ...
                if (\RectorPrefix20210503\Composer\Semver\Intervals::isSubsetOf($constraint, $this->packagesToLoad[$name])) {
                    return;
                }
                // extend the constraint to be loaded
                $constraint = \RectorPrefix20210503\Composer\Semver\Intervals::compactConstraint(\RectorPrefix20210503\Composer\Semver\Constraint\MultiConstraint::create(array($this->packagesToLoad[$name], $constraint), \false));
            }
        ...
        if (\RectorPrefix20210503\Composer\Semver\Intervals::isSubsetOf($constraint, $this->loadedPackages[$name])) {
            return;
        }
        ...
        $this->packagesToLoad[$name] = \RectorPrefix20210503\Composer\Semver\Intervals::compactConstraint(\RectorPrefix20210503\Composer\Semver\Constraint\MultiConstraint::create(array($this->loadedPackages[$name], $constraint), \false));
        unset($this->loadedPackages[$name]);
    }
    private function loadPackagesMarkedForLoading(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, $repositories)
    {
        foreach ($this->packagesToLoad as $name => $constraint) {
        ...
        foreach ($repositories as $repoIndex => $repository) {
            ...
            if ($repository instanceof \RectorPrefix20210503\Composer\Repository\PlatformRepository || $repository === $request->getLockedRepository()) {
                continue;
            }
    ...
    private function loadPackage(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\Package\PackageInterface $package, $propagateUpdate = \true)
    {
        ...
        if ($propagateUpdate && isset($this->rootAliases[$name][$package->getVersion()])) {
            ...
            $aliasPackage = new \RectorPrefix20210503\Composer\Package\AliasPackage($basePackage, $alias['alias_normalized'], $alias['alias']);
            ...
            if (isset($this->skippedLoad[$require])) {
                ...
                if ($propagateUpdate && $request->getUpdateAllowTransitiveDependencies()) {
                    if ($request->getUpdateAllowTransitiveRootDependencies() || !$this->isRootRequire($request, $this->skippedLoad[$require])) {
                        ...
                        $this->updateAllowWarned[$this->skippedLoad[$require]] = \true;
            ...
            foreach ($package->getReplaces() as $link) {
                ...
                if (isset($this->loadedPackages[$replace], $this->skippedLoad[$replace])) {
                    if ($request->getUpdateAllowTransitiveRootDependencies() || !$this->isRootRequire($request, $this->skippedLoad[$replace])) {
                        ...
                        $this->updateAllowWarned[$replace] = \true;
     ...
     * @return bool
     */
    private function isRootRequire(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, $name)
    {
        $rootRequires = $request->getRequires();
     ...
     * @return bool
     */
    private function isUpdateAllowed(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        foreach ($this->updateAllowList as $pattern => $void) {
            $patternRegexp = \RectorPrefix20210503\Composer\Package\BasePackage::packageNameToRegexp($pattern);
    ...
    private function warnAboutNonMatchingUpdateAllowList(\RectorPrefix20210503\Composer\DependencyResolver\Request $request)
    {
        foreach ($this->updateAllowList as $pattern => $void) {
            $patternRegexp = \RectorPrefix20210503\Composer\Package\BasePackage::packageNameToRegexp($pattern);
     ...
     */
    private function unlockPackage(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, $name)
    {
        if ($this->skippedLoad[$name] !== $name && isset($this->skippedLoad[$this->skippedLoad[$name]])) {
        ...
        foreach ($request->getLockedPackages() as $lockedPackage) {
            if (!$lockedPackage instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && $lockedPackage->getName() === $name) {
    ...
    private function removeLoadedPackage(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\Package\PackageInterface $package, $index)
    {
        unset($this->packages[$index]);
        if (isset($this->aliasMap[\spl_object_hash($package)])) {
