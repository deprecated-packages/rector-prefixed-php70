...
class Problem
{
     ...
     */
    public function addRule(\RectorPrefix20210503\Composer\DependencyResolver\Rule $rule)
    {
     ...
     * @return string
     */
    public function getPrettyString(\RectorPrefix20210503\Composer\Repository\RepositorySet $repositorySet, \RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\DependencyResolver\Pool $pool, $isVerbose, array $installedMap = array(), array $learnedPool = array())
    {
        ...
        if (\count($reasons) === 1) {
            ...
            if (!\in_array($rule->getReason(), array(\RectorPrefix20210503\Composer\DependencyResolver\Rule::RULE_ROOT_REQUIRE, \RectorPrefix20210503\Composer\DependencyResolver\Rule::RULE_FIXED), \true)) {
     ...
     * @internal
     */
    public static function formatDeduplicatedRules($rules, $indent, \RectorPrefix20210503\Composer\Repository\RepositorySet $repositorySet, \RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\DependencyResolver\Pool $pool, $isVerbose, array $installedMap = array(), array $learnedPool = array())
    {
        $messages = array();
        $templates = array();
        $parser = new \RectorPrefix20210503\Composer\Package\Version\VersionParser();
        $deduplicatableRuleTypes = array(\RectorPrefix20210503\Composer\DependencyResolver\Rule::RULE_PACKAGE_REQUIRES, \RectorPrefix20210503\Composer\DependencyResolver\Rule::RULE_PACKAGE_CONFLICT);
        foreach ($rules as $rule) {
            $message = $rule->getPrettyString($repositorySet, $request, $pool, $isVerbose, $installedMap, $learnedPool);
            if (\in_array($rule->getReason(), $deduplicatableRuleTypes, \true) && \preg_match('{^(?P<package>\\S+) (?P<version>\\S+) (?P<type>requires|conflicts)}', $message, $m)) {
    ...
    public function isCausedByLock(\RectorPrefix20210503\Composer\Repository\RepositorySet $repositorySet, \RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\DependencyResolver\Pool $pool)
    {
     ...
     */
    protected function addReason($id, \RectorPrefix20210503\Composer\DependencyResolver\Rule $reason)
    {
     ...
     * @internal
     */
    public static function getMissingPackageReason(\RectorPrefix20210503\Composer\Repository\RepositorySet $repositorySet, \RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\DependencyResolver\Pool $pool, $isVerbose, $packageName, $constraint = null)
    {
        // handle php/hhvm
        if ($packageName === 'php' || $packageName === 'php-64bit' || $packageName === 'hhvm') {
            ...
            if (\count($available)) {
                ...
                if ($firstAvailable instanceof \RectorPrefix20210503\Composer\Package\CompletePackageInterface && isset($extra['config.platform']) && $extra['config.platform'] === \true) {
        ...
        if ($packages = $repositorySet->findPackages($packageName, $constraint)) {
            ...
            if (isset($rootReqs[$packageName])) {
                $filtered = \array_filter($packages, function ($p) use($rootReqs, $packageName) {
                    return $rootReqs[$packageName]->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $p->getVersion()));
                });
                if (0 === \count($filtered)) {
            ...
            if ($lockedPackage) {
                $fixedConstraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $lockedPackage->getVersion());
                $filtered = \array_filter($packages, function ($p) use($fixedConstraint) {
                    return $fixedConstraint->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $p->getVersion()));
                });
                if (0 === \count($filtered)) {
            ...
            $nonLockedPackages = \array_filter($packages, function ($p) {
                return !$p->getRepository() instanceof \RectorPrefix20210503\Composer\Repository\LockArrayRepository;
            });
        ...
        if ($packages = $repositorySet->findPackages($packageName, $constraint, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_UNACCEPTABLE_STABILITIES)) {
            // we must first verify if a valid package would be found in a lower priority repository
            if ($allReposPackages = $repositorySet->findPackages($packageName, $constraint, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_SHADOWED_REPOSITORIES)) {
        ...
        if ($packages = $repositorySet->findPackages($packageName, null, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_UNACCEPTABLE_STABILITIES)) {
            // we must first verify if a valid package would be found in a lower priority repository
            if ($allReposPackages = $repositorySet->findPackages($packageName, $constraint, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_SHADOWED_REPOSITORIES)) {
    ...
    public static function getPackageList(array $packages, $isVerbose)
    {
        ...
        foreach ($packages as $package) {
            ...
            $prepared[$package->getName()]['versions'][$package->getVersion()] = $package->getPrettyVersion() . ($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage ? ' (alias of ' . $package->getAliasOf()->getPrettyVersion() . ')' : '');
        ...
        foreach ($prepared as $name => $package) {
            ...
            if (isset($package['versions'][\RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS], $hasDefaultBranch[$name])) {
                unset($package['versions'][\RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS]);
            }
