...
class Transaction
{
    ...
    {
        $packageSort = function (\RectorPrefix20210503\Composer\Package\PackageInterface $a, \RectorPrefix20210503\Composer\Package\PackageInterface $b) {
            // sort alias packages by the same name behind their non alias version
            if ($a->getName() == $b->getName()) {
                if ($a instanceof \RectorPrefix20210503\Composer\Package\AliasPackage != $b instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
                    return $a instanceof \RectorPrefix20210503\Composer\Package\AliasPackage ? -1 : 1;
                }
    ...
    protected function calculateOperations()
    {
            ...
            if (!isset($visited[\spl_object_hash($package)])) {
                ...
                if ($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
                    ...
                    if (isset($presentAliasMap[$aliasKey])) {
                        ...
                        $operations[] = new \RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasInstalledOperation($package);
                    }
                } else {
                    if (isset($presentPackageMap[$package->getName()])) {
                        ...
                        if ($package->getVersion() != $presentPackageMap[$package->getName()]->getVersion() || $package->getDistReference() !== $presentPackageMap[$package->getName()]->getDistReference() || $package->getSourceReference() !== $presentPackageMap[$package->getName()]->getSourceReference()) {
                            $operations[] = new \RectorPrefix20210503\Composer\DependencyResolver\Operation\UpdateOperation($source, $package);
                        }
                        unset($removeMap[$package->getName()]);
                    } else {
                        $operations[] = new \RectorPrefix20210503\Composer\DependencyResolver\Operation\InstallOperation($package);
                        unset($removeMap[$package->getName()]);
                    }
                }
            }
        ...
        foreach ($removeMap as $name => $package) {
            \array_unshift($operations, new \RectorPrefix20210503\Composer\DependencyResolver\Operation\UninstallOperation($package));
        }
        foreach ($removeAliasMap as $nameVersion => $package) {
            $operations[] = new \RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasUninstalledOperation($package);
        }
    ...
    protected function getProvidersInResult(\RectorPrefix20210503\Composer\Package\Link $link)
    {
    ...
    private function movePluginsToFront(array $operations)
    {
        ...
        foreach (\array_reverse($operations, \true) as $idx => $op) {
            if ($op instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\InstallOperation) {
                $package = $op->getPackage();
            } elseif ($op instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\UpdateOperation) {
                $package = $op->getTargetPackage();
            } else {
                continue;
            }
            $isDownloadsModifyingPlugin = $package->getType() === 'composer-plugin' && ($extra = $package->getExtra()) && isset($extra['plugin-modifies-downloads']) && $extra['plugin-modifies-downloads'] === \true;
    ...
    private function moveUninstallsToFront(array $operations)
    {
        ...
        foreach ($operations as $idx => $op) {
            if ($op instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\UninstallOperation || $op instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\MarkAliasUninstalledOperation) {
                $uninstOps[] = $op;
                unset($operations[$idx]);
            }
        }
