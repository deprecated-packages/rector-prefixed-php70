...
abstract class Rule
{
    ...
    public abstract function getLiterals();
    public abstract function getHash();
    public abstract function equals(\RectorPrefix20210503\Composer\DependencyResolver\Rule $rule);
    ...
    public abstract function isAssertion();
    public function isCausedByLock(\RectorPrefix20210503\Composer\Repository\RepositorySet $repositorySet, \RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\DependencyResolver\Pool $pool)
    {
        if ($this->getReason() === self::RULE_PACKAGE_REQUIRES) {
            if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($this->reasonData->getTarget())) {
                return \false;
            }
            if ($request->getLockedRepository()) {
                foreach ($request->getLockedRepository()->getPackages() as $package) {
                    if ($package->getName() === $this->reasonData->getTarget()) {
                        ...
                        if (!$this->reasonData->getConstraint()->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $package->getVersion()))) {
                            return \true;
                        }
        ...
        if ($this->getReason() === self::RULE_ROOT_REQUIRE) {
            if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($this->reasonData['packageName'])) {
                return \false;
            }
            if ($request->getLockedRepository()) {
                foreach ($request->getLockedRepository()->getPackages() as $package) {
                    if ($package->getName() === $this->reasonData['packageName']) {
                        ...
                        if (!$this->reasonData['constraint']->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $package->getVersion()))) {
                            return \true;
                        }
                        break;
                    }
        ...
        switch ($this->getReason()) {
            case self::RULE_ROOT_REQUIRE:
                ...
                $packagesNonAlias = \array_values(\array_filter($packages, function ($p) {
                    return !$p instanceof \RectorPrefix20210503\Composer\Package\AliasPackage;
                }));
            ...
            case self::RULE_PACKAGE_REQUIRES:
                ...
                if ($requires) {
                    ...
                    $reason = \RectorPrefix20210503\Composer\DependencyResolver\Problem::getMissingPackageReason($repositorySet, $request, $pool, $isVerbose, $targetName, $this->reasonData->getConstraint());
            ...
            case self::RULE_PACKAGE_ALIAS:
                ...
                if ($aliasPackage->getVersion() === \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
                    return '';
                }
    ...
    protected function formatPackagesUnique($pool, array $packages, $isVerbose)
    {
        ...
        return \RectorPrefix20210503\Composer\DependencyResolver\Problem::getPackageList($packages, $isVerbose);
    }
    private function deduplicateDefaultBranchAlias(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
