...
class Decisions implements \Iterator, \Countable
{
    ...
    public function decide($literal, $level, $why)
    {
        ...
        $this->decisionQueue[] = array(self::DECISION_LITERAL => $literal, self::DECISION_REASON => $why);
    }
    ...
    protected function addDecision($literal, $level)
    {
        ...
        if ($previousDecision != 0) {
            ...
            throw new \RectorPrefix20210503\Composer\DependencyResolver\SolverBugException("Trying to decide {$literalString} on level {$level}, even though {$package} was previously decided as " . (int) $previousDecision . ".");
        }
        if ($literal > 0) {
    ...
    public function toString(\RectorPrefix20210503\Composer\DependencyResolver\Pool $pool = null)
    {
