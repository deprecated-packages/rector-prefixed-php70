...
class RuleWatchGraph
{
     ...
     */
    public function insert(\RectorPrefix20210503\Composer\DependencyResolver\RuleWatchNode $node)
    {
        if ($node->getRule()->isAssertion()) {
            return;
        }
        if (!$node->getRule() instanceof \RectorPrefix20210503\Composer\DependencyResolver\MultiConflictRule) {
            foreach (array($node->watch1, $node->watch2) as $literal) {
                if (!isset($this->watchChains[$literal])) {
                    $this->watchChains[$literal] = new \RectorPrefix20210503\Composer\DependencyResolver\RuleWatchChain();
                }
    ...
    protected function moveWatch($fromLiteral, $toLiteral, $node)
    {
        if (!isset($this->watchChains[$toLiteral])) {
            $this->watchChains[$toLiteral] = new \RectorPrefix20210503\Composer\DependencyResolver\RuleWatchChain();
        }
