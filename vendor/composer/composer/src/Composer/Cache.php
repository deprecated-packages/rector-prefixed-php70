...
class Cache
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, $cacheDir, $allowlist = 'a-z0-9.', \RectorPrefix20210503\Composer\Util\Filesystem $filesystem = null, $readOnly = \false)
    {
        $this->io = $io;
        ...
        if (!\is_dir($this->root) && !\RectorPrefix20210503\Composer\Util\Silencer::call('mkdir', $this->root, 0777, \true) || !\is_writable($this->root)) {
    ...
    public function read($file)
    {
        if ($this->enabled) {
            ...
            if (\file_exists($this->root . $file)) {
                $this->io->writeError('Reading ' . $this->root . $file . ' from cache', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
    ...
    public function write($file, $contents)
    {
        if ($this->enabled && !$this->readOnly) {
            $file = \preg_replace('{[^' . $this->allowlist . ']}i', '-', $file);
            $this->io->writeError('Writing ' . $this->root . $file . ' into cache', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
            $tempFileName = $this->root . $file . \uniqid('.', \true) . '.tmp';
            try {
                return \file_put_contents($tempFileName, $contents) !== \false && \rename($tempFileName, $this->root . $file);
            } catch (\ErrorException $e) {
                $this->io->writeError('<warning>Failed to write into cache: ' . $e->getMessage() . '</warning>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                if (\preg_match('{^file_put_contents\\(\\): Only ([0-9]+) of ([0-9]+) bytes written}', $e->getMessage(), $m)) {
                    // Remove partial file.
                    \unlink($tempFileName);
                    $message = \sprintf('<warning>Writing %1$s into cache failed after %2$u of %3$u bytes written, only %4$u bytes of free space available</warning>', $tempFileName, $m[1], $m[2], @\disk_free_space(\dirname($tempFileName)));
    ...
    public function copyTo($file, $target)
    {
        if ($this->enabled) {
            ...
            if (\file_exists($this->root . $file)) {
                try {
                    \touch($this->root . $file, \filemtime($this->root . $file), \time());
                } catch (\ErrorException $e) {
                    // fallback in case the above failed due to incorrect ownership
                    // see https://github.com/composer/composer/issues/4070
                    \RectorPrefix20210503\Composer\Util\Silencer::call('touch', $this->root . $file);
                }
                $this->io->writeError('Reading ' . $this->root . $file . ' from cache', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
    ...
    {
        return \RectorPrefix20210503\Symfony\Component\Finder\Finder::create()->in($this->root)->files();
    }
}
