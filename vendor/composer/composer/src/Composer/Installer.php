...
use RectorPrefix20210503\Composer\Console\GithubActionError;
use RectorPrefix20210503\Composer\DependencyResolver\DefaultPolicy;
use RectorPrefix20210503\Composer\DependencyResolver\LocalRepoTransaction;
use RectorPrefix20210503\Composer\DependencyResolver\LockTransaction;
...
use RectorPrefix20210503\Composer\DependencyResolver\Solver;
use RectorPrefix20210503\Composer\DependencyResolver\SolverProblemsException;
use RectorPrefix20210503\Composer\DependencyResolver\PolicyInterface;
...
use RectorPrefix20210503\Composer\Installer\InstallerEvents;
...
use RectorPrefix20210503\Composer\Package\RootAliasPackage;
use RectorPrefix20210503\Composer\Package\BasePackage;
use RectorPrefix20210503\Composer\Package\CompletePackage;
...
use RectorPrefix20210503\Composer\Package\Package;
use RectorPrefix20210503\Composer\Repository\ArrayRepository;
...
use RectorPrefix20210503\Composer\Repository\FilterRepository;
...
class Installer
{
    ...
    protected $updateAllowTransitiveDependencies = \RectorPrefix20210503\Composer\DependencyResolver\Request::UPDATE_ONLY_LISTED;
    /**
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Package\RootPackageInterface $package, \RectorPrefix20210503\Composer\Downloader\DownloadManager $downloadManager, \RectorPrefix20210503\Composer\Repository\RepositoryManager $repositoryManager, \RectorPrefix20210503\Composer\Package\Locker $locker, \RectorPrefix20210503\Composer\Installer\InstallationManager $installationManager, \RectorPrefix20210503\Composer\EventDispatcher\EventDispatcher $eventDispatcher, \RectorPrefix20210503\Composer\Autoload\AutoloadGenerator $autoloadGenerator)
    {
        $this->io = $io;
        $this->config = $config;
    ...
    public function run()
    {
        ...
        if ($this->runScripts) {
            ...
            $eventName = $this->update ? \RectorPrefix20210503\Composer\Script\ScriptEvents::PRE_UPDATE_CMD : \RectorPrefix20210503\Composer\Script\ScriptEvents::PRE_INSTALL_CMD;
        ...
        if (!$this->suggestedPackagesReporter) {
            $this->suggestedPackagesReporter = new \RectorPrefix20210503\Composer\Installer\SuggestedPackagesReporter($this->io);
        }
        try {
            if ($this->update) {
        ...
        if ($this->update) {
            $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($this->locker->getLockedRepository($this->devMode), $this->createPlatformRepo(\false), new \RectorPrefix20210503\Composer\Repository\RootPackageRepository(clone $this->package)));
            if ($isFreshInstall) {
        ...
        foreach ($lockedRepository->getPackages() as $package) {
            if (!$package instanceof \RectorPrefix20210503\Composer\Package\CompletePackage || !$package->isAbandoned()) {
                continue;
            }
        ...
        $fundingCount = 0;
        foreach ($localRepo->getPackages() as $package) {
            if ($package instanceof \RectorPrefix20210503\Composer\Package\CompletePackageInterface && !$package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && $package->getFunding()) {
                $fundingCount++;
            }
        }
        if ($fundingCount) {
            $this->io->writeError(array(\sprintf("<info>%d package%s you are using %s looking for funding.</info>", $fundingCount, 1 === $fundingCount ? '' : 's', 1 === $fundingCount ? 'is' : 'are'), '<info>Use the `composer fund` command to find out more!</info>'));
        }
        if ($this->runScripts) {
            // dispatch post event
            $eventName = $this->update ? \RectorPrefix20210503\Composer\Script\ScriptEvents::POST_UPDATE_CMD : \RectorPrefix20210503\Composer\Script\ScriptEvents::POST_INSTALL_CMD;
            $this->eventDispatcher->dispatchScript($eventName, $this->devMode);
        }
    ...
    protected function doUpdate(\RectorPrefix20210503\Composer\Repository\InstalledRepositoryInterface $localRepo, $doInstall)
    {
        ...
        if (($this->updateAllowList || $this->updateMirrors) && !$lockedRepository) {
            $this->io->writeError('<error>Cannot update ' . ($this->updateMirrors ? 'lock file information' : 'only a partial set of packages') . ' without a lock file present. Run `composer update` to generate a lock file.</error>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            return 1;
        }
        $this->io->writeError('<info>Loading composer repositories with package information</info>');
        // creating repository set
        $policy = $this->createPolicy(\true);
        ...
        // solve dependencies
        $solver = new \RectorPrefix20210503\Composer\DependencyResolver\Solver($policy, $pool, $this->io);
        try {
            $lockTransaction = $solver->solve($request, $this->ignorePlatformReqs);
            $ruleSetSize = $solver->getRuleSetSize();
            $solver = null;
        } catch (\RectorPrefix20210503\Composer\DependencyResolver\SolverProblemsException $e) {
            ...
            $this->io->writeError('<error>' . $err . '</error>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            $this->io->writeError($prettyProblem);
            if (!$this->devMode) {
                $this->io->writeError('<warning>Running update with --no-dev does not mean require-dev is ignored, it just means the packages will not be installed. If dev requirements are blocking the update you have to resolve those problems.</warning>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            }
            $ghe = new \RectorPrefix20210503\Composer\Console\GithubActionError($this->io);
            $ghe->emit($err . "\n" . $prettyProblem);
            return \max(1, $e->getCode());
        }
        $this->io->writeError("Analyzed " . \count($pool) . " packages to resolve dependencies", \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
        $this->io->writeError("Analyzed " . $ruleSetSize . " rules to resolve dependencies", \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
        ...
        if ($lockTransaction->getOperations()) {
            ...
            foreach ($lockTransaction->getOperations() as $operation) {
                if ($operation instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\InstallOperation) {
                    $installsUpdates[] = $operation;
                    $installNames[] = $operation->getPackage()->getPrettyName() . ':' . $operation->getPackage()->getFullPrettyVersion();
                } elseif ($operation instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\UpdateOperation) {
                ...
                } elseif ($operation instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\UninstallOperation) {
            ...
            if ($installNames) {
                $this->io->writeError("Installs: " . \implode(', ', $installNames), \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            }
            if ($updateNames) {
                $this->io->writeError("Updates: " . \implode(', ', $updateNames), \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            }
            if ($uninstalls) {
                $this->io->writeError("Removals: " . \implode(', ', $uninstallNames), \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            }
        }
        $sortByName = function ($a, $b) {
            if ($a instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\UpdateOperation) {
                $a = $a->getTargetPackage()->getName();
            } else {
                $a = $a->getPackage()->getName();
            }
            if ($b instanceof \RectorPrefix20210503\Composer\DependencyResolver\Operation\UpdateOperation) {
        ...
        foreach (\array_merge($uninstalls, $installsUpdates) as $operation) {
            ...
            if (\false === \strpos($operation->getOperationType(), 'Alias') || $this->io->isDebug()) {
                $this->io->writeError('  - ' . $operation->show(\true));
            }
        }
        $updatedLock = $this->locker->setLockData($lockTransaction->getNewLockPackages(\false, $this->updateMirrors), $lockTransaction->getNewLockPackages(\true, $this->updateMirrors), $platformReqs, $platformDevReqs, $lockTransaction->getAliases($aliases), $this->package->getMinimumStability(), $this->package->getStabilityFlags(), $this->preferStable || $this->package->getPreferStable(), $this->preferLowest, $this->config->get('platform') ?: array(), $this->writeLock && $this->executeOperations);
     ...
     */
    protected function extractDevPackages(\RectorPrefix20210503\Composer\DependencyResolver\LockTransaction $lockTransaction, \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo, array $aliases, \RectorPrefix20210503\Composer\DependencyResolver\PolicyInterface $policy, \RectorPrefix20210503\Composer\Repository\LockArrayRepository $lockedRepository = null)
    {
        if (!$this->package->getDevRequires()) {
            return 0;
        }
        $resultRepo = new \RectorPrefix20210503\Composer\Repository\ArrayRepository(array());
        $loader = new \RectorPrefix20210503\Composer\Package\Loader\ArrayLoader(null, \true);
     ...
     */
    protected function doInstall(\RectorPrefix20210503\Composer\Repository\InstalledRepositoryInterface $localRepo, $alreadySolved = \false)
    {
        ...
        if (!$alreadySolved) {
            ...
            if (!$this->locker->isFresh()) {
                $this->io->writeError('<warning>Warning: The lock file is not up to date with the latest changes in composer.json. You may be getting outdated dependencies. It is recommended that you run `composer update` or `composer update <package name>`.</warning>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            }
            foreach ($lockedRepository->getPackages() as $package) {
            ...
            try {
                ...
                if (0 !== \count($lockTransaction->getOperations())) {
                    $this->io->writeError('<error>Your lock file cannot be installed on this system without changes. Please run composer update.</error>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
                    return 1;
                }
            } catch (\RectorPrefix20210503\Composer\DependencyResolver\SolverProblemsException $e) {
        ...
        $localRepoTransaction = new \RectorPrefix20210503\Composer\DependencyResolver\LocalRepoTransaction($lockedRepository, $localRepo);
        $this->eventDispatcher->dispatchInstallerEvent(\RectorPrefix20210503\Composer\Installer\InstallerEvents::PRE_OPERATIONS_EXEC, $this->devMode, $this->executeOperations, $localRepoTransaction);
        ...
        if ($localRepoTransaction->getOperations()) {
            ...
            if ($installs) {
                $this->io->writeError("Installs: " . \implode(', ', $installs), \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            }
            if ($updates) {
                $this->io->writeError("Updates: " . \implode(', ', $updates), \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            }
            if ($uninstalls) {
                $this->io->writeError("Removals: " . \implode(', ', $uninstalls), \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            }
        }
    ...
    protected function createPlatformRepo($forUpdate)
    {
        ...
        return new \RectorPrefix20210503\Composer\Repository\PlatformRepository(array(), $platformOverrides);
    }
    /**
     ...
     */
    private function createRepositorySet($forUpdate, \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo, array $rootAliases = array(), $lockedRepository = null)
    {
        if ($forUpdate) {
            ...
            $requires = array();
            foreach ($lockedRepository->getPackages() as $package) {
                $constraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $package->getVersion());
        ...
        foreach ($requires as $req => $constraint) {
            ...
            if ((\true === $this->ignorePlatformReqs || \is_array($this->ignorePlatformReqs) && \in_array($req, $this->ignorePlatformReqs, \true)) && \RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($req)) {
                continue;
            }
            if ($constraint instanceof \RectorPrefix20210503\Composer\Package\Link) {
        ...
        $stabilityFlags[$this->package->getName()] = \RectorPrefix20210503\Composer\Package\BasePackage::$stabilities[\RectorPrefix20210503\Composer\Package\Version\VersionParser::parseStability($this->package->getVersion())];
        $repositorySet = new \RectorPrefix20210503\Composer\Repository\RepositorySet($minimumStability, $stabilityFlags, $rootAliases, $this->package->getReferences(), $rootRequires);
        $repositorySet->addRepository(new \RectorPrefix20210503\Composer\Repository\RootPackageRepository($this->fixedRootPackage));
        ...
        if ($this->additionalFixedRepository) {
            ...
            if ($additionalFixedRepositories instanceof \RectorPrefix20210503\Composer\Repository\CompositeRepository) {
            ...
            foreach ($additionalFixedRepositories as $additionalFixedRepository) {
                if ($additionalFixedRepository instanceof \RectorPrefix20210503\Composer\Repository\InstalledRepository || $additionalFixedRepository instanceof \RectorPrefix20210503\Composer\Repository\InstalledRepositoryInterface) {
    ...
    private function createPolicy($forUpdate)
    {
        ...
        return new \RectorPrefix20210503\Composer\DependencyResolver\DefaultPolicy($preferStable, $preferLowest);
    }
    /**
     * @return Request
     */
    private function createRequest(\RectorPrefix20210503\Composer\Package\RootPackageInterface $rootPackage, \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo, \RectorPrefix20210503\Composer\Repository\LockArrayRepository $lockedRepository = null)
    {
        $request = new \RectorPrefix20210503\Composer\DependencyResolver\Request($lockedRepository);
        $request->fixPackage($rootPackage);
        if ($rootPackage instanceof \RectorPrefix20210503\Composer\Package\RootAliasPackage) {
        ...
        foreach ($fixedPackages as $package) {
            ...
            if ($package->getRepository() !== $platformRepo || !isset($provided[$package->getName()]) || !$provided[$package->getName()]->getConstraint()->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $package->getVersion()))) {
                $request->fixPackage($package);
            }
        }
        return $request;
    }
    private function requirePackagesForUpdate(\RectorPrefix20210503\Composer\DependencyResolver\Request $request, \RectorPrefix20210503\Composer\Repository\LockArrayRepository $lockedRepository = null, $includeDevRequires = \true)
    {
        ...
        if ($this->updateMirrors) {
            ...
            foreach ($lockedRepository->getPackages() as $lockedPackage) {
                ...
                if (!$lockedPackage instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && !isset($excludedPackages[$lockedPackage->getName()])) {
                    $request->requireName($lockedPackage->getName(), new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $lockedPackage->getVersion()));
                }
            }
        } else {
     ...
     */
    private function mockLocalRepositories(\RectorPrefix20210503\Composer\Repository\RepositoryManager $rm)
    {
        $packages = array();
        ...
        foreach ($packages as $key => $package) {
            if ($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
                ...
                $packages[$key] = new \RectorPrefix20210503\Composer\Package\AliasPackage($packages[$alias], $package->getVersion(), $package->getPrettyVersion());
            }
        }
        $rm->setLocalRepository(new \RectorPrefix20210503\Composer\Repository\InstalledArrayRepository($packages));
    }
    /**
     * Create Installer
     *
     ...
     * @return Installer
     */
    public static function create(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Composer $composer)
    {
     ...
     * @return $this
     */
    public function setAdditionalFixedRepository(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $additionalFixedRepository)
    {
     ...
     * @return Installer
     */
    public function setDryRun($dryRun = \true)
    {
     ...
     * @return Installer
     */
    public function setPreferSource($preferSource = \true)
    {
     ...
     * @return Installer
     */
    public function setPreferDist($preferDist = \true)
    {
     ...
     * @return Installer
     */
    public function setDevMode($devMode = \true)
    {
     ...
     * @return Installer
     */
    public function setDumpAutoloader($dumpAutoloader = \true)
    {
     ...
     * @return Installer
     */
    public function setRunScripts($runScripts = \true)
    {
     ...
     * @return Installer
     */
    public function setVerbose($verbose = \true)
    {
    ...
    public function setIgnorePlatformRequirements($ignorePlatformReqs)
    {
        if (\is_array($ignorePlatformReqs)) {
            $this->ignorePlatformReqs = \array_filter($ignorePlatformReqs, function ($req) {
                return \RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($req);
            });
        } else {
    ...
    {
        if (!\in_array($updateAllowTransitiveDependencies, array(\RectorPrefix20210503\Composer\DependencyResolver\Request::UPDATE_ONLY_LISTED, \RectorPrefix20210503\Composer\DependencyResolver\Request::UPDATE_LISTED_WITH_TRANSITIVE_DEPS_NO_ROOT_REQUIRE, \RectorPrefix20210503\Composer\DependencyResolver\Request::UPDATE_LISTED_WITH_TRANSITIVE_DEPS), \true)) {
     ...
     * @return Installer
     */
    public function setPreferStable($preferStable = \true)
    {
     ...
     * @return Installer
     */
    public function setPreferLowest($preferLowest = \true)
    {
     ...
     * @return Installer
     */
    public function setWriteLock($writeLock = \true)
    {
     ...
     * @return Installer
     */
    public function setExecuteOperations($executeOperations = \true)
    {
     ...
     * @return Installer
     */
    public function setSuggestedPackagesReporter(\RectorPrefix20210503\Composer\Installer\SuggestedPackagesReporter $suggestedPackagesReporter)
    {
