 ...
 */
class FilesystemRepository extends \RectorPrefix20210503\Composer\Repository\WritableArrayRepository
{
    protected $file;
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\Json\JsonFile $repositoryFile, $dumpVersions = \false, \RectorPrefix20210503\Composer\Package\RootPackageInterface $rootPackage = null)
    {
        parent::__construct();
    ...
    protected function initialize()
    {
        ...
        try {
            ...
            throw new \RectorPrefix20210503\Composer\Repository\InvalidRepositoryException('Invalid repository data in ' . $this->file->getPath() . ', packages could not be loaded: [' . \get_class($e) . '] ' . $e->getMessage());
        }
    ...
    public function write($devMode, \RectorPrefix20210503\Composer\Installer\InstallationManager $installationManager)
    {
        ...
        foreach ($this->getCanonicalPackages() as $package) {
            ...
            $pkgArray['install-path'] = '' !== $path && null !== $path ? $fs->findShortestPath($repoDir, $fs->isAbsolutePath($path) ? $path : \getcwd() . '/' . $path, \true) : null;
        ...
        if ($this->dumpVersions) {
            ...
            $fs->filePutContentsIfModified($repoDir . '/installed.php', '<?php return ' . \var_export($versions, \true) . ';' . "\n");
            ...
            $installedVersionsClass = \str_replace('private static $installed;', 'private static $installed = ' . \var_export($versions, \true) . ';', $installedVersionsClass);
            $fs->filePutContentsIfModified($repoDir . '/InstalledVersions.php', $installedVersionsClass);
            \RectorPrefix20210503\Composer\InstalledVersions::reload($versions);
        }
    }
    /**
     * @return ?array
     */
    private function generateInstalledVersions(\RectorPrefix20210503\Composer\Installer\InstallationManager $installationManager)
    {
        if (!$this->dumpVersions) {
            return null;
        }
        ...
        while ($rootPackage instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
        ...
        foreach ($packages as $package) {
            ...
            $versions['versions'][$package->getName()] = array('pretty_version' => $package->getPrettyVersion(), 'version' => $package->getVersion(), 'aliases' => array(), 'reference' => $reference);
        ...
        foreach ($packages as $package) {
            foreach ($package->getReplaces() as $replace) {
                ...
                if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($replace->getTarget())) {
                    continue;
                }
            ...
            foreach ($package->getProvides() as $provide) {
                ...
                if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($provide->getTarget())) {
                    continue;
                }
