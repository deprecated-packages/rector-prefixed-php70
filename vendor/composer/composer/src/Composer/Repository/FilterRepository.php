 ...
 */
class FilterRepository implements \RectorPrefix20210503\Composer\Repository\RepositoryInterface
{
    private $only = array();
    private $exclude = array();
    private $canonical = \true;
    private $repo;
    public function __construct(\RectorPrefix20210503\Composer\Repository\RepositoryInterface $repo, array $options)
    {
        if (isset($options['only'])) {
            ...
            $this->only = '{^' . \implode('|', \array_map(function ($val) {
                return \RectorPrefix20210503\Composer\Package\BasePackage::packageNameToRegexp($val, '%s');
            }, $options['only'])) . '$}iD';
        }
