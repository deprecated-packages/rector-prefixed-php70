 ...
 */
class ArrayRepository implements \RectorPrefix20210503\Composer\Repository\RepositoryInterface
{
    ...
    public function loadPackages(array $packageMap, array $acceptableStabilities, array $stabilityFlags, array $alreadyLoaded = array())
    {
        ...
        foreach ($packages as $package) {
            if (\array_key_exists($package->getName(), $packageMap)) {
                if ((!$packageMap[$package->getName()] || $packageMap[$package->getName()]->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $package->getVersion()))) && \RectorPrefix20210503\Composer\Package\Version\StabilityFilter::isPackageAcceptable($acceptableStabilities, $stabilityFlags, $package->getNames(), $package->getStability()) && !isset($alreadyLoaded[$package->getName()][$package->getVersion()])) {
                    ...
                    if ($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && !isset($result[\spl_object_hash($package->getAliasOf())])) {
    ...
    public function findPackages($name, $constraint = null)
    {
        ...
        foreach ($this->getPackages() as $package) {
            if ($name === $package->getName()) {
                if (null === $constraint || $constraint->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $package->getVersion()))) {
                    $packages[] = $package;
                }
            }
        }
    ...
    public function search($query, $mode = 0, $type = null)
    {
        ...
        foreach ($this->getPackages() as $package) {
            ...
            if (\preg_match($regex, $name) || $mode === self::SEARCH_FULLTEXT && $package instanceof \RectorPrefix20210503\Composer\Package\CompletePackageInterface && \preg_match($regex, \implode(' ', (array) $package->getKeywords()) . ' ' . $package->getDescription())) {
                if (null !== $type && $package->getType() !== $type) {
                    continue;
                }
                $matches[$name] = array('name' => $package->getPrettyName(), 'description' => $package instanceof \RectorPrefix20210503\Composer\Package\CompletePackageInterface ? $package->getDescription() : null);
            }
        }
     ...
     * {@inheritDoc}
     */
    public function hasPackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        ...
        foreach ($this->getPackages() as $candidate) {
            ...
            foreach ($candidate->getProvides() as $link) {
                if ($packageName === $link->getTarget()) {
                    $result[$candidate->getName()] = array('name' => $candidate->getName(), 'description' => $candidate->getDescription(), 'type' => $candidate->getType());
                    continue 2;
                }
            }
        }
    ...
    protected function createAliasPackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $alias, $prettyAlias)
    {
        return new \RectorPrefix20210503\Composer\Package\AliasPackage($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage ? $package->getAliasOf() : $package, $alias, $prettyAlias);
    }
    /**
