...
use RectorPrefix20210503\Composer\Platform\HhvmDetector;
use RectorPrefix20210503\Composer\Platform\Runtime;
use RectorPrefix20210503\Composer\Platform\Version;
 ...
 */
class PlatformRepository extends \RectorPrefix20210503\Composer\Repository\ArrayRepository
{
    ...
    public function __construct(array $packages = array(), array $overrides = array(), \RectorPrefix20210503\Composer\Platform\Runtime $runtime = null, \RectorPrefix20210503\Composer\Platform\HhvmDetector $hhvmDetector = null)
    {
        $this->runtime = $runtime ?: new \RectorPrefix20210503\Composer\Platform\Runtime();
        $this->hhvmDetector = $hhvmDetector ?: new \RectorPrefix20210503\Composer\Platform\HhvmDetector();
    ...
    protected function initialize()
    {
        ...
        $prettyVersion = \RectorPrefix20210503\Composer\Plugin\PluginInterface::PLUGIN_API_VERSION;
        $version = $this->versionParser->normalize($prettyVersion);
        $composerPluginApi = new \RectorPrefix20210503\Composer\Package\CompletePackage('composer-plugin-api', $version, $prettyVersion);
        $composerPluginApi->setDescription('The Composer Plugin API');
        $this->addPackage($composerPluginApi);
        $prettyVersion = \RectorPrefix20210503\Composer\Composer::RUNTIME_API_VERSION;
        $version = $this->versionParser->normalize($prettyVersion);
        $composerRuntimeApi = new \RectorPrefix20210503\Composer\Package\CompletePackage('composer-runtime-api', $version, $prettyVersion);
        ...
        $php = new \RectorPrefix20210503\Composer\Package\CompletePackage('php', $version, $prettyVersion);
        $php->setDescription('The PHP interpreter');
        $this->addPackage($php);
        if ($this->runtime->getConstant('PHP_DEBUG')) {
            $phpdebug = new \RectorPrefix20210503\Composer\Package\CompletePackage('php-debug', $version, $prettyVersion);
        ...
        if ($this->runtime->hasConstant('PHP_ZTS') && $this->runtime->getConstant('PHP_ZTS')) {
            $phpzts = new \RectorPrefix20210503\Composer\Package\CompletePackage('php-zts', $version, $prettyVersion);
        ...
        if ($this->runtime->getConstant('PHP_INT_SIZE') === 8) {
            $php64 = new \RectorPrefix20210503\Composer\Package\CompletePackage('php-64bit', $version, $prettyVersion);
        ...
        if ($this->runtime->hasConstant('AF_INET6') || \RectorPrefix20210503\Composer\Util\Silencer::call(array($this->runtime, 'invoke'), 'inet_pton', array('::')) !== \false) {
            $phpIpv6 = new \RectorPrefix20210503\Composer\Package\CompletePackage('php-ipv6', $version, $prettyVersion);
        ...
        if (!\in_array('xdebug', $loadedExtensions, \true) && ($prettyVersion = \RectorPrefix20210503\Composer\XdebugHandler\XdebugHandler::getSkippedVersion())) {
            ...
            switch ($name) {
                ...
                case 'curl':
                    ...
                    if (\preg_match('{^SSL Version => (?<library>[^/]+)/(?<version>.+)$}im', $info, $sslMatches)) {
                        ...
                        if ($library === 'openssl') {
                            $parsedVersion = \RectorPrefix20210503\Composer\Platform\Version::parseOpenssl($sslMatches['version'], $isFips);
                ...
                case 'gd':
                    ...
                    if (\preg_match('/^libJPEG Version => (?<version>.+?)(?: compatible)?$/im', $info, $libjpegMatches)) {
                        $this->addLibrary($name . '-libjpeg', \RectorPrefix20210503\Composer\Platform\Version::parseLibjpeg($libjpegMatches['version']), 'libjpeg version for gd');
                    }
                    ...
                    if (\preg_match('/^libXpm Version => (?<versionId>\\d+)$/im', $info, $libxpmMatches)) {
                        $this->addLibrary($name . '-libxpm', \RectorPrefix20210503\Composer\Platform\Version::convertLibxpmVersionId($libxpmMatches['versionId']), 'libxpm version for gd');
                    }
                    break;
                case 'gmp':
                ...
                case 'intl':
                    ...
                    if (\preg_match('/^ICU TZData version => (?<version>.*)$/im', $info, $zoneinfoMatches)) {
                        $this->addLibrary('icu-zoneinfo', \RectorPrefix20210503\Composer\Platform\Version::parseZoneinfoVersion($zoneinfoMatches['version']), 'zoneinfo ("Olson") database for icu');
                    }
                ...
                case 'ldap':
                    ...
                    if (\preg_match('/^Vendor Version => (?<versionId>\\d+)$/im', $info, $matches) && \preg_match('/^Vendor Name => (?<vendor>.+)$/im', $info, $vendorMatches)) {
                        $this->addLibrary($name . '-' . \strtolower($vendorMatches['vendor']), \RectorPrefix20210503\Composer\Platform\Version::convertOpenldapVersionId($matches['versionId']), $vendorMatches['vendor'] . ' version of ldap');
                    }
                    break;
                case 'libxml':
                ...
                case 'openssl':
                    ...
                    if (\preg_match('{^(?:OpenSSL|LibreSSL)?\\s*(?<version>\\S+)}i', $this->runtime->getConstant('OPENSSL_VERSION_TEXT'), $matches)) {
                        $parsedVersion = \RectorPrefix20210503\Composer\Platform\Version::parseOpenssl($matches['version'], $isFips);
        ...
        if ($hhvmVersion) {
            ...
            $hhvm = new \RectorPrefix20210503\Composer\Package\CompletePackage('hhvm', $version, $prettyVersion);
    ...
    private function addOverriddenPackage(array $override, $name = null)
    {
        ...
        $package = new \RectorPrefix20210503\Composer\Package\CompletePackage($name ?: $override['name'], $version, $override['version']);
        $package->setDescription('Package overridden via config.platform');
        $package->setExtra(array('config.platform' => \true));
        parent::addPackage($package);
        return $package;
    }
    /**
    ...
    private function addExtension($name, $prettyVersion)
    {
        ...
        $ext = new \RectorPrefix20210503\Composer\Package\CompletePackage($packageName, $version, $prettyVersion);
        $ext->setDescription('The ' . $name . ' PHP extension' . $extraDescription);
        if ($name === 'uuid') {
            $ext->setReplaces(array(new \RectorPrefix20210503\Composer\Package\Link('ext-uuid', 'lib-uuid', new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $version), \RectorPrefix20210503\Composer\Package\Link::TYPE_REPLACE, $ext->getPrettyVersion())));
        }
        $this->addPackage($ext);
    }
    /**
    ...
    private function addLibrary($name, $prettyVersion, $description = null, array $replaces = array(), array $provides = array())
    {
        ...
        $lib = new \RectorPrefix20210503\Composer\Package\CompletePackage('lib-' . $name, $version, $prettyVersion);
        $lib->setDescription($description);
        $links = function ($alias) use($name, $version, $lib) {
            return new \RectorPrefix20210503\Composer\Package\Link('lib-' . $name, 'lib-' . $alias, new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $version), \RectorPrefix20210503\Composer\Package\Link::TYPE_REPLACE, $lib->getPrettyVersion());
        };
    ...
    public static function isPlatformPackage($name)
    {
        ...
        return $cache[$name] = (bool) \preg_match(\RectorPrefix20210503\Composer\Repository\PlatformRepository::PLATFORM_PACKAGE_REGEX, $name);
    }
}
