...
use RectorPrefix20210503\Composer\Semver\CompilingMatcher;
...
use RectorPrefix20210503\Composer\MetadataMinifier\MetadataMinifier;
use RectorPrefix20210503\Composer\Util\Url;
/**
 * @author Jordi Boggiano <j.boggiano@seld.be>
 */
class ComposerRepository extends \RectorPrefix20210503\Composer\Repository\ArrayRepository implements \RectorPrefix20210503\Composer\Repository\ConfigurableRepositoryInterface
{
    private $repoConfig;
    private $options;
    private $url;
    ...
    public function __construct(array $repoConfig, \RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Util\HttpDownloader $httpDownloader, \RectorPrefix20210503\Composer\EventDispatcher\EventDispatcher $eventDispatcher = null)
    {
        parent::__construct();
        ...
        $this->io = $io;
        $this->cache = new \RectorPrefix20210503\Composer\Cache($io, $config->get('cache-repo-dir') . '/' . \preg_replace('{[^a-z0-9.]}i', '-', \RectorPrefix20210503\Composer\Util\Url::sanitize($this->url)), 'a-z0-9.$~');
        ...
        $this->loader = new \RectorPrefix20210503\Composer\Package\Loader\ArrayLoader($this->versionParser);
        $this->httpDownloader = $httpDownloader;
        $this->eventDispatcher = $eventDispatcher;
        $this->repoConfig = $repoConfig;
        $this->loop = new \RectorPrefix20210503\Composer\Util\Loop($this->httpDownloader);
    }
    public function getRepoName()
    {
        return 'composer repo (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($this->url) . ')';
    }
    ...
    public function findPackage($name, $constraint)
    {
        ...
        if (!$constraint instanceof \RectorPrefix20210503\Composer\Semver\Constraint\ConstraintInterface) {
    ...
    private function filterPackages(array $packages, $constraint = null, $returnFirstMatch = \false)
    {
        ...
        foreach ($packages as $package) {
            $pkgConstraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $package->getVersion());
        ...
        if ($this->lazyProvidersUrl) {
            if (\is_array($this->availablePackages) && !$this->availablePackagePatterns) {
                ...
                foreach ($this->availablePackages as $name) {
                    $packageMap[$name] = new \RectorPrefix20210503\Composer\Semver\Constraint\MatchAllConstraint();
                }
        ...
        if ($hasProviders || $this->hasPartialPackages()) {
            foreach ($packageNameMap as $name => $constraint) {
                ...
                foreach ($candidates as $candidate) {
                    ...
                    $namesFound[$name] = \true;
                    if (!$constraint || $constraint->matches(new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('==', $candidate->getVersion()))) {
                        $matches[\spl_object_hash($candidate)] = $candidate;
                        if ($candidate instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && !isset($matches[\spl_object_hash($candidate->getAliasOf())])) {
                ...
                foreach ($candidates as $candidate) {
                    if ($candidate instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
        ...
        if ($this->hasPartialPackages()) {
            foreach ($this->partialPackagesByName as $versions) {
                foreach ($versions as $candidate) {
                    ...
                    $result[$candidate['name']] = array('name' => $candidate['name'], 'description' => isset($candidate['description']) ? $candidate['description'] : '', 'type' => isset($candidate['type']) ? $candidate['type'] : '');
                }
            }
        }
        if ($this->packages) {
    ...
    protected function configurePackageTransportOptions(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
    ...
    private function whatProvides($name, array $acceptableStabilities = null, array $stabilityFlags = null, array $alreadyLoaded = array())
    {
        ...
        if (!$this->hasPartialPackages() || !isset($this->partialPackagesByName[$name])) {
            ...
            if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($name) || '__root__' === $name) {
                return array();
            }
            if (null === $this->providerListing) {
                $this->loadProviderListings($this->loadRootServerFile());
            ...
            if ($cacheKey) {
                if (!$useLastModifiedCheck && $hash && $this->cache->sha256($cacheKey) === $hash) {
                    ...
                    $packagesSource = 'cached file (' . $cacheKey . ' originating from ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')';
                    ...
                    if ($contents = $this->cache->read($cacheKey)) {
                        ...
                        if (isset($alreadyLoaded[$name])) {
                            ...
                            $packagesSource = \true === $response ? 'cached file (' . $cacheKey . ' originating from ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')' : 'downloaded file (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')';
                        }
                    }
                }
            }
            if (!$packages) {
                try {
                    ...
                    $packagesSource = 'downloaded file (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')';
                } catch (\RectorPrefix20210503\Composer\Downloader\TransportException $e) {
                    // 404s are acceptable for lazy provider repos
                    if ($this->lazyProvidersUrl && \in_array($e->getStatusCode(), array(404, 499), \true)) {
                        $packages = array('packages' => array());
                        $packagesSource = 'not-found file (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')';
            ...
            $packagesSource = 'root file (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($this->getPackagesJsonUrl()) . ')';
        ...
        foreach ($packages['packages'] as $versions) {
            foreach ($versions as $version) {
                ...
                if (!isset($versionsToLoad[$version['uid']])) {
                    ...
                    } elseif ($version['version_normalized'] === \RectorPrefix20210503\Composer\Package\Version\VersionParser::DEFAULT_BRANCH_ALIAS) {
    ...
    protected function initialize()
    {
        ...
        foreach ($this->createPackages($repoData, 'root file (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($this->getPackagesJsonUrl()) . ')') as $package) {
    ...
    private function loadAsyncPackages(array $packageNames, array $acceptableStabilities = null, array $stabilityFlags = null, array $alreadyLoaded = array())
    {
        ...
        foreach ($packageNames as $name => $constraint) {
            if ($acceptableStabilities === null || $stabilityFlags === null || \RectorPrefix20210503\Composer\Package\Version\StabilityFilter::isPackageAcceptable($acceptableStabilities, $stabilityFlags, array($name), 'dev')) {
        ...
        foreach ($packageNames as $name => $constraint) {
            ...
            // skip platform packages, root package and composer-plugin-api
            if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($realName) || '__root__' === $realName) {
                continue;
            }
            $url = \str_replace('%package%', $name, $this->lazyProvidersUrl);
            $cacheKey = 'provider-' . \strtr($name, '/', '~') . '.json';
            ...
            $promises[] = $this->asyncFetchFile($url, $cacheKey, $lastModified)->then(function ($response) use(&$packages, &$namesFound, $url, $cacheKey, $contents, $realName, $constraint, $repo, $acceptableStabilities, $stabilityFlags, $alreadyLoaded) {
                $packagesSource = 'downloaded file (' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')';
                if (\true === $response) {
                    $packagesSource = 'cached file (' . $cacheKey . ' originating from ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . ')';
                    $response = $contents;
                ...
                if (isset($response['minified']) && $response['minified'] === 'composer/2.0') {
                    $versions = \RectorPrefix20210503\Composer\MetadataMinifier\MetadataMinifier::expand($versions);
                }
                ...
                foreach ($loadedPackages as $package) {
                    ...
                    if ($package instanceof \RectorPrefix20210503\Composer\Package\AliasPackage && !isset($packages[\spl_object_hash($package->getAliasOf())])) {
    ...
    public function isVersionAcceptable($constraint, $name, $versionData, array $acceptableStabilities = null, array $stabilityFlags = null)
    {
        ...
        foreach ($versions as $version) {
            if (null !== $acceptableStabilities && null !== $stabilityFlags && !\RectorPrefix20210503\Composer\Package\Version\StabilityFilter::isPackageAcceptable($acceptableStabilities, $stabilityFlags, array($name), \RectorPrefix20210503\Composer\Package\Version\VersionParser::parseStability($version))) {
                continue;
            }
            if ($constraint && !\RectorPrefix20210503\Composer\Semver\CompilingMatcher::match($constraint, \RectorPrefix20210503\Composer\Semver\Constraint\Constraint::OP_EQ, $version)) {
                continue;
            }
            return \true;
        }
        ...
        if (!empty($data['mirrors'])) {
            foreach ($data['mirrors'] as $mirror) {
                ...
                if (!empty($mirror['dist-url'])) {
                    $this->distMirrors[] = array('url' => $this->canonicalizeUrl($mirror['dist-url']), 'preferred' => !empty($mirror['preferred']));
                }
            }
        }
        ...
        if (!empty($data['metadata-url'])) {
            ...
            if (!empty($data['available-package-patterns'])) {
                $this->availablePackagePatterns = \array_map(function ($pattern) {
                    return \RectorPrefix20210503\Composer\Package\BasePackage::packageNameToRegexp($pattern);
        ...
        if (isset($data['includes'])) {
            foreach ($data['includes'] as $include => $metadata) {
                if (isset($metadata['sha1']) && $this->cache->sha1($include) === $metadata['sha1']) {
                    $includedData = \json_decode($this->cache->read($include), \true);
                } else {
    ...
    public function createPackages(array $packages, $source = null)
    {
        ...
        try {
            ...
            $packageInstances = $this->loader->loadPackages($packages, 'RectorPrefix20210503\\Composer\\Package\\CompletePackage');
        ...
        while ($retries--) {
            try {
                if ($this->eventDispatcher) {
                    $preFileDownloadEvent = new \RectorPrefix20210503\Composer\Plugin\PreFileDownloadEvent(\RectorPrefix20210503\Composer\Plugin\PluginEvents::PRE_FILE_DOWNLOAD, $this->httpDownloader, $filename, 'metadata');
                ...
                if ($sha256 && $sha256 !== \hash('sha256', $json)) {
                    ...
                    throw new \RectorPrefix20210503\Composer\Repository\RepositorySecurityException('The contents of ' . $filename . ' do not match its signature. This could indicate a man-in-the-middle attack or e.g. antivirus software corrupting files. Try running composer again and report this if you think it is a mistake.');
                }
                $data = $response->decodeJson();
                \RectorPrefix20210503\Composer\Util\HttpDownloader::outputWarnings($this->io, $this->url, $data);
                ...
                if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $e->getStatusCode() === 404) {
                    throw $e;
                }
                if ($retries) {
                    \usleep(100000);
                ...
                if ($e instanceof \RectorPrefix20210503\Composer\Repository\RepositorySecurityException) {
                    throw $e;
                }
                if ($cacheKey && ($contents = $this->cache->read($cacheKey))) {
                    ...
                    $data = \RectorPrefix20210503\Composer\Json\JsonFile::parseJson($contents, $this->cache->getRoot() . $cacheKey);
                    break;
                }
                throw $e;
            }
    ...
    private function asyncFetchFile($filename, $cacheKey, $lastModifiedTime = null)
    {
        ...
        if (isset($this->packagesNotFoundCache[$filename])) {
            return \RectorPrefix20210503\React\Promise\resolve(array('packages' => array()));
        }
        if (isset($this->freshMetadataUrls[$filename]) && $lastModifiedTime) {
            // make it look like we got a 304 response
            return \RectorPrefix20210503\React\Promise\resolve(\true);
        }
        ...
        $accept = function ($response) use($io, $url, $filename, $cache, $cacheKey, $repo) {
            ...
            if ($response->getStatusCode() === 404) {
                $repo->packagesNotFoundCache[$filename] = \true;
                return array('packages' => array());
            }
            $json = $response->getBody();
            if ($json === '' && $response->getStatusCode() === 304) {
                $repo->freshMetadataUrls[$filename] = \true;
                return \true;
            }
            $data = $response->decodeJson();
            \RectorPrefix20210503\Composer\Util\HttpDownloader::outputWarnings($io, $url, $data);
            $lastModifiedDate = $response->getHeader('last-modified');
            $response->collect();
            if ($lastModifiedDate) {
                $data['last-modified'] = $lastModifiedDate;
                $json = \RectorPrefix20210503\Composer\Json\JsonFile::encode($data, \RectorPrefix20210503\Composer\Json\JsonFile::JSON_UNESCAPED_SLASHES | \RectorPrefix20210503\Composer\Json\JsonFile::JSON_UNESCAPED_UNICODE);
            }
            if (!$cache->isReadOnly()) {
        ...
        $reject = function ($e) use(&$retries, $httpDownloader, $filename, $options, &$reject, $accept, $io, $url, &$degradedMode, $repo) {
            ...
            // special error code returned when network is being artificially disabled
            if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $e->getStatusCode() === 499) {
                return $accept(new \RectorPrefix20210503\Composer\Util\Http\Response(array('url' => $url), 404, array(), ''));
            }
            throw $e;
        };
        return $httpDownloader->add($filename, $options)->then($accept, $reject);
