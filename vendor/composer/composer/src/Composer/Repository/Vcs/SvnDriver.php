 ...
 */
class SvnDriver extends \RectorPrefix20210503\Composer\Repository\Vcs\VcsDriver
{
    protected $baseUrl;
    protected $tags;
    protected $branches;
    ...
    public function initialize()
    {
        ...
        $this->cache = new \RectorPrefix20210503\Composer\Cache($this->io, $this->config->get('cache-repo-dir') . '/' . \preg_replace('{[^a-z0-9.]}i', '-', \RectorPrefix20210503\Composer\Util\Url::sanitize($this->baseUrl)));
    ...
    public function getFileContent($file, $identifier)
    {
        ...
        try {
            ...
            throw new \RectorPrefix20210503\Composer\Downloader\TransportException($e->getMessage());
        }
        return $output;
    }
    /**
                    ...
                    foreach ($this->process->splitLines($output) as $line) {
                        ...
                        if ($line && \preg_match('{^\\s*(\\S+).*?(\\S+)\\s*$}', $line, $match)) {
                            if (isset($match[1], $match[2]) && $match[2] !== './') {
                                $this->tags[\rtrim($match[2], '/')] = $this->buildIdentifier('/' . $this->tagsPath . '/' . $match[2], $match[1]);
                            }
                        }
                    }
                }
        ...
        if (null === $this->branches) {
            ...
            if ($output) {
                foreach ($this->process->splitLines($output) as $line) {
                    ...
                    if ($line && \preg_match('{^\\s*(\\S+).*?(\\S+)\\s*$}', $line, $match)) {
                        if (isset($match[1], $match[2]) && $match[2] === './') {
                            $this->branches['trunk'] = $this->buildIdentifier('/' . $this->trunkPath, $match[1]);
            ...
            if ($this->branchesPath !== \false) {
                ...
                if ($output) {
                    foreach ($this->process->splitLines(\trim($output)) as $line) {
                        ...
                        if ($line && \preg_match('{^\\s*(\\S+).*?(\\S+)\\s*$}', $line, $match)) {
                            if (isset($match[1], $match[2]) && $match[2] !== './') {
                                $this->branches[\rtrim($match[2], '/')] = $this->buildIdentifier('/' . $this->branchesPath . '/' . $match[2], $match[1]);
                            }
                        }
                    }
                }
    ...
    public static function supports(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, $url, $deep = \false)
    {
        ...
        if (!$deep && !\RectorPrefix20210503\Composer\Util\Filesystem::isLocalPath($url)) {
            return \false;
        }
        $process = new \RectorPrefix20210503\Composer\Util\ProcessExecutor($io);
        $exit = $process->execute("svn info --non-interactive -- " . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url), $ignoredOutput);
        if ($exit === 0) {
    ...
    {
        if (null === $this->util) {
            $this->util = new \RectorPrefix20210503\Composer\Util\Svn($this->baseUrl, $this->io, $this->config, $this->process);
