...
use RectorPrefix20210503\Composer\Util\GitLab;
 ...
 */
class GitLabDriver extends \RectorPrefix20210503\Composer\Repository\Vcs\VcsDriver
{
    private $scheme;
    private $namespace;
    private $repository;
    ...
    public function initialize()
    {
        ...
        $this->cache = new \RectorPrefix20210503\Composer\Cache($this->io, $this->config->get('cache-repo-dir') . '/' . $this->originUrl . '/' . $this->namespace . '/' . $this->repository);
     ...
     * @internal
     */
    public function setHttpDownloader(\RectorPrefix20210503\Composer\Util\HttpDownloader $httpDownloader)
    {
    ...
    private function urlEncodeAll($string)
    {
        ...
        for ($i = 0; isset($string[$i]); $i++) {
            $character = $string[$i];
            if (!\ctype_alnum($character) && !\in_array($character, array('-', '_'), \true)) {
        ...
        try {
            ...
            if ($fetchingRepoData) {
                ...
                if (!isset($json['default_branch'])) {
                    ...
                    throw new \RectorPrefix20210503\Composer\Downloader\TransportException('GitLab API seems to not be authenticated as it did not return a default_branch', 401);
                }
            }
            return $response;
        } catch (\RectorPrefix20210503\Composer\Downloader\TransportException $e) {
            $gitLabUtil = new \RectorPrefix20210503\Composer\Util\GitLab($this->io, $this->config, $this->process, $this->httpDownloader);
            switch ($e->getCode()) {
                case 401:
                case 404:
    ...
    public static function supports(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, $url, $deep = \false)
    {
        ...
        if ('https' === $scheme && !\extension_loaded('openssl')) {
            $io->writeError('Skipping GitLab driver for ' . $url . ' because the OpenSSL PHP extension is missing.', \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            return \false;
        }
        return \true;
    }
