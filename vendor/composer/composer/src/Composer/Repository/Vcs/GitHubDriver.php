...
use RectorPrefix20210503\Composer\Util\GitHub;
use RectorPrefix20210503\Composer\Util\Http\Response;
/**
 * @author Jordi Boggiano <j.boggiano@seld.be>
 */
class GitHubDriver extends \RectorPrefix20210503\Composer\Repository\Vcs\VcsDriver
{
    protected $owner;
    ...
    public function initialize()
    {
        ...
        $this->cache = new \RectorPrefix20210503\Composer\Cache($this->io, $this->config->get('cache-repo-dir') . '/' . $this->originUrl . '/' . $this->owner . '/' . $this->repository);
    ...
    public function getComposerInformation($identifier)
    {
        ...
        if (!isset($this->infoCache[$identifier])) {
            if ($this->shouldCache($identifier) && ($res = $this->cache->read($identifier))) {
                $composer = \RectorPrefix20210503\Composer\Json\JsonFile::parseJson($res);
            } else {
    ...
    private function getFundingInfo()
    {
        ...
        foreach (array($this->getApiUrl() . '/repos/' . $this->owner . '/' . $this->repository . '/contents/.github/FUNDING.yml', $this->getApiUrl() . '/repos/' . $this->owner . '/.github/contents/FUNDING.yml') as $file) {
            try {
                $response = $this->httpDownloader->get($file, array('retry-auth-failure' => \false))->decodeJson();
            ...
            if (\preg_match('{^(\\w+)\\s*:\\s*(.+)$}', $line, $match)) {
                if (\preg_match('{^\\[(.*)\\](?:\\s*#.*)?$}', $match[2], $match2)) {
                    foreach (\array_map('trim', \preg_split('{[\'"]?\\s*,\\s*[\'"]?}', $match2[1])) as $item) {
                        $result[] = array('type' => $match[1], 'url' => \trim($item, '"\' '));
                    }
                } elseif (\preg_match('{^([^#].*?)(\\s+#.*)?$}', $match[2], $match2)) {
                    $result[] = array('type' => $match[1], 'url' => \trim($match2[1], '"\' '));
                }
                $key = null;
            } elseif (\preg_match('{^(\\w+)\\s*:\\s*#\\s*$}', $line, $match)) {
                $key = $match[1];
            } elseif ($key && \preg_match('{^-\\s*(.+)(\\s+#.*)?$}', $line, $match)) {
                $result[] = array('type' => $key, 'url' => \trim($match[1], '"\' '));
            }
        }
    ...
    public static function supports(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, $url, $deep = \false)
    {
        ...
        if (!\extension_loaded('openssl')) {
            $io->writeError('Skipping GitHub driver for ' . $url . ' because the OpenSSL PHP extension is missing.', \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            return \false;
        }
        return \true;
    }
    ...
    protected function getContents($url, $fetchingRepoData = \false)
    {
        try {
            ...
            $gitHubUtil = new \RectorPrefix20210503\Composer\Util\GitHub($this->io, $this->config, $this->process, $this->httpDownloader);
            switch ($e->getCode()) {
                case 401:
                case 404:
                    ...
                    if (!$this->io->isInteractive()) {
                        if ($this->attemptCloneFallback()) {
                            return new \RectorPrefix20210503\Composer\Util\Http\Response(array('url' => 'dummy'), 200, array(), 'null');
                        }
                    }
                    $scopesIssued = array();
                    $scopesNeeded = array();
                    if ($headers = $e->getHeaders()) {
                        if ($scopes = \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($headers, 'X-OAuth-Scopes')) {
                            $scopesIssued = \explode(' ', $scopes);
                        }
                        if ($scopes = \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($headers, 'X-Accepted-OAuth-Scopes')) {
    ...
    {
        $this->gitDriver = new \RectorPrefix20210503\Composer\Repository\Vcs\GitDriver(array('url' => $url), $this->io, $this->config, $this->httpDownloader, $this->process);
        $this->gitDriver->initialize();
    }
    protected function getNextPage(\RectorPrefix20210503\Composer\Util\Http\Response $response)
    {
