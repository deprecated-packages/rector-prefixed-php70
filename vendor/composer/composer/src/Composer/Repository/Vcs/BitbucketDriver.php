...
use RectorPrefix20210503\Composer\Util\Bitbucket;
use RectorPrefix20210503\Composer\Util\Http\Response;
abstract class BitbucketDriver extends \RectorPrefix20210503\Composer\Repository\Vcs\VcsDriver
{
    protected $owner;
    ...
    public function initialize()
    {
        ...
        $this->cache = new \RectorPrefix20210503\Composer\Cache($this->io, \implode('/', array($this->config->get('cache-repo-dir'), $this->originUrl, $this->owner, $this->repository)));
    ...
    {
        $resource = \sprintf('https://api.bitbucket.org/2.0/repositories/%s/%s?%s', $this->owner, $this->repository, \http_build_query(array('fields' => '-project,-owner'), null, '&'));
    ...
    public function getTags()
    {
        ...
        if (null === $this->tags) {
            $this->tags = array();
            $resource = \sprintf('%s?%s', $this->tagsUrl, \http_build_query(array('pagelen' => 100, 'fields' => 'values.name,values.target.hash,next', 'sort' => '-target.date'), null, '&'));
            $hasNext = \true;
            while ($hasNext) {
    ...
    public function getBranches()
    {
        ...
        if (null === $this->branches) {
            $this->branches = array();
            $resource = \sprintf('%s?%s', $this->branchesUrl, \http_build_query(array('pagelen' => 100, 'fields' => 'values.name,values.target.hash,values.heads,next', 'sort' => '-target.date'), null, '&'));
            $hasNext = \true;
            while ($hasNext) {
    ...
    protected function fetchWithOAuthCredentials($url, $fetchingRepoData = \false)
    {
        try {
            ...
            $bitbucketUtil = new \RectorPrefix20210503\Composer\Util\Bitbucket($this->io, $this->config, $this->process, $this->httpDownloader);
     ...
     * @return string
     */
    protected abstract function generateSshUrl();
     ...
     * @return void
     */
    protected abstract function setupFallbackDriver($url);
    /**
    ...
    {
        $resource = \sprintf('https://api.bitbucket.org/2.0/repositories/%s/%s?fields=mainbranch', $this->owner, $this->repository);
