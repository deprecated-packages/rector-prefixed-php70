 ...
 */
class HgBitbucketDriver extends \RectorPrefix20210503\Composer\Repository\Vcs\BitbucketDriver
{
    /**
     * {@inheritDoc}
     */
    public function getRootIdentifier()
    {
        ...
        if (null === $this->rootIdentifier) {
            ...
            if ($this->vcsType !== 'hg') {
                throw new \RuntimeException($this->url . ' does not appear to be a mercurial repository, use ' . $this->cloneHttpsUrl . ' if this is a git bitbucket repository');
            }
    ...
    public static function supports(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, $url, $deep = \false)
    {
        ...
        if (!\extension_loaded('openssl')) {
            $io->writeError('Skipping Bitbucket hg driver for ' . $url . ' because the OpenSSL PHP extension is missing.', \true, \RectorPrefix20210503\Composer\IO\IOInterface::VERBOSE);
            return \false;
        }
        return \true;
    }
    ...
    {
        $this->fallbackDriver = new \RectorPrefix20210503\Composer\Repository\Vcs\HgDriver(array('url' => $url), $this->io, $this->config, $this->httpDownloader, $this->process);
