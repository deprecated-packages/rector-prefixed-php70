 ...
 */
class ZipDownloader extends \RectorPrefix20210503\Composer\Downloader\ArchiveDownloader
{
    ...
    public function download(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $path, \RectorPrefix20210503\Composer\Package\PackageInterface $prevPackage = null, $output = \true)
    {
        ...
        if (null === self::$isWindows) {
            self::$isWindows = \RectorPrefix20210503\Composer\Util\Platform::isWindows();
     ...
     */
    private function extractWithSystemUnzip(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $file, $path, $isLastChance, $async = \false)
    {
        ...
        if (!self::$hasSystemUnzip && !$isLastChance) {
            ...
            return $this->extractWithZipArchive($package, $file, $path, \true);
        }
        // When called after a ZipArchive failed, perhaps there is some files to overwrite
        $overwrite = $isLastChance ? '-o' : '';
        $command = 'unzip -qq ' . $overwrite . ' ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($file) . ' -d ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($path);
        if ($async) {
            $self = $this;
            $io = $this->io;
            $tryFallback = function ($processError) use($isLastChance, $io, $self, $file, $path, $package) {
                ...
                return $self->extractWithZipArchive($package, $file, $path, \true);
            };
            try {
     ...
     * @protected
     */
    public function extractWithZipArchive(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $file, $path, $isLastChance)
    {
        ...
        if (!self::$hasZipArchive && !$isLastChance) {
            // This was call as the favorite extract way, but is not available
            // We switch to the alternative
            return $this->extractWithSystemUnzip($package, $file, $path, \true);
        }
        $processError = null;
        $zipArchive = $this->zipArchiveObject ?: new \ZipArchive();
        try {
            if (\true === ($retval = $zipArchive->open($file))) {
     ...
     * @protected
     */
    public function extract(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $file, $path)
    {
        // Each extract calls its alternative if not available or fails
        if (self::$isWindows) {
            return $this->extractWithZipArchive($package, $file, $path, \false);
        }
        return $this->extractWithSystemUnzip($package, $file, $path, \false, \true);
    }
    /**
