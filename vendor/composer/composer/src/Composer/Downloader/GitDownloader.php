 ...
 */
class GitDownloader extends \RectorPrefix20210503\Composer\Downloader\VcsDownloader implements \RectorPrefix20210503\Composer\Downloader\DvcsDownloaderInterface
{
    /**
     * @var bool[]
    ...
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process = null, \RectorPrefix20210503\Composer\Util\Filesystem $fs = null)
    {
        ...
        $this->gitUtil = new \RectorPrefix20210503\Composer\Util\Git($this->io, $this->config, $this->process, $this->filesystem);
    }
    /**
     * {@inheritDoc}
     */
    protected function doDownload(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $path, $url, \RectorPrefix20210503\Composer\Package\PackageInterface $prevPackage = null)
    {
        ...
        $gitVersion = \RectorPrefix20210503\Composer\Util\Git::getVersion($this->process);
        // --dissociate option is only available since git 2.3.0-rc0
        if ($gitVersion && \version_compare($gitVersion, '2.3.0-rc0', '>=') && \RectorPrefix20210503\Composer\Cache::isUsable($cachePath)) {
            $this->io->writeError("  - Syncing <info>" . $package->getName() . "</info> (<comment>" . $package->getFullPrettyVersion() . "</comment>) into cache");
            $this->io->writeError(\sprintf('    Cloning to cache at %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($cachePath)), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
    ...
    protected function doInstall(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $path, $url)
    {
        ...
        $cachePath = $this->config->get('cache-vcs-dir') . '/' . \preg_replace('{[^a-z0-9.]}i', '-', $url) . '/';
        $ref = $package->getSourceReference();
        $flag = \RectorPrefix20210503\Composer\Util\Platform::isWindows() ? '/D ' : '';
        if (!empty($this->cachedPackages[$package->getId()][$ref])) {
            $msg = "Cloning " . $this->getShortHash($ref) . ' from cache';
            $command = 'git clone --no-checkout %cachePath% %path% --dissociate --reference %cachePath% ' . '&& cd ' . $flag . '%path% ' . '&& git remote set-url origin -- %sanitizedUrl% && git remote add composer -- %sanitizedUrl%';
        } else {
        ...
        $commandCallable = function ($url) use($path, $command, $cachePath) {
            return \str_replace(array('%url%', '%path%', '%cachePath%', '%sanitizedUrl%'), array(\RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($path), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($cachePath), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape(\preg_replace('{://([^@]+?):(.+?)@}', '://', $url))), $command);
        };
        $this->gitUtil->runCommand($commandCallable, $url, $path, \true);
    ...
    protected function doUpdate(\RectorPrefix20210503\Composer\Package\PackageInterface $initial, \RectorPrefix20210503\Composer\Package\PackageInterface $target, $path, $url)
    {
        ...
        $commandCallable = function ($url) use($ref, $command, $cachePath) {
            return \str_replace(array('%url%', '%ref%', '%cachePath%', '%sanitizedUrl%'), array(\RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($ref . '^{commit}'), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($cachePath), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape(\preg_replace('{://([^@]+?):(.+?)@}', '://', $url))), $command);
        };
    ...
    protected function updateToCommit($path, $reference, $branch, $date)
    {
        ...
        if (!\preg_match('{^[a-f0-9]{40}$}', $reference) && $branches && \preg_match('{^\\s+composer/' . \preg_quote($reference) . '$}m', $branches)) {
            $command = \sprintf('git checkout ' . $force . '-B %s %s -- && git reset --hard %2$s --', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($branch), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape('composer/' . $reference));
            if (0 === $this->process->execute($command, $output, $path)) {
                return null;
            }
        }
        ...
        if (\preg_match('{^[a-f0-9]{40}$}', $reference)) {
            ...
            $command = \sprintf('git checkout %s --', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($branch));
            $fallbackCommand = \sprintf('git checkout ' . $force . '-B %s %s --', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($branch), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape('composer/' . $branch));
            $resetCommand = \sprintf('git reset --hard %s --', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($reference));
            if (0 === $this->process->execute("({$command} || {$fallbackCommand}) && {$resetCommand}", $output, $path)) {
                return null;
            }
        }
        $command = \sprintf($template, \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($gitRef));
        if (0 === $this->process->execute($command, $output, $path)) {
            return null;
        }
        ...
        throw new \RuntimeException(\RectorPrefix20210503\Composer\Util\Url::sanitize('Failed to execute ' . $command . "\n\n" . $this->process->getErrorOutput()));
    }
    protected function updateOriginUrl($path, $url)
    {
        $this->process->execute(\sprintf('git remote set-url origin -- %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url)), $output, $path);
        $this->setPushUrl($path, $url);
    }
    protected function setPushUrl($path, $url)
    {
        // set push url for github projects
        if (\preg_match('{^(?:https?|git)://' . \RectorPrefix20210503\Composer\Util\Git::getGitHubDomainsRegex($this->config) . '/([^/]+)/([^/]+?)(?:\\.git)?$}', $url, $match)) {
            $protocols = $this->config->get('github-protocols');
            $pushUrl = 'git@' . $match[1] . ':' . $match[2] . '/' . $match[3] . '.git';
            if (!\in_array('ssh', $protocols, \true)) {
                $pushUrl = 'https://' . $match[1] . '/' . $match[2] . '/' . $match[3] . '.git';
            }
            $cmd = \sprintf('git remote set-url --push origin -- %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($pushUrl));
    ...
    {
        $path = $this->normalizePath($path);
        $command = \sprintf('git log %s..%s --pretty=format:"%%h - %%an: %%s"' . \RectorPrefix20210503\Composer\Util\Git::getNoShowSignatureFlag($this->process), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($fromReference), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($toReference));
        if (0 !== $this->process->execute($command, $output, $path)) {
            throw new \RuntimeException('Failed to execute ' . $command . "\n\n" . $this->process->getErrorOutput());
        }
        return $output;
    ...
    {
        if (\RectorPrefix20210503\Composer\Util\Platform::isWindows() && \strlen($path) > 0) {
            $basePath = $path;
            $removed = array();
