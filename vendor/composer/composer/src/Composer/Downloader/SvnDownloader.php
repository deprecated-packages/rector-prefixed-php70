...
use RectorPrefix20210503\Composer\Util\Svn as SvnUtil;
use RectorPrefix20210503\Composer\Repository\VcsRepository;
use RectorPrefix20210503\Composer\Util\ProcessExecutor;
/**
 * @author Ben Bieker <mail@ben-bieker.de>
 * @author Till Klampaeckel <till@php.net>
 */
class SvnDownloader extends \RectorPrefix20210503\Composer\Downloader\VcsDownloader
{
    ...
    {
        \RectorPrefix20210503\Composer\Util\Svn::cleanEnv();
        $util = new \RectorPrefix20210503\Composer\Util\Svn($url, $this->io, $this->config, $this->process);
    ...
    protected function doInstall(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $path, $url)
    {
        ...
        if ($repo instanceof \RectorPrefix20210503\Composer\Repository\VcsRepository) {
     ...
     * @return string
     */
    protected function execute(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $baseUrl, $command, $url, $cwd = null, $path = null)
    {
        $util = new \RectorPrefix20210503\Composer\Util\Svn($baseUrl, $this->io, $this->config, $this->process);
    ...
    protected function cleanChanges(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $path, $update)
    {
        ...
        while (\true) {
            switch ($this->io->ask('    <info>Discard changes [y,n,v,?]?</info> ', '?')) {
                ...
                case '?':
                default:
                    $this->io->writeError(array('    y - discard changes and apply the ' . ($update ? 'update' : 'uninstall'), '    n - abort the ' . ($update ? 'update' : 'uninstall') . ' and let you manually clean things up', '    v - view modified files', '    ? - print help'));
                    break;
            }
        }
    }
    ...
    protected function getCommitLogs($fromReference, $toReference, $path)
    {
        if (\preg_match('{@(\\d+)$}', $fromReference) && \preg_match('{@(\\d+)$}', $toReference)) {
            ...
            $command = \sprintf('svn info --non-interactive --xml -- %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($path));
            ...
            $command = \sprintf('svn log -r%s:%s --incremental', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($fromRevision), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($toRevision));
            $util = new \RectorPrefix20210503\Composer\Util\Svn($baseUrl, $this->io, $this->config, $this->process);
            $util->setCacheCredentials($this->cacheCredentials);
            try {
