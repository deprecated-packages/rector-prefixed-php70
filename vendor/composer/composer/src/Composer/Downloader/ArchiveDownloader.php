 ...
 */
abstract class ArchiveDownloader extends \RectorPrefix20210503\Composer\Downloader\FileDownloader
{
    /**
     * {@inheritDoc}
    ...
    {
        if ($output) {
            $this->io->writeError("  - " . \RectorPrefix20210503\Composer\DependencyResolver\Operation\InstallOperation::format($package) . ": Extracting archive");
        } else {
        ...
        do {
            $temporaryDir = $vendorDir . '/composer/' . \substr(\md5(\uniqid('', \true)), 0, 8);
        ...
        return $promise->then(function () use($self, $package, $filesystem, $fileName, $temporaryDir, $path) {
            ...
            $getFolderContent = function ($dir) {
                $finder = \RectorPrefix20210503\Symfony\Component\Finder\Finder::create()->ignoreVCS(\false)->ignoreDotFiles(\false)->notName('.DS_Store')->depth(0)->in($dir);
     ...
     */
    protected abstract function extract(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $file, $path);
}
