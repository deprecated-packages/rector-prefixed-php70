 ...
 */
namespace RectorPrefix20210503\Composer\Autoload;
...
use RectorPrefix20210503\Composer\Semver\Constraint\Bound;
...
use RectorPrefix20210503\Composer\Util\PackageSorter;
/**
...
class AutoloadGenerator
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\EventDispatcher\EventDispatcher $eventDispatcher, \RectorPrefix20210503\Composer\IO\IOInterface $io = null)
    {
    ...
    public function dump(\RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Repository\InstalledRepositoryInterface $localRepo, \RectorPrefix20210503\Composer\Package\RootPackageInterface $rootPackage, \RectorPrefix20210503\Composer\Installer\InstallationManager $installationManager, $targetDir, $scanPsrPackages = \false, $suffix = '')
    {
        ...
        if ($this->runScripts) {
            $this->eventDispatcher->dispatchScript(\RectorPrefix20210503\Composer\Script\ScriptEvents::PRE_AUTOLOAD_DUMP, $this->devMode, array(), array('optimize' => (bool) $scanPsrPackages));
        }
        $filesystem = new \RectorPrefix20210503\Composer\Util\Filesystem();
...
EOF;
        ...
        foreach ($ambiguousClasses as $className => $ambigiousPaths) {
            ...
            $this->io->writeError('<warning>Warning: Ambiguous class resolution, "' . $className . '"' . ' was found ' . (\count($ambigiousPaths) + 1) . 'x: in "' . $cleanPath . '" and "' . \implode('", "', $ambigiousPaths) . '", the first will be used.</warning>');
        }
        $classMap['Composer\\InstalledVersions'] = "\$vendorDir . '/composer/InstalledVersions.php',\n";
        \ksort($classMap);
        foreach ($classMap as $class => $code) {
            $classmapFile .= '    ' . \var_export($class, \true) . ' => ' . $code;
        }
        $classmapFile .= ");\n";
        if (!$suffix) {
            ...
            if (!$suffix) {
                $suffix = $config->get('autoloader-suffix') ?: \md5(\uniqid('', \true));
            }
        }
        ...
        $checkPlatform = $config->get('platform-check') && $this->ignorePlatformReqs !== \true;
        ...
        $filesystem->filePutContentsIfModified($targetDir . '/autoload_real.php', $this->getAutoloadRealFile(\true, (bool) $includePathFileContents, $targetDirLoader, (bool) $includeFilesFileContents, $vendorPathCode, $appBaseDirCode, $suffix, $useGlobalIncludePath, $prependAutoloader, $staticPhpVersion, $checkPlatform));
        $filesystem->safeCopy(__DIR__ . '/ClassLoader.php', $targetDir . '/ClassLoader.php');
        $filesystem->safeCopy(__DIR__ . '/../../../LICENSE', $targetDir . '/LICENSE');
        if ($this->runScripts) {
            $this->eventDispatcher->dispatchScript(\RectorPrefix20210503\Composer\Script\ScriptEvents::POST_AUTOLOAD_DUMP, $this->devMode, array(), array('optimize' => (bool) $scanPsrPackages));
        }
    ...
    {
        foreach ($this->generateClassMap($dir, $excluded, $namespaceFilter, $autoloadType, \true, $scannedFiles) as $class => $path) {
    ...
    private function generateClassMap($dir, $excluded, $namespaceFilter, $autoloadType, $showAmbiguousWarning, array &$scannedFiles)
    {
        ...
        return \RectorPrefix20210503\Composer\Autoload\ClassMapGenerator::createMap($dir, $excluded, $showAmbiguousWarning ? $this->io : null, $namespaceFilter, $autoloadType, $scannedFiles);
    }
    /**
     * @param RootPackageInterface $rootPackage
     */
    public function buildPackageMap(\RectorPrefix20210503\Composer\Installer\InstallationManager $installationManager, \RectorPrefix20210503\Composer\Package\PackageInterface $rootPackage, array $packages)
    {
     ...
     */
    protected function validatePackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
     ...
     */
    public function parseAutoloads(array $packageMap, \RectorPrefix20210503\Composer\Package\PackageInterface $rootPackage, $filteredDevPackages = \false)
    {
        ...
        \krsort($psr0);
        \krsort($psr4);
        return array('psr-0' => $psr0, 'psr-4' => $psr4, 'classmap' => $classmap, 'files' => $files, 'exclude-from-classmap' => $exclude);
    }
    /**
    ...
    {
        $loader = new \RectorPrefix20210503\Composer\Autoload\ClassLoader($vendorDir);
    ...
    protected function getIncludePathsFile(array $packageMap, \RectorPrefix20210503\Composer\Util\Filesystem $filesystem, $basePath, $vendorPath, $vendorPathCode, $appBaseDirCode)
    {
        $includePaths = array();
...
EOF;
    ...
    protected function getIncludeFilesFile(array $files, \RectorPrefix20210503\Composer\Util\Filesystem $filesystem, $basePath, $vendorPath, $vendorPathCode, $appBaseDirCode)
    {
        $filesCode = '';
        foreach ($files as $fileIdentifier => $functionFile) {
            $filesCode .= '    ' . \var_export($fileIdentifier, \true) . ' => ' . $this->getPathCode($filesystem, $basePath, $vendorPath, $functionFile) . ",\n";
        }
        if (!$filesCode) {
            return \false;
        }
...
EOF;
    ...
    protected function getPathCode(\RectorPrefix20210503\Composer\Util\Filesystem $filesystem, $basePath, $vendorPath, $path)
    {
        ...
        return $baseDir . ($path !== \false ? \var_export($path, \true) : "");
    }
    protected function getPlatformCheck(array $packageMap, array $ignorePlatformReqs, $checkPlatform, array $devPackageNames)
    {
        $lowestPhpVersion = \RectorPrefix20210503\Composer\Semver\Constraint\Bound::zero();
        ...
        foreach ($packageMap as $item) {
            ...
            foreach (\array_merge($package->getReplaces(), $package->getProvides()) as $link) {
                if (\preg_match('{^ext-(.+)$}iD', $link->getTarget(), $match)) {
                    $extensionProviders[$match[1]][] = $link->getConstraint() ?: new \RectorPrefix20210503\Composer\Semver\Constraint\MatchAllConstraint();
                }
            }
        }
        foreach ($packageMap as $item) {
            ...
            foreach ($package->getRequires() as $link) {
                if (\in_array($link->getTarget(), $ignorePlatformReqs, \true)) {
                    continue;
                }
                ...
                if ($checkPlatform === \true && \preg_match('{^ext-(.+)$}iD', $link->getTarget(), $match)) {
        ...
        $formatToPhpVersionId = function (\RectorPrefix20210503\Composer\Semver\Constraint\Bound $bound) {
            if ($bound->isZero()) {
                return 0;
            }
        ...
        $formatToHumanReadable = function (\RectorPrefix20210503\Composer\Semver\Constraint\Bound $bound) {
            if ($bound->isZero()) {
                return 0;
            }
            if ($bound->isPositiveInfinity()) {
...
CLASSMAPAUTHORITATIVE;
        ...
        if ($this->apcu) {
            $apcuPrefix = \var_export($this->apcuPrefix !== null ? $this->apcuPrefix : \substr(\base64_encode(\md5(\uniqid('', \true), \true)), 0, -3), \true);
            $file .= <<<APCU
...
HEADER;
        $loader = new \RectorPrefix20210503\Composer\Autoload\ClassLoader();
        ...
        foreach ($maps as $prop => $value) {
            ...
            $value = \strtr(\var_export($value, \true), array($absoluteVendorPathCode => $vendorPathCode, $absoluteVendorPharPathCode => $vendorPharPathCode, $absoluteAppBaseDirCode => $appBaseDirCode, $absoluteAppBaseDirPharCode => $appBaseDirPharCode));
...
INITIALIZER;
    ...
    protected function parseAutoloadsType(array $packageMap, $type, \RectorPrefix20210503\Composer\Package\RootPackageInterface $rootPackage)
    {
        $autoloads = array();
        foreach ($packageMap as $item) {
            list($package, $installPath) = $item;
            ...
            foreach ($autoload[$type] as $namespace => $paths) {
                foreach ((array) $paths as $path) {
                    ...
                    if ($type === 'exclude-from-classmap') {
                        ...
                        $path = \preg_replace('{/+}', '/', \preg_quote(\trim(\strtr($path, '\\', '/'), '/')));
                        // add support for wildcards * and **
                        $path = \strtr($path, array('\\*\\*' => '.+?', '\\*' => '[^/]+?'));
                        // add support for up-level relative paths
                        $updir = null;
                        $path = \preg_replace_callback('{^((?:(?:\\\\\\.){1,2}+/)+)}', function ($matches) use(&$updir) {
                            if (isset($matches[1])) {
    ...
    protected function getFileIdentifier(\RectorPrefix20210503\Composer\Package\PackageInterface $package, $path)
    {
     ...
     * @return array
     */
    protected function filterPackageMap(array $packageMap, \RectorPrefix20210503\Composer\Package\RootPackageInterface $rootPackage)
    {
        $packages = array();
        $include = array();
        $replacedBy = array();
        ...
        $add = function (\RectorPrefix20210503\Composer\Package\PackageInterface $package) use(&$add, $packages, &$include, $replacedBy) {
            foreach ($package->getRequires() as $link) {
        ...
        $add($rootPackage);
        return \array_filter($packageMap, function ($item) use($include) {
            $package = $item[0];
    ...
    protected function sortPackageMap(array $packageMap)
    {
        ...
        $sortedPackages = \RectorPrefix20210503\Composer\Util\PackageSorter::sortPackages($packages);
