...
use RectorPrefix20210503\Symfony\Component\Process\ExecutableFinder;
use RectorPrefix20210503\Symfony\Component\Process\Process;
/**
 * @author Justin Rainbow <justin.rainbow@gmail.com>
 * @author Jordi Boggiano <j.boggiano@seld.be>
 */
class InitCommand extends \RectorPrefix20210503\Composer\Command\BaseCommand
{
    ...
    {
        $this->setName('init')->setDescription('Creates a basic composer.json file in current directory.')->setDefinition(array(
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('name', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Name of the package'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('description', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Description of package'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('author', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Author name of package'),
            // new InputOption('version', null, InputOption::VALUE_NONE, 'Version of package'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('type', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_OPTIONAL, 'Type of package (e.g. library, project, metapackage, composer-plugin)'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('homepage', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Homepage of package'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('require', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY | \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Package to require with a version constraint, e.g. foo/bar:1.0.0 or foo/bar=1.0.0 or "foo/bar 1.0.0"'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('require-dev', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY | \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Package to require for development with a version constraint, e.g. foo/bar:1.0.0 or foo/bar=1.0.0 or "foo/bar 1.0.0"'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('stability', 's', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Minimum stability (empty or one of: ' . \implode(', ', \array_keys(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities)) . ')'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('license', 'l', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'License of package'),
            new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('repository', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED | \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY, 'Add custom repositories, either by URL or using JSON arrays'),
        ))->setHelp(<<<EOT
...
EOT
    ...
    protected function execute(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        if ($repositories) {
            ...
            foreach ($repositories as $repo) {
                $options['repositories'][] = \RectorPrefix20210503\Composer\Repository\RepositoryFactory::configFromString($io, $config, $repo, \true);
            }
        }
        ...
        $file = new \RectorPrefix20210503\Composer\Json\JsonFile(\RectorPrefix20210503\Composer\Factory::getComposerFile());
        $json = \RectorPrefix20210503\Composer\Json\JsonFile::encode($options);
    ...
    protected function interact(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        if ($repositories) {
            $config = \RectorPrefix20210503\Composer\Factory::createConfig($io);
            $repos = array(new \RectorPrefix20210503\Composer\Repository\PlatformRepository());
            ...
            foreach ($repositories as $repo) {
                ...
                $repos[] = \RectorPrefix20210503\Composer\Repository\RepositoryFactory::createRepo($io, $config, $repoConfig);
            }
            if ($createDefaultPackagistRepo) {
                $repos[] = \RectorPrefix20210503\Composer\Repository\RepositoryFactory::createRepo($io, $config, array('type' => 'composer', 'url' => 'https://repo.packagist.org'));
            }
            $this->repos = new \RectorPrefix20210503\Composer\Repository\CompositeRepository($repos);
            unset($repos, $config, $repositories);
        }
        $io->writeError(array('', $formatter->formatBlock('Welcome to the Composer config generator', 'bg=blue;fg=white', \true), ''));
        // namespace
        $io->writeError(array('', 'This command will guide you through creating your composer.json config.', ''));
        $cwd = \realpath(".");
        if (!($name = $input->getOption('name'))) {
            $name = \basename($cwd);
            $name = \preg_replace('{(?:([a-z])([A-Z])|([A-Z])([A-Z][a-z]))}', 'RectorPrefix20210503\\1\\3-\\2\\4', $name);
        ...
        $minimumStability = $io->askAndValidate('Minimum Stability [<comment>' . $minimumStability . '</comment>]: ', function ($value) use($minimumStability) {
            if (null === $value) {
                return $minimumStability;
            }
            if (!isset(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities[$value])) {
                throw new \InvalidArgumentException('Invalid minimum stability "' . $value . '". Must be empty or one of: ' . \implode(', ', \array_keys(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities)));
            }
            return $value;
        ...
        if ($repos instanceof \RectorPrefix20210503\Composer\Repository\CompositeRepository) {
            foreach ($repos->getRepositories() as $candidateRepo) {
                if ($candidateRepo instanceof \RectorPrefix20210503\Composer\Repository\PlatformRepository) {
    ...
    public function parseAuthorString($author)
    {
        if (\preg_match('/^(?P<name>[- .,\\p{L}\\p{N}\\p{Mn}\'’"()]+) <(?P<email>.+?)>$/u', $author, $match)) {
            if ($this->isValidEmail($match['email'])) {
                return array('name' => \trim($match['name']), 'email' => $match['email']);
            }
        }
    ...
    {
        if (!$this->repos) {
            $this->repos = new \RectorPrefix20210503\Composer\Repository\CompositeRepository(\array_merge(array(new \RectorPrefix20210503\Composer\Repository\PlatformRepository()), \RectorPrefix20210503\Composer\Repository\RepositoryFactory::defaultRepos($this->getIO())));
        }
        return $this->repos;
    }
    protected final function determineRequirements(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output, $requires = array(), \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo = null, $preferredStability = 'stable', $checkProvidedVersions = \true, $fixed = \false)
    {
        if ($requires) {
        ...
        while (null !== ($package = $io->ask('Search for a package: '))) {
            ...
            if (\count($matches)) {
                ...
                if (!$exactMatch) {
                    $io->writeError(array('', \sprintf('Found <info>%s</info> packages matching <info>%s</info>', \count($matches), $package), ''));
                    ...
                    $package = $io->askAndValidate('Enter package # to add, or the complete package name if it is not listed: ', $validator, 3, \false);
                }
                ...
                if (\false !== $package && \false === \strpos($package, ' ')) {
                    ...
                    $constraint = $io->askAndValidate('Enter the version constraint to require (or leave blank to use the latest version): ', $validator, 3, \false);
                    if (\false === $constraint) {
    ...
    protected function getGitConfig()
    {
        ...
        $finder = new \RectorPrefix20210503\Symfony\Component\Process\ExecutableFinder();
        $gitBin = $finder->find('git');
        // TODO in v3 always call with an array
        if (\method_exists('RectorPrefix20210503\\Symfony\\Component\\Process\\Process', 'fromShellCommandline')) {
            $cmd = new \RectorPrefix20210503\Symfony\Component\Process\Process(array($gitBin, 'config', '-l'));
        } else {
            $cmd = new \RectorPrefix20210503\Symfony\Component\Process\Process(\sprintf('%s config -l', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($gitBin)));
        }
        $cmd->run();
        if ($cmd->isSuccessful()) {
    ...
    private function getRepositorySet(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, $minimumStability = null)
    {
        $key = $minimumStability ?: 'default';
        if (!isset($this->repositorySets[$key])) {
            $this->repositorySets[$key] = $repositorySet = new \RectorPrefix20210503\Composer\Repository\RepositorySet($minimumStability ?: $this->getMinimumStability($input));
            $repositorySet->addRepository($this->getRepos());
        }
        return $this->repositorySets[$key];
    }
    private function getMinimumStability(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input)
    {
        if ($input->hasOption('stability')) {
            return \RectorPrefix20210503\Composer\Package\Version\VersionParser::normalizeStability($input->getOption('stability') ?: 'stable');
        }
        ...
        if (\is_file($file) && \is_readable($file) && \is_array($composer = \json_decode(\file_get_contents($file), \true))) {
            if (!empty($composer['minimum-stability'])) {
                return \RectorPrefix20210503\Composer\Package\Version\VersionParser::normalizeStability($composer['minimum-stability']);
            }
        }
        return 'stable';
    }
     ...
     */
    private function findBestVersionAndNameForPackage(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, $name, \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo = null, $preferredStability = 'stable', $requiredVersion = null, $minimumStability = null, $fixed = null)
    {
        ...
        $versionSelector = new \RectorPrefix20210503\Composer\Package\Version\VersionSelector($this->getRepositorySet($input, $minimumStability), $platformRepo);
        ...
        if (!$package) {
            ...
            if ((\true === $ignorePlatformReqs || \is_array($ignorePlatformReqs) && \in_array($name, $ignorePlatformReqs)) && \RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($name)) {
                return array($name, $requiredVersion ?: '*');
            }
            // Check whether the PHP version was the problem
            if (\true !== $ignorePlatformReqs && $versionSelector->findBestCandidate($name, $requiredVersion, $preferredStability, \true)) {
                throw new \InvalidArgumentException(\sprintf('Package %s%s has a PHP requirement incompatible with your PHP version, PHP extensions and Composer version', $name, $requiredVersion ? ' at version ' . $requiredVersion : ''));
            }
            // Check whether the minimum stability was the problem but the package exists
            if ($package = $versionSelector->findBestCandidate($name, $requiredVersion, $preferredStability, $ignorePlatformReqs, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_UNACCEPTABLE_STABILITIES)) {
                // we must first verify if a valid package would be found in a lower priority repository
                if ($allReposPackage = $versionSelector->findBestCandidate($name, $requiredVersion, $preferredStability, $ignorePlatformReqs, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_SHADOWED_REPOSITORIES)) {
                ...
                throw new \InvalidArgumentException(\sprintf('Could not find a version of package %s matching your minimum-stability (%s). Require it with an explicit version constraint allowing its desired stability.', $name, $this->getMinimumStability($input)));
            }
            ...
            if ($requiredVersion && ($package = $versionSelector->findBestCandidate($name, null, $preferredStability, $ignorePlatformReqs))) {
                // we must first verify if a valid package would be found in a lower priority repository
                if ($allReposPackage = $versionSelector->findBestCandidate($name, $requiredVersion, $preferredStability, \false, \RectorPrefix20210503\Composer\Repository\RepositorySet::ALLOW_SHADOWED_REPOSITORIES)) {
                ...
                throw new \InvalidArgumentException(\sprintf('Could not find package %s in a version matching %s', $name, $requiredVersion));
            }
            // Check whether the PHP version was the problem for all versions
            if (\true !== $ignorePlatformReqs && $versionSelector->findBestCandidate($name, null, $preferredStability, \true)) {
                throw new \InvalidArgumentException(\sprintf('Could not find package %s in any version matching your PHP version, PHP extensions and Composer version', $name));
            }
            ...
            throw new \InvalidArgumentException(\sprintf('Could not find a matching version of package %s. Check the package spelling, your version constraint and that the package is available in a stability which matches your minimum-stability (%s).', $name, $this->getMinimumStability($input)));
        }
        return array($package->getPrettyName(), $fixed ? $package->getPrettyVersion() : $versionSelector->findRecommendedRequireVersion($package));
    }
    ...
    private function updateDependencies($output)
    {
        try {
            ...
            $updateCommand->run(new \RectorPrefix20210503\Symfony\Component\Console\Input\ArrayInput(array()), $output);
        } catch (\Exception $e) {
