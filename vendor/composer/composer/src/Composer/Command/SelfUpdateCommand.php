...
use RectorPrefix20210503\Composer\SelfUpdate\Keys;
use RectorPrefix20210503\Composer\SelfUpdate\Versions;
use RectorPrefix20210503\Composer\IO\IOInterface;
use RectorPrefix20210503\Composer\Downloader\FilesystemException;
 ...
 */
class SelfUpdateCommand extends \RectorPrefix20210503\Composer\Command\BaseCommand
{
    const HOMEPAGE = 'getcomposer.org';
    const OLD_INSTALL_EXT = '-old.phar';
    protected function configure()
    {
        $this->setName('self-update')->setAliases(array('selfupdate'))->setDescription('Updates composer.phar to the latest version.')->setDefinition(array(new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('rollback', 'r', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Revert to an older installation of composer'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('clean-backups', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Delete old backups during an update. This makes the current version of composer the only backup available after the update'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument('version', \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'The version to update to'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('no-progress', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Do not output download progress.'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('update-keys', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Prompt user for a key update'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('stable', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Force an update to the stable channel'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('preview', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Force an update to the preview channel'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('snapshot', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Force an update to the snapshot channel'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('1', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Force an update to the stable channel, but only use 1.x versions'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('2', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Force an update to the stable channel, but only use 2.x versions'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('set-channel-only', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Only store the channel as the default one and then exit')))->setHelp(<<<EOT
...
EOT
    ...
    protected function execute(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        $versionsUtil = new \RectorPrefix20210503\Composer\SelfUpdate\Versions($config, $httpDownloader);
        // switch channel if requested
        $requestedChannel = null;
        foreach (\RectorPrefix20210503\Composer\SelfUpdate\Versions::$channels as $channel) {
        ...
        if (!\file_exists($localFilename)) {
            throw new \RectorPrefix20210503\Composer\Downloader\FilesystemException('Composer update failed: the "' . $localFilename . '" is not accessible');
        }
        // check if current dir is writable and if not try the cache dir from settings
        $tmpDir = \is_writable(\dirname($localFilename)) ? \dirname($localFilename) : $cacheDir;
        // check for permissions in local filesystem before start connection process
        if (!\is_writable($tmpDir)) {
            throw new \RectorPrefix20210503\Composer\Downloader\FilesystemException('Composer update failed: the "' . $tmpDir . '" directory used to download the temp file could not be written');
        }
        ...
        $currentMajorVersion = \preg_replace('{^(\\d+).*}', '$1', \RectorPrefix20210503\Composer\Composer::getVersion());
        ...
        if (\RectorPrefix20210503\Composer\Composer::VERSION === $updateVersion) {
        ...
        $backupFile = \sprintf('%s/%s-%s%s', $rollbackDir, \strtr(\RectorPrefix20210503\Composer\Composer::RELEASE_DATE, ' :', '_-'), \preg_replace('{^([0-9a-f]{7})[0-9a-f]{33}$}', '$1', \RectorPrefix20210503\Composer\Composer::VERSION), self::OLD_INSTALL_EXT);
...
TAGSPUBKEY
            ...
            $signature = \json_decode($signature, \true);
        ...
        if (\file_exists($backupFile)) {
            $io->writeError(\sprintf('Use <info>composer self-update --rollback</info> to return to version <comment>%s</comment>', \RectorPrefix20210503\Composer\Composer::VERSION));
        } else {
            $io->writeError('<warning>A backup of the current version could not be written to ' . $backupFile . ', no rollback possible</warning>');
        }
        return 0;
    }
    protected function fetchKeys(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config)
    {
        if (!$io->isInteractive()) {
        ...
        $validator = function ($value) {
            if (!\preg_match('{^-----BEGIN PUBLIC KEY-----$}', \trim($value))) {
        ...
        while (!\preg_match('{(-----BEGIN PUBLIC KEY-----.+?-----END PUBLIC KEY-----)}s', $devKey, $match)) {
            ...
            while ($line = $io->ask('')) {
                $devKey .= \trim($line) . "\n";
                if (\trim($line) === '-----END PUBLIC KEY-----') {
                    break;
                }
            }
        }
        \file_put_contents($keyPath = $config->get('home') . '/keys.dev.pub', $match[0]);
        $io->write('Stored key with fingerprint: ' . \RectorPrefix20210503\Composer\SelfUpdate\Keys::fingerprint($keyPath));
        $tagsKey = '';
    ...
    protected function rollback(\RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output, $rollbackDir, $localFilename)
    {
        ...
        if (!\is_file($oldFile)) {
            throw new \RectorPrefix20210503\Composer\Downloader\FilesystemException('Composer rollback failed: "' . $oldFile . '" could not be found');
        }
        if (!\is_readable($oldFile)) {
            throw new \RectorPrefix20210503\Composer\Downloader\FilesystemException('Composer rollback failed: "' . $oldFile . '" could not be read');
        }
        $io = $this->getIO();
    ...
    protected function setLocalPhar($localFilename, $newFilename, $backupTarget = null)
    {
        ...
        try {
            ...
            throw new \RectorPrefix20210503\Composer\Downloader\FilesystemException($action . ' failed: "' . $localFilename . '" could not be written.' . \PHP_EOL . $e->getMessage());
        }
    }
    ...
    {
        return \RectorPrefix20210503\Symfony\Component\Finder\Finder::create()->depth(0)->files()->name('*' . self::OLD_INSTALL_EXT)->in($rollbackDir);
    }
    /**
