...
class CheckPlatformReqsCommand extends \RectorPrefix20210503\Composer\Command\BaseCommand
{
    protected function configure()
    {
        $this->setName('check-platform-reqs')->setDescription('Check that platform requirements are satisfied.')->setDefinition(array(new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('no-dev', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Disables checking of require-dev packages requirements.'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('lock', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Checks requirements only from the lock file, not from installed packages.')))->setHelp(<<<EOT
...
EOT
    ...
    protected function execute(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($installedRepo, new \RectorPrefix20210503\Composer\Repository\RootPackageRepository($composer->getPackage())));
        ...
        \ksort($requires);
        $installedRepo->addRepository(new \RectorPrefix20210503\Composer\Repository\PlatformRepository(array(), array()));
        $results = array();
        $exitCode = 0;
        /**
         * @var Link[] $links
        ...
        foreach ($requires as $require => $links) {
            if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($require)) {
                ...
                if ($candidates) {
                    ...
                    foreach ($candidates as $candidate) {
                        ...
                        if ($candidate->getName() === $require) {
                            $candidateConstraint = new \RectorPrefix20210503\Composer\Semver\Constraint\Constraint('=', $candidate->getVersion());
                        ...
                        foreach ($links as $link) {
                            if (!$link->getConstraint()->matches($candidateConstraint)) {
                                $reqResults[] = array($candidate->getName() === $require ? $candidate->getPrettyName() : $require, $candidateConstraint->getPrettyString(), $link, '<error>failed</error>' . ($candidate->getName() === $require ? '' : ' <comment>provided by ' . $candidate->getPrettyName() . '</comment>'));
                                // skip to next candidate
                                continue 2;
                            }
                        }
                        $results[] = array($candidate->getName() === $require ? $candidate->getPrettyName() : $require, $candidateConstraint->getPrettyString(), null, '<info>success</info>' . ($candidate->getName() === $require ? '' : ' <comment>provided by ' . $candidate->getPrettyName() . '</comment>'));
                ...
                $results[] = array($require, 'n/a', $links[0], '<error>missing</error>');
    ...
    protected function printTable(\RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output, $results)
    {
        ...
        foreach ($results as $result) {
            ...
            $rows[] = array($platformPackage, $version, $link ? \sprintf('%s %s %s (%s)', $link->getSource(), $link->getDescription(), $link->getTarget(), $link->getPrettyConstraint()) : '', $status);
        }
