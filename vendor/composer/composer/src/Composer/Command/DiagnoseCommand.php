 ...
 */
class DiagnoseCommand extends \RectorPrefix20210503\Composer\Command\BaseCommand
{
    /** @var HttpDownloader */
...
EOT
    ...
    protected function execute(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        $io = $this->getIO();
        if ($composer) {
            $commandEvent = new \RectorPrefix20210503\Composer\Plugin\CommandEvent(\RectorPrefix20210503\Composer\Plugin\PluginEvents::COMMAND, 'diagnose', $input, $output);
        ...
        $config->prohibitUrlByConfig('http://repo.packagist.org', new \RectorPrefix20210503\Composer\IO\NullIO());
        $this->httpDownloader = \RectorPrefix20210503\Composer\Factory::createHttpDownloader($io, $config);
        $this->process = new \RectorPrefix20210503\Composer\Util\ProcessExecutor($io);
        ...
        $opts = \stream_context_get_options(\RectorPrefix20210503\Composer\Util\StreamContextFactory::getContext('http://example.org'));
        ...
        if ($oauth = $config->get('github-oauth')) {
            ...
            try {
                ...
                if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $e->getCode() === 401) {
        ...
        $io->write(\sprintf('Composer version: <comment>%s</comment>', \RectorPrefix20210503\Composer\Composer::getVersion()));
        ...
        $io->write('zip: ' . (\extension_loaded('zip') ? '<comment>extension present</comment>' : '<comment>extension not loaded</comment>') . ', ' . ($hasSystemUnzip ? '<comment>unzip present</comment>' : '<comment>unzip not available</comment>'));
        return $this->exitCode;
    }
    private function checkComposerSchema()
    {
        $validator = new \RectorPrefix20210503\Composer\Util\ConfigValidator($this->getIO());
        list($errors, , $warnings) = $validator->validate(\RectorPrefix20210503\Composer\Factory::getComposerFile());
        if ($errors || $warnings) {
    ...
    private function checkHttp($proto, \RectorPrefix20210503\Composer\Config $config)
    {
        ...
        try {
            ...
            if ($hints = \RectorPrefix20210503\Composer\Util\HttpDownloader::getExceptionHints($e)) {
    ...
    private function checkGithubOauth($domain, $token)
    {
        ...
        try {
            ...
            return $this->httpDownloader->get($url, array('retry-auth-failure' => \false)) ? \true : 'Unexpected error';
        } catch (\Exception $e) {
            if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $e->getCode() === 401) {
    ...
    private function checkPubKeys($config)
    {
        ...
        if (\file_exists($home . '/keys.tags.pub')) {
            $io->write('Tags Public Key Fingerprint: ' . \RectorPrefix20210503\Composer\SelfUpdate\Keys::fingerprint($home . '/keys.tags.pub'));
        } else {
            $errors[] = '<error>Missing pubkey for tags verification</error>';
        }
        if (\file_exists($home . '/keys.dev.pub')) {
            $io->write('Dev Public Key Fingerprint: ' . \RectorPrefix20210503\Composer\SelfUpdate\Keys::fingerprint($home . '/keys.dev.pub'));
        } else {
    ...
    private function checkVersion($config)
    {
        ...
        $versionsUtil = new \RectorPrefix20210503\Composer\SelfUpdate\Versions($config, $this->httpDownloader);
        try {
            $latest = $versionsUtil->getLatest();
        } catch (\Exception $e) {
            return $e;
        }
        if (\RectorPrefix20210503\Composer\Composer::VERSION !== $latest['version'] && \RectorPrefix20210503\Composer\Composer::VERSION !== '@package_version@') {
            return '<comment>You are not running the latest ' . $versionsUtil->getChannel() . ' version, run `composer self-update` to update (' . \RectorPrefix20210503\Composer\Composer::VERSION . ' => ' . $latest['version'] . ')</comment>';
        }
        return \true;
    }
    private function getCurlVersion()
    {
        if (\extension_loaded('curl')) {
            ...
            return '<comment>' . $version['version'] . '</comment> ' . 'libz <comment>' . (isset($version['libz_version']) ? $version['libz_version'] : 'missing') . '</comment> ' . 'ssl <comment>' . (isset($version['ssl_version']) ? $version['ssl_version'] : 'missing') . '</comment>';
        }
    ...
    private function checkPlatform()
    {
        ...
        $iniMessage = \PHP_EOL . \PHP_EOL . \RectorPrefix20210503\Composer\Util\IniHelper::getMessage();
            ...
            foreach ($warnings as $warning => $current) {
                switch ($warning) {
                    ...
                    case 'openssl_version':
                        ...
                        $opensslVersion = \strstr(\trim(\strstr(\OPENSSL_VERSION_TEXT, ' ')), ' ', \true);
        ...
        return !$warnings && !$errors ? \true : $output;
    }
    /**
