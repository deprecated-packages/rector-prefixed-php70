...
use RectorPrefix20210503\Composer\DependencyResolver\DefaultPolicy;
...
use RectorPrefix20210503\Composer\Repository\ComposerRepository;
...
use RectorPrefix20210503\Composer\Semver\Semver;
use RectorPrefix20210503\Composer\Spdx\SpdxLicenses;
...
use RectorPrefix20210503\Symfony\Component\Console\Terminal;
/**
 ...
 */
class ShowCommand extends \RectorPrefix20210503\Composer\Command\BaseCommand
{
    /** @var VersionParser */
    ...
    {
        $this->setName('show')->setAliases(array('info'))->setDescription('Shows information about packages.')->setDefinition(array(new \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument('package', \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'Package to inspect. Or a name including a wildcard (*) to filter lists of packages instead.'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument('version', \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'Version or version constraint to inspect'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('all', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List all packages'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('locked', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List all locked packages'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('installed', 'i', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List installed packages only (enabled by default, only present for BC).'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('platform', 'p', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List platform packages only'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('available', 'a', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List available packages only'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('self', 's', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Show the root package information'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('name-only', 'N', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List package names only'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('path', 'P', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Show package paths'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('tree', 't', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List the dependencies as a tree'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('latest', 'l', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Show the latest version'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('outdated', 'o', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Show the latest version but only for packages that are outdated'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('ignore', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED | \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY, 'Ignore specified package(s). Use it with the --outdated option if you don\'t want to be informed about new versions of some packages.'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('minor-only', 'm', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Show only packages that have minor SemVer-compatible updates. Use with the --outdated option.'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('direct', 'D', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Shows only packages that are directly required by the root package'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('strict', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Return a non-zero exit code when there are outdated packages'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('format', 'f', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Format of the output: text or json', 'text'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('no-dev', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Disables search in require-dev packages.')))->setHelp(<<<EOT
...
EOT
    ...
    {
        $this->versionParser = new \RectorPrefix20210503\Composer\Package\Version\VersionParser();
        ...
        if ($input->getOption('self')) {
            ...
            $repos = $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array(new \RectorPrefix20210503\Composer\Repository\RootPackageRepository($package)));
        } elseif ($input->getOption('platform')) {
            $repos = $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($platformRepo));
        } elseif ($input->getOption('available')) {
            $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($platformRepo));
            if ($composer) {
                $repos = new \RectorPrefix20210503\Composer\Repository\CompositeRepository($composer->getRepositoryManager()->getRepositories());
                $installedRepo->addRepository($composer->getRepositoryManager()->getLocalRepository());
            } else {
                $defaultRepos = \RectorPrefix20210503\Composer\Repository\RepositoryFactory::defaultRepos($io);
                $repos = new \RectorPrefix20210503\Composer\Repository\CompositeRepository($defaultRepos);
            ...
            if ($locker->isLocked()) {
                ...
                $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($lockedRepo, $localRepo, $platformRepo));
            } else {
                $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($localRepo, $platformRepo));
            }
            ...
            $io->writeError('No composer.json found in the current directory, showing available packages from ' . \implode(', ', \array_keys($defaultRepos)));
            $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($platformRepo));
            $repos = new \RectorPrefix20210503\Composer\Repository\CompositeRepository(\array_merge(array($installedRepo), $defaultRepos));
            ...
            $repos = $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($lockedRepo));
        } else {
            ...
            $repos = $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array($composer->getRepositoryManager()->getLocalRepository()));
            if ($input->getOption('no-dev')) {
                $packages = $this->filterRequiredPackages($installedRepo, $rootPkg);
                $repos = $installedRepo = new \RectorPrefix20210503\Composer\Repository\InstalledRepository(array(new \RectorPrefix20210503\Composer\Repository\InstalledArrayRepository(\array_map(function ($pkg) {
                    return clone $pkg;
                }, $packages))));
            }
        ...
        if ($composer) {
            $commandEvent = new \RectorPrefix20210503\Composer\Plugin\CommandEvent(\RectorPrefix20210503\Composer\Plugin\PluginEvents::COMMAND, 'show', $input, $output);
        ...
        if ($packageFilter && \false === \strpos($packageFilter, '*') || !empty($package)) {
            if (empty($package)) {
                ...
                if (empty($package)) {
                    ...
                    if (!isset($options['working-dir']) || !\file_exists('composer.json')) {
                        if (\RectorPrefix20210503\Composer\Repository\PlatformRepository::isPlatformPackage($input->getArgument('package')) && !$input->getOption('platform')) {
            ...
            if ($input->getOption('tree')) {
                ...
                if ('json' === $format) {
                    $io->write(\RectorPrefix20210503\Composer\Json\JsonFile::encode(array('installed' => array($arrayTree))));
                } else {
        ...
        if ($input->getOption('tree')) {
            ...
            if ('json' === $format) {
                $io->write(\RectorPrefix20210503\Composer\Json\JsonFile::encode(array('installed' => $arrayTree)));
            } else {
                $this->displayPackageTree($arrayTree);
            }
            return 0;
        }
        if ($repos instanceof \RectorPrefix20210503\Composer\Repository\CompositeRepository) {
        ...
        if (\class_exists('RectorPrefix20210503\\Symfony\\Component\\Console\\Terminal')) {
            $terminal = new \RectorPrefix20210503\Symfony\Component\Console\Terminal();
        ...
        foreach ($repos as $repo) {
            ...
            if ($repo instanceof \RectorPrefix20210503\Composer\Repository\ComposerRepository) {
                ...
                foreach ($repo->getPackages() as $package) {
                    if (!isset($packages[$type][$package->getName()]) || !\is_object($packages[$type][$package->getName()]) || \version_compare($packages[$type][$package->getName()]->getVersion(), $package->getVersion(), '<')) {
                        ...
                        if (!$packageFilterRegex || \preg_match($packageFilterRegex, $package->getName())) {
                            if (!$packageListFilter || \in_array($package->getName(), $packageListFilter, \true)) {
        ...
        $viewData = array();
        $viewMetaData = array();
        foreach (array('platform' => \true, 'locked' => \true, 'available' => \false, 'installed' => \true) as $type => $showVersion) {
            if (isset($packages[$type])) {
                ...
                foreach ($packages[$type] as $package) {
                    ...
                    if (\is_object($package)) {
                        ...
                        $packageIsIgnored = \in_array($package->getPrettyName(), $ignoredPackages, \true);
                        ...
                        if ($latestPackage && $latestPackage->isAbandoned()) {
                            $replacement = \is_string($latestPackage->getReplacementPackage()) ? 'Use ' . $latestPackage->getReplacementPackage() . ' instead' : 'No replacement was suggested';
                ...
                $viewMetaData[$type] = array('nameLength' => $nameLength, 'versionLength' => $versionLength, 'latestLength' => $latestLength);
        ...
        if ('json' === $format) {
            $io->write(\RectorPrefix20210503\Composer\Json\JsonFile::encode($viewData));
        } else {
    ...
    protected function getVersionStyle(\RectorPrefix20210503\Composer\Package\PackageInterface $latestPackage, \RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
     ...
     */
    protected function getPackage(\RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, \RectorPrefix20210503\Composer\Repository\RepositoryInterface $repos, $name, $version = null)
    {
        $name = \strtolower($name);
        $constraint = \is_string($version) ? $this->versionParser->parseConstraints($version) : $version;
        $policy = new \RectorPrefix20210503\Composer\DependencyResolver\DefaultPolicy();
        $repositorySet = new \RectorPrefix20210503\Composer\Repository\RepositorySet('dev');
     ...
     */
    protected function printPackageInfo(\RectorPrefix20210503\Composer\Package\CompletePackageInterface $package, array $versions, \RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, \RectorPrefix20210503\Composer\Package\PackageInterface $latestPackage = null)
    {
        $io = $this->getIO();
        $this->printMeta($package, $versions, $installedRepo, $latestPackage ?: null);
        $this->printLinks($package, \RectorPrefix20210503\Composer\Package\Link::TYPE_REQUIRE);
        $this->printLinks($package, \RectorPrefix20210503\Composer\Package\Link::TYPE_DEV_REQUIRE, 'requires (dev)');
        ...
        $this->printLinks($package, \RectorPrefix20210503\Composer\Package\Link::TYPE_PROVIDE);
        $this->printLinks($package, \RectorPrefix20210503\Composer\Package\Link::TYPE_CONFLICT);
        $this->printLinks($package, \RectorPrefix20210503\Composer\Package\Link::TYPE_REPLACE);
    }
    /**
     ...
     * @param PackageInterface|null    $latestPackage
     */
    protected function printMeta(\RectorPrefix20210503\Composer\Package\CompletePackageInterface $package, array $versions, \RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, \RectorPrefix20210503\Composer\Package\PackageInterface $latestPackage = null)
    {
        $io = $this->getIO();
     ...
     * @param InstalledRepository      $installedRepo
     */
    protected function printVersions(\RectorPrefix20210503\Composer\Package\CompletePackageInterface $package, array $versions, \RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo)
    {
        $versions = \array_keys($versions);
        $versions = \RectorPrefix20210503\Composer\Semver\Semver::rsort($versions);
     ...
     * @param string                   $title
     */
    protected function printLinks(\RectorPrefix20210503\Composer\Package\CompletePackageInterface $package, $linkType, $title = null)
    {
     ...
     * @param CompletePackageInterface $package
     */
    protected function printLicenses(\RectorPrefix20210503\Composer\Package\CompletePackageInterface $package)
    {
        $spdxLicenses = new \RectorPrefix20210503\Composer\Spdx\SpdxLicenses();
     ...
     * @param PackageInterface|null    $latestPackage
     */
    protected function printPackageInfoAsJson(\RectorPrefix20210503\Composer\Package\CompletePackageInterface $package, array $versions, \RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, \RectorPrefix20210503\Composer\Package\PackageInterface $latestPackage = null)
    {
        $json = array('name' => $package->getPrettyName(), 'description' => $package->getDescription(), 'keywords' => $package->getKeywords() ?: array(), 'type' => $package->getType(), 'homepage' => $package->getHomepage(), 'names' => $package->getNames());
        ...
        if ($package->getSourceType()) {
            $json['source'] = array('type' => $package->getSourceType(), 'url' => $package->getSourceUrl(), 'reference' => $package->getSourceReference());
        }
        if ($package->getDistType()) {
            $json['dist'] = array('type' => $package->getDistType(), 'url' => $package->getDistUrl(), 'reference' => $package->getDistReference());
        }
        if ($installedRepo->hasPackage($package)) {
        ...
        $this->getIO()->write(\RectorPrefix20210503\Composer\Json\JsonFile::encode($json));
    }
    ...
    private function appendLicenses($json, \RectorPrefix20210503\Composer\Package\CompletePackageInterface $package)
    {
        if ($licenses = $package->getLicense()) {
            ...
            $json['licenses'] = \array_map(function ($licenseId) use($spdxLicenses) {
                ...
                return array('name' => $license[0], 'osi' => $licenseId, 'url' => $license[2]);
            }, $licenses);
        }
        return $json;
    }
    private function appendAutoload($json, \RectorPrefix20210503\Composer\Package\CompletePackageInterface $package)
    {
        if ($package->getAutoload()) {
            $autoload = array();
            foreach ($package->getAutoload() as $type => $autoloads) {
    ...
    private function appendLinks($json, \RectorPrefix20210503\Composer\Package\CompletePackageInterface $package)
    {
        foreach (\RectorPrefix20210503\Composer\Package\Link::$TYPES as $linkType) {
            $json = $this->appendLink($json, $package, $linkType);
        }
        return $json;
    }
    private function appendLink($json, \RectorPrefix20210503\Composer\Package\CompletePackageInterface $package, $linkType)
    {
     ...
     * @return array
     */
    protected function generatePackageTree(\RectorPrefix20210503\Composer\Package\PackageInterface $package, \RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, \RectorPrefix20210503\Composer\Repository\RepositoryInterface $remoteRepos)
    {
        ...
        foreach ($requires as $requireName => $require) {
            ...
            $treeChildDesc = array('name' => $requireName, 'version' => $require->getPrettyConstraint());
        ...
        $tree = array('name' => $package->getPrettyName(), 'version' => $package->getPrettyVersion(), 'description' => $package->getDescription());
        if ($children) {
     ...
     * @param int          $level
     */
    protected function displayTree($package, array $packagesInTree, $previousTreeBar = '├', $level = 1)
    {
     ...
     * @return array
     */
    protected function addTree($name, $package, \RectorPrefix20210503\Composer\Repository\InstalledRepository $installedRepo, \RectorPrefix20210503\Composer\Repository\RepositoryInterface $remoteRepos, array $packagesInTree)
    {
        $children = array();
        list($package) = $this->getPackage($installedRepo, $remoteRepos, $name, $package->getPrettyConstraint() === 'self.version' ? $package->getConstraint() : $package->getPrettyConstraint());
        if (\is_object($package)) {
            $requires = $package->getRequires();
            \ksort($requires);
            foreach ($requires as $requireName => $require) {
    ...
    private function getUpdateStatus(\RectorPrefix20210503\Composer\Package\PackageInterface $latestPackage, \RectorPrefix20210503\Composer\Package\PackageInterface $package)
    {
        ...
        if ($latestPackage->getVersion() && \RectorPrefix20210503\Composer\Semver\Semver::satisfies($latestPackage->getVersion(), $constraint)) {
     ...
     */
    private function findLatestPackage(\RectorPrefix20210503\Composer\Package\PackageInterface $package, \RectorPrefix20210503\Composer\Composer $composer, \RectorPrefix20210503\Composer\Repository\PlatformRepository $platformRepo, $minorOnly = \false)
    {
        // find the latest version allowed in this repo set
        $name = $package->getName();
        $versionSelector = new \RectorPrefix20210503\Composer\Package\Version\VersionSelector($this->getRepositorySet($composer), $platformRepo);
        $stability = $composer->getPackage()->getMinimumStability();
        $flags = $composer->getPackage()->getStabilityFlags();
        if (isset($flags[$name])) {
            $stability = \array_search($flags[$name], \RectorPrefix20210503\Composer\Package\BasePackage::$stabilities, \true);
        }
        ...
        while ($candidate instanceof \RectorPrefix20210503\Composer\Package\AliasPackage) {
            $candidate = $candidate->getAliasOf();
        }
        return $candidate;
    }
    private function getRepositorySet(\RectorPrefix20210503\Composer\Composer $composer)
    {
        if (!$this->repositorySet) {
            $this->repositorySet = new \RectorPrefix20210503\Composer\Repository\RepositorySet($composer->getPackage()->getMinimumStability(), $composer->getPackage()->getStabilityFlags());
            $this->repositorySet->addRepository(new \RectorPrefix20210503\Composer\Repository\CompositeRepository($composer->getRepositoryManager()->getRepositories()));
        }
