 ...
 */
class ConfigCommand extends \RectorPrefix20210503\Composer\Command\BaseCommand
{
    /**
     * @var Config
     */
    ...
    {
        $this->setName('config')->setDescription('Sets config options.')->setDefinition(array(new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('global', 'g', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Apply command to the global config file'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('editor', 'e', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Open editor'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('auth', 'a', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Affect auth config file (only used for --editor)'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('unset', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Unset the given setting-key'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('list', 'l', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'List configuration settings'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('file', 'f', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'If you want to choose a different composer.json or config.json'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('absolute', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Returns absolute paths when fetching *-dir config values instead of relative'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('json', 'j', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'JSON decode the setting value, to be used with extra.* keys'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('merge', 'm', \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Merge the setting value with the current value, to be used with extra.* keys in combination with --json'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption('append', null, \RectorPrefix20210503\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'When adding a repository, append it (lowest priority) to the existing ones instead of prepending it (highest priority)'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument('setting-key', null, 'Setting key'), new \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument('setting-value', \RectorPrefix20210503\Symfony\Component\Console\Input\InputArgument::IS_ARRAY, 'Setting value')))->setHelp(<<<EOT
...
EOT
    ...
    protected function initialize(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        $io = $this->getIO();
        $this->config = \RectorPrefix20210503\Composer\Factory::createConfig($io);
        // Get the local composer.json, global config.json, or if the user
        // passed in a file to use
        $configFile = $input->getOption('global') ? $this->config->get('home') . '/config.json' : ($input->getOption('file') ?: \RectorPrefix20210503\Composer\Factory::getComposerFile());
        // Create global composer.json if this was invoked using `composer global config`
        if (($configFile === 'composer.json' || $configFile === './composer.json') && !\file_exists($configFile) && \realpath(\getcwd()) === \realpath($this->config->get('home'))) {
            \file_put_contents($configFile, "{\n}\n");
        }
        $this->configFile = new \RectorPrefix20210503\Composer\Json\JsonFile($configFile, null, $io);
        $this->configSource = new \RectorPrefix20210503\Composer\Config\JsonConfigSource($this->configFile);
        $authConfigFile = $input->getOption('global') ? $this->config->get('home') . '/auth.json' : \dirname(\realpath($configFile)) . '/auth.json';
        $this->authConfigFile = new \RectorPrefix20210503\Composer\Json\JsonFile($authConfigFile, null, $io);
        $this->authConfigSource = new \RectorPrefix20210503\Composer\Config\JsonConfigSource($this->authConfigFile, \true);
        // Initialize the global file if it's not there, ignoring any warnings or notices
        if ($input->getOption('global') && !$this->configFile->exists()) {
            \touch($this->configFile->getPath());
            $this->configFile->write(array('config' => new \ArrayObject()));
            \RectorPrefix20210503\Composer\Util\Silencer::call('chmod', $this->configFile->getPath(), 0600);
        }
        if ($input->getOption('global') && !$this->authConfigFile->exists()) {
            \touch($this->authConfigFile->getPath());
            ...
            \RectorPrefix20210503\Composer\Util\Silencer::call('chmod', $this->authConfigFile->getPath(), 0600);
        }
    ...
    protected function execute(\RectorPrefix20210503\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        if ($input->getOption('editor')) {
            ...
            \system($editor . ' ' . $file . (\RectorPrefix20210503\Composer\Util\Platform::isWindows() ? '' : ' > `tty`'));
            return 0;
        }
        ...
        if (array() === $input->getArgument('setting-value') && !$input->getOption('unset')) {
            ...
            if (\preg_match('/^repos?(?:itories)?(?:\\.(.+))?/', $settingKey, $matches)) {
                ...
                $value = $this->config->get($settingKey, $input->getOption('absolute') ? 0 : \RectorPrefix20210503\Composer\Config::RELATIVE_PATHS);
            ...
            $this->getIO()->write($value, \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            return 0;
        }
        $values = $input->getArgument('setting-value');
        // what the user is trying to add/change
        $booleanValidator = function ($val) {
            return \in_array($val, array('true', 'false', '1', '0'), \true);
        };
        $booleanNormalizer = function ($val) {
            return $val !== 'false' && (bool) $val;
        };
        // handle config values
        $uniqueConfigValues = array('process-timeout' => array('is_numeric', 'intval'), 'use-include-path' => array($booleanValidator, $booleanNormalizer), 'use-github-api' => array($booleanValidator, $booleanNormalizer), 'preferred-install' => array(function ($val) {
            return \in_array($val, array('auto', 'source', 'dist'), \true);
        }, function ($val) {
            return $val;
        }), 'store-auths' => array(function ($val) {
            return \in_array($val, array('true', 'false', 'prompt'), \true);
        }, function ($val) {
            if ('prompt' === $val) {
                return 'prompt';
            }
            ...
            return \in_array($val, array('stash', 'true', 'false', '1', '0'), \true);
        }, function ($val) {
            if ('stash' === $val) {
                return 'stash';
            }
        ...
        }), 'sort-packages' => array($booleanValidator, $booleanNormalizer), 'optimize-autoloader' => array($booleanValidator, $booleanNormalizer), 'classmap-authoritative' => array($booleanValidator, $booleanNormalizer), 'apcu-autoloader' => array($booleanValidator, $booleanNormalizer), 'prepend-autoloader' => array($booleanValidator, $booleanNormalizer), 'disable-tls' => array($booleanValidator, $booleanNormalizer), 'secure-http' => array($booleanValidator, $booleanNormalizer), 'cafile' => array(function ($val) {
            ...
            return $val === 'null' ? null : $val;
        }), 'github-expose-hostname' => array($booleanValidator, $booleanNormalizer), 'htaccess-protect' => array($booleanValidator, $booleanNormalizer), 'lock' => array($booleanValidator, $booleanNormalizer), 'platform-check' => array(function ($val) {
            return \in_array($val, array('php-only', 'true', 'false', '1', '0'), \true);
        }, function ($val) {
            if ('php-only' === $val) {
                return 'php-only';
            }
        ...
        $uniqueProps = array('name' => array('is_string', function ($val) {
            ...
            return isset(\RectorPrefix20210503\Composer\Package\BasePackage::$stabilities[\RectorPrefix20210503\Composer\Semver\VersionParser::normalizeStability($val)]);
        }, function ($val) {
            return \RectorPrefix20210503\Composer\Semver\VersionParser::normalizeStability($val);
        ...
        if (\preg_match('/^repos?(?:itories)?\\.(.+)/', $settingKey, $matches)) {
            ...
            if (2 === \count($values)) {
                $this->configSource->addRepository($matches[1], array('type' => $values[0], 'url' => $values[1]), $input->getOption('append'));
                return 0;
            }
            if (1 === \count($values)) {
                ...
                if (\true === $booleanValidator($value)) {
                    ...
                    $value = \RectorPrefix20210503\Composer\Json\JsonFile::parseJson($values[0]);
        ...
        if (\preg_match('/^extra\\.(.+)/', $settingKey, $matches)) {
            ...
            if ($input->getOption('json')) {
                $value = \RectorPrefix20210503\Composer\Json\JsonFile::parseJson($value);
        ...
        if (\in_array($settingKey, array('suggest', 'extra'), \true) && $input->getOption('unset')) {
            $this->configSource->removeProperty($settingKey);
            return 0;
        }
        // handle platform
        ...
        if (\preg_match('/^(bitbucket-oauth|github-oauth|gitlab-oauth|gitlab-token|http-basic|bearer)\\.(.+)/', $settingKey, $matches)) {
            ...
            } elseif (\in_array($matches[1], array('github-oauth', 'gitlab-oauth', 'gitlab-token', 'bearer'), \true)) {
                if (1 !== \count($values)) {
     ...
     * @param string|null     $k
     */
    protected function listConfiguration(array $contents, array $rawContents, \RectorPrefix20210503\Symfony\Component\Console\Output\OutputInterface $output, $k = null)
    {
        $origK = $k;
        $io = $this->getIO();
        foreach ($contents as $key => $value) {
            ...
            if (\is_string($rawVal) && $rawVal != $value) {
                $io->write('[<comment>' . $k . $key . '</comment>] <info>' . $rawVal . ' (' . $value . ')</info>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            } else {
                $io->write('[<comment>' . $k . $key . '</comment>] <info>' . $value . '</info>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::QUIET);
            }
        }
    }
}
