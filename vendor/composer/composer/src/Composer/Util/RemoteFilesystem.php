...
use RectorPrefix20210503\Composer\Downloader\MaxFileSizeExceededException;
use RectorPrefix20210503\Composer\IO\IOInterface;
use RectorPrefix20210503\Composer\Downloader\TransportException;
use RectorPrefix20210503\Composer\CaBundle\CaBundle;
use RectorPrefix20210503\Composer\Util\Http\Response;
use RectorPrefix20210503\Composer\Util\Http\ProxyManager;
/**
 * @internal
...
class RemoteFilesystem
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, array $options = array(), $disableTls = \false, \RectorPrefix20210503\Composer\Util\AuthHelper $authHelper = null)
    {
        $this->io = $io;
        // Setup TLS options
        ...
        $this->config = $config;
        $this->authHelper = isset($authHelper) ? $authHelper : new \RectorPrefix20210503\Composer\Util\AuthHelper($io, $config);
        $this->proxyManager = \RectorPrefix20210503\Composer\Util\Http\ProxyManager::getInstance();
    }
    /**
     ...
     * @return bool true
     */
    public function copy($originUrl, $fileUrl, $fileName, $progress = \true, $options = array())
    {
     ...
     */
    public function getContents($originUrl, $fileUrl, $progress = \true, $options = array())
    {
     ...
     * @return bool|string
     */
    protected function get($originUrl, $fileUrl, $additionalOptions = array(), $fileName = null, $progress = \true)
    {
        ...
        $ctx = \RectorPrefix20210503\Composer\Util\StreamContextFactory::getContext($fileUrl, $options, array('notification' => array($this, 'callbackGet')));
        $proxy = $this->proxyManager->getProxyForRequest($fileUrl);
        $usingProxy = $proxy->getFormattedUrl(' using proxy (%s)');
        $this->io->writeError((\strpos($origFileUrl, 'http') === 0 ? 'Downloading ' : 'Reading ') . \RectorPrefix20210503\Composer\Util\Url::sanitize($origFileUrl) . $usingProxy, \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
        ...
        try {
            ...
            if (!empty($http_response_header[0])) {
                ...
                if ($statusCode >= 400 && \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($http_response_header, 'content-type') === 'application/json') {
                    \RectorPrefix20210503\Composer\Util\HttpDownloader::outputWarnings($this->io, $originUrl, \json_decode($result, \true));
                }
            ...
            $contentLength = !empty($http_response_header[0]) ? \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($http_response_header, 'content-length') : null;
            if ($contentLength && \RectorPrefix20210503\Composer\Util\Platform::strlen($result) < $contentLength) {
                ...
                $e = new \RectorPrefix20210503\Composer\Downloader\TransportException('Content-Length mismatch, received ' . \RectorPrefix20210503\Composer\Util\Platform::strlen($result) . ' bytes out of the expected ' . $contentLength);
                ...
                $this->io->writeError('Content-Length mismatch, received ' . \RectorPrefix20210503\Composer\Util\Platform::strlen($result) . ' out of ' . $contentLength . ' bytes: (' . \base64_encode($result) . ')', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                throw $e;
            }
            if (\PHP_VERSION_ID < 50600 && !empty($options['ssl']['peer_fingerprint'])) {
                ...
                $peerFingerprint = \RectorPrefix20210503\Composer\Util\TlsHelper::getCertificateFingerprint($params['options']['ssl']['peer_certificate']);
                // Constant time compare??!
                if ($expectedPeerFingerprint !== $peerFingerprint) {
                    throw new \RectorPrefix20210503\Composer\Downloader\TransportException('Peer fingerprint did not match');
                }
            }
        } catch (\Exception $e) {
            if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && !empty($http_response_header[0])) {
                $e->setHeaders($http_response_header);
                $e->setStatusCode(self::findStatusCode($http_response_header));
            }
            if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $result !== \false) {
                $e->setResponse($this->decodeResult($result, $http_response_header));
            }
            $result = \false;
        }
        ...
        if (!empty($http_response_header[0])) {
            $statusCode = self::findStatusCode($http_response_header);
            $contentType = \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($http_response_header, 'content-type');
            $locationHeader = \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($http_response_header, 'location');
        }
        // check for bitbucket login page asking to authenticate
        if ($originUrl === 'bitbucket.org' && !$this->authHelper->isPublicBitBucketDownload($fileUrl) && \substr($fileUrl, -4) === '.zip' && (!$locationHeader || \substr(\parse_url($locationHeader, \PHP_URL_PATH), -4) !== '.zip') && $contentType && \preg_match('{^text/html\\b}i', $contentType)) {
            $result = \false;
            if ($retryAuthFailure) {
        ...
        if ($statusCode === 404 && $this->config && \in_array($originUrl, $this->config->get('gitlab-domains'), \true) && \false !== \strpos($fileUrl, 'archive.zip')) {
            $result = \false;
            if ($retryAuthFailure) {
                $this->promptAuthAndRetry(401);
            }
        ...
        if ($statusCode && $statusCode >= 400 && $statusCode <= 599) {
            if (!$this->retry) {
                ...
                $e = new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $this->fileUrl . '" file could not be downloaded (' . $http_response_header[0] . ')', $statusCode);
                $e->setHeaders($http_response_header);
                $e->setResponse($this->decodeResult($result, $http_response_header));
        ...
        if (\false !== $result && null !== $fileName && !$isRedirect) {
            if ('' === $result) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException('"' . $this->fileUrl . '" appears broken, and returned an empty 200 response');
            }
            $errorMessage = '';
            \set_error_handler(function ($code, $msg) use(&$errorMessage) {
                if ($errorMessage) {
            ...
            \restore_error_handler();
            if (\false === $result) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $this->fileUrl . '" file could not be written to ' . $fileName . ': ' . $errorMessage);
            }
        }
        ...
        if (\false === $result && \false !== \strpos($errorMessage, 'Peer certificate') && \PHP_VERSION_ID < 50600) {
            ...
            if (\RectorPrefix20210503\Composer\CaBundle\CaBundle::isOpensslParseSafe()) {
        ...
        if (\false === $result) {
            $e = new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $this->fileUrl . '" file could not be downloaded: ' . $errorMessage, $errorCode);
            if (!empty($http_response_header[0])) {
                $e->setHeaders($http_response_header);
            }
            if (!$this->degradedMode && \false !== \strpos($e->getMessage(), 'Operation timed out')) {
    ...
    protected function getRemoteContents($originUrl, $fileUrl, $context, array &$responseHeaders = null, $maxFileSize = null)
    {
        ...
        if ($maxFileSize !== null && \RectorPrefix20210503\Composer\Util\Platform::strlen($result) >= $maxFileSize) {
            throw new \RectorPrefix20210503\Composer\Downloader\MaxFileSizeExceededException('Maximum allowed download size reached. Downloaded ' . \RectorPrefix20210503\Composer\Util\Platform::strlen($result) . ' of allowed ' . $maxFileSize . ' bytes');
        }
        ...
        switch ($notificationCode) {
            case \STREAM_NOTIFY_FAILURE:
                if (400 === $messageCode) {
                    ...
                    throw new \RectorPrefix20210503\Composer\Downloader\TransportException("The '" . $this->fileUrl . "' URL could not be accessed: " . $message, $messageCode);
                }
                break;
    ...
    protected function promptAuthAndRetry($httpStatus, $reason = null, $headers = array())
    {
        ...
        if ($this->retry) {
            throw new \RectorPrefix20210503\Composer\Downloader\TransportException('RETRY');
        }
    }
    protected function getOptionsForUrl($originUrl, $additionalOptions)
    {
        ...
        if ($this->disableTls === \false && \PHP_VERSION_ID < 50600 && !\stream_is_local($this->fileUrl)) {
            ...
            if (isset($this->peerCertificateMap[$urlAuthority])) {
                ...
                $this->io->writeError(\sprintf('Using <info>%s</info> as CN for subjectAltName enabled host <info>%s</info>', $certMap['cn'], $urlAuthority), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                $tlsOptions['ssl']['CN_match'] = $certMap['cn'];
                $tlsOptions['ssl']['peer_fingerprint'] = $certMap['fp'];
            } elseif (!\RectorPrefix20210503\Composer\CaBundle\CaBundle::isOpensslParseSafe() && $host === 'repo.packagist.org') {
    ...
    {
        if ($locationHeader = \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($http_response_header, 'location')) {
        ...
        if (!empty($targetUrl)) {
            $this->redirects++;
            $this->io->writeError('', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
            $this->io->writeError(\sprintf('Following redirect (%u) %s', $this->redirects, \RectorPrefix20210503\Composer\Util\Url::sanitize($targetUrl)), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
            $additionalOptions['redirects'] = $this->redirects;
            return $this->get(\parse_url($targetUrl, \PHP_URL_HOST), $targetUrl, $additionalOptions, $this->fileName, $this->progress);
        }
        if (!$this->retry) {
            $e = new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $this->fileUrl . '" file could not be downloaded, got redirect without Location (' . $http_response_header[0] . ')');
            $e->setHeaders($http_response_header);
            $e->setResponse($this->decodeResult($result, $http_response_header));
            throw $e;
        }
    ...
    private function getCertificateCnAndFp($url, $options)
    {
        ...
        $context = \RectorPrefix20210503\Composer\Util\StreamContextFactory::getContext($url, $options, array('options' => array('ssl' => array('capture_peer_cert' => \true, 'verify_peer' => \false))));
        ...
        if (!empty($params['options']['ssl']['peer_certificate'])) {
            ...
            if (\RectorPrefix20210503\Composer\Util\TlsHelper::checkCertificateHost($peerCertificate, \parse_url($url, \PHP_URL_HOST), $commonName)) {
                return array('cn' => $commonName, 'fp' => \RectorPrefix20210503\Composer\Util\TlsHelper::getCertificateFingerprint($peerCertificate));
            }
        }
    }
    private function getUrlAuthority($url)
    {
        $defaultPorts = array('ftp' => 21, 'http' => 80, 'https' => 443, 'ssh2.sftp' => 22, 'ssh2.scp' => 22);
        $scheme = \parse_url($url, \PHP_URL_SCHEME);
        if (!isset($defaultPorts[$scheme])) {
            throw new \InvalidArgumentException(\sprintf('Could not get default port for unknown scheme: %s', $scheme));
        }
    ...
    private function decodeResult($result, $http_response_header)
    {
        ...
        if ($result && \extension_loaded('zlib')) {
            $contentEncoding = \RectorPrefix20210503\Composer\Util\Http\Response::findHeaderValue($http_response_header, 'content-encoding');
            ...
            if ($decode) {
                ...
                if ($result === \false) {
                    throw new \RectorPrefix20210503\Composer\Downloader\TransportException('Failed to decode zlib stream');
                }
            }
        }
        return $result;
