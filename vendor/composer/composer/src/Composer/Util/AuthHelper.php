...
class AuthHelper
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config)
    {
        $this->io = $io;
        $this->config = $config;
    }
    ...
    public function storeAuth($origin, $storeAuth)
    {
        ...
        if ($storeAuth === \true) {
            ...
            $answer = $this->io->askAndValidate('Do you want to store credentials for ' . $origin . ' in ' . $configSource->getName() . ' ? [Yn] ', function ($value) {
    ...
    public function promptAuthIfNeeded($url, $origin, $statusCode, $reason = null, $headers = array())
    {
        ...
        if (\in_array($origin, $this->config->get('github-domains'), \true)) {
            $gitHubUtil = new \RectorPrefix20210503\Composer\Util\GitHub($this->io, $this->config, null);
            $message = "\n";
            ...
            if (!$gitHubUtil->authorizeOAuth($origin) && (!$this->io->isInteractive() || !$gitHubUtil->authorizeOAuthInteractively($origin, $message))) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException('Could not authenticate against ' . $origin, 401);
            }
            ...
            $gitLabUtil = new \RectorPrefix20210503\Composer\Util\GitLab($this->io, $this->config, null);
            if ($this->io->hasAuthentication($origin) && ($auth = $this->io->getAuthentication($origin)) && \in_array($auth['password'], array('gitlab-ci-token', 'private-token', 'oauth2'), \true)) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException("Invalid credentials for '" . $url . "', aborting.", $statusCode);
            }
            ...
            if ($this->io->hasAuthentication($origin)) {
                ...
                if ($auth['username'] !== 'x-token-auth') {
                    $bitbucketUtil = new \RectorPrefix20210503\Composer\Util\Bitbucket($this->io, $this->config);
            ...
            if (!$this->io->isInteractive()) {
                ...
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException($message, $statusCode);
            }
        ...
        return array('retry' => \true, 'storeAuth' => $storeAuth);
    }
    /**
     * @param  array  $headers
     * @param  string $origin
    ...
    public function addAuthenticationHeader(array $headers, $origin, $url)
    {
        if ($this->io->hasAuthentication($origin)) {
            ...
            } elseif (\in_array($origin, $this->config->get('gitlab-domains'), \true) && \in_array($auth['password'], array('oauth2', 'private-token', 'gitlab-ci-token'), \true)) {
            ...
            } elseif ('bitbucket.org' === $origin && $url !== \RectorPrefix20210503\Composer\Util\Bitbucket::OAUTH2_ACCESS_TOKEN_URL && 'x-token-auth' === $auth['username']) {
            ...
            if ($authenticationDisplayMessage && (!isset($this->displayedOriginAuthentications[$origin]) || $this->displayedOriginAuthentications[$origin] !== $authenticationDisplayMessage)) {
                $this->io->writeError($authenticationDisplayMessage, \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                $this->displayedOriginAuthentications[$origin] = $authenticationDisplayMessage;
            }
        } elseif (\in_array($origin, array('api.bitbucket.org', 'api.github.com'), \true)) {
