...
class ConfigValidator
{
     ...
     */
    public function validate($file, $arrayLoaderValidationFlags = \RectorPrefix20210503\Composer\Package\Loader\ValidatingArrayLoader::CHECK_ALL, $flags = self::CHECK_VERSION)
    {
        $errors = array();
        $publishErrors = array();
        $warnings = array();
        ...
        $laxValid = \false;
        try {
            $json = new \RectorPrefix20210503\Composer\Json\JsonFile($file, null, $this->io);
            $manifest = $json->read();
            $json->validateSchema(\RectorPrefix20210503\Composer\Json\JsonFile::LAX_SCHEMA);
            $laxValid = \true;
            $json->validateSchema();
        ...
        if (!empty($manifest['name']) && \preg_match('{[A-Z]}', $manifest['name'])) {
            $suggestName = \preg_replace('{(?:([a-z])([A-Z])|([A-Z])([A-Z][a-z]))}', 'RectorPrefix20210503\\1\\3-\\2\\4', $manifest['name']);
        ...
        $loader = new \RectorPrefix20210503\Composer\Package\Loader\ValidatingArrayLoader(new \RectorPrefix20210503\Composer\Package\Loader\ArrayLoader(), \true, null, $arrayLoaderValidationFlags);
        try {
        ...
        } catch (\RectorPrefix20210503\Composer\Package\Loader\InvalidPackageException $e) {
