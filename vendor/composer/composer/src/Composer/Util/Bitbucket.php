...
class Bitbucket
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process = null, \RectorPrefix20210503\Composer\Util\HttpDownloader $httpDownloader = null, $time = null)
    {
        $this->io = $io;
        $this->config = $config;
        $this->process = $process ?: new \RectorPrefix20210503\Composer\Util\ProcessExecutor($io);
        $this->httpDownloader = $httpDownloader ?: \RectorPrefix20210503\Composer\Factory::createHttpDownloader($this->io, $config);
        $this->time = $time;
    }
    /**
     * @return string
    ...
    {
        try {
            $response = $this->httpDownloader->get(self::OAUTH2_ACCESS_TOKEN_URL, array('retry-auth-failure' => \false, 'http' => array('method' => 'POST', 'content' => 'grant_type=client_credentials')));
    ...
    private function storeInAuthConfig($originUrl, $consumerKey, $consumerSecret)
    {
        ...
        $consumer = array("consumer-key" => $consumerKey, "consumer-secret" => $consumerSecret, "access-token" => $this->token['access_token'], "access-token-expiration" => $time + $this->token['expires_in']);
    ...
    private function getTokenFromConfig($originUrl)
    {
        ...
        if (!isset($authConfig[$originUrl]['access-token'], $authConfig[$originUrl]['access-token-expiration']) || \time() > $authConfig[$originUrl]['access-token-expiration']) {
            return \false;
        }
        $this->token = array('access_token' => $authConfig[$originUrl]['access-token']);
        return \true;
    }
}
