...
class Git
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process, \RectorPrefix20210503\Composer\Util\Filesystem $fs)
    {
        $this->io = $io;
        $this->config = $config;
    ...
    public function runCommand($commandCallable, $url, $cwd, $initialClone = \false)
    {
        ...
        $bypassSshForGitHub = \preg_match('{^git@' . self::getGitHubDomainsRegex($this->config) . ':(.+?)\\.git$}i', $url) && !\in_array('ssh', $protocols, \true);
        $command = \call_user_func($commandCallable, $url);
        $auth = null;
        if ($bypassSshForGitHub || 0 !== $this->process->execute($command, $ignoredOutput, $cwd)) {
            $errorMsg = $this->process->getErrorOutput();
            // private github repository without ssh key access, try https with auth
            if (\preg_match('{^git@' . self::getGitHubDomainsRegex($this->config) . ':(.+?)\\.git$}i', $url, $match) || \preg_match('{^https?://' . self::getGitHubDomainsRegex($this->config) . '/(.*?)(?:\\.git)?$}i', $url, $match)) {
                if (!$this->io->hasAuthentication($match[1])) {
                    $gitHubUtil = new \RectorPrefix20210503\Composer\Util\GitHub($this->io, $this->config, $this->process);
            ...
            } elseif (\preg_match('{^https://(bitbucket\\.org)/(.*?)(?:\\.git)?$}i', $url, $match)) {
                //bitbucket oauth
                $bitbucketUtil = new \RectorPrefix20210503\Composer\Util\Bitbucket($this->io, $this->config, $this->process);
                if (!$this->io->hasAuthentication($match[1])) {
            ...
            } elseif (\preg_match('{^(git)@' . self::getGitLabDomainsRegex($this->config) . ':(.+?\\.git)$}i', $url, $match) || \preg_match('{^(https?)://' . self::getGitLabDomainsRegex($this->config) . '/(.*)}i', $url, $match)) {
                if ($match[1] === 'git') {
                    $match[1] = 'https';
                }
                if (!$this->io->hasAuthentication($match[2])) {
                    $gitLabUtil = new \RectorPrefix20210503\Composer\Util\GitLab($this->io, $this->config, $this->process);
                ...
                if ($auth) {
                    ...
                    if (0 === $this->process->execute($command, $ignoredOutput, $cwd)) {
                        ...
                        $authHelper = new \RectorPrefix20210503\Composer\Util\AuthHelper($this->io, $this->config);
    ...
    public function syncMirror($url, $dir)
    {
        ...
        if (\is_dir($dir) && 0 === $this->process->execute('git rev-parse --git-dir', $output, $dir) && \trim($output) === '.') {
            try {
                $commandCallable = function ($url) {
                    $sanitizedUrl = \preg_replace('{://([^@]+?):(.+?)@}', '://', $url);
                    return \sprintf('git remote set-url origin -- %s && git remote update --prune origin && git remote set-url origin -- %s && git gc --auto', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($sanitizedUrl));
                };
                $this->runCommand($commandCallable, $url, $dir);
            } catch (\Exception $e) {
                $this->io->writeError('<error>Sync mirror failed: ' . $e->getMessage() . '</error>', \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                return \false;
            }
            return \true;
        }
        ...
        $commandCallable = function ($url) use($dir) {
            return \sprintf('git clone --mirror -- %s %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($dir));
        };
        $this->runCommand($commandCallable, $url, $dir, \true);
        return \true;
    }
    ...
    public static function getNoShowSignatureFlag(\RectorPrefix20210503\Composer\Util\ProcessExecutor $process)
    {
    ...
    {
        if (\is_dir($dir) && 0 === $this->process->execute('git rev-parse --git-dir', $output, $dir) && \trim($output) === '.') {
            $escapedRef = \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($ref . '^{commit}');
    ...
    private function isAuthenticationFailure($url, &$match)
    {
        ...
        $authFailures = array('fatal: Authentication failed', 'remote error: Invalid username or password.', 'error: 401 Unauthorized', 'fatal: unable to access', 'fatal: could not read Username');
    ...
    public static function getGitHubDomainsRegex(\RectorPrefix20210503\Composer\Config $config)
    {
        return '(' . \implode('|', \array_map('preg_quote', $config->get('github-domains'))) . ')';
    }
    public static function getGitLabDomainsRegex(\RectorPrefix20210503\Composer\Config $config)
    {
    ...
    private function throwException($message, $url)
    {
        ...
        if (0 !== $this->process->execute('git --version', $ignoredOutput)) {
            throw new \RuntimeException(\RectorPrefix20210503\Composer\Util\Url::sanitize('Failed to clone ' . $url . ', git was not found, check that it is installed and in your PATH env.' . "\n\n" . $this->process->getErrorOutput()));
        }
        throw new \RuntimeException(\RectorPrefix20210503\Composer\Util\Url::sanitize($message));
    }
    /**
     ...
     */
    public static function getVersion(\RectorPrefix20210503\Composer\Util\ProcessExecutor $process)
    {
