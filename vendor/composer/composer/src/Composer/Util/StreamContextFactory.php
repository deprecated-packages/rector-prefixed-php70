...
final class StreamContextFactory
{
    ...
    public static function initOptions($url, array $options, $forCurl = \false)
    {
        ...
        if (!$forCurl) {
            $proxy = \RectorPrefix20210503\Composer\Util\Http\ProxyManager::getInstance()->getProxyForRequest($url);
            if ($proxyOptions = $proxy->getContextOptions()) {
                $isHttpsRequest = 0 === \strpos($url, 'https://');
                if ($proxy->isSecure()) {
                    if (!\extension_loaded('openssl')) {
                        throw new \RectorPrefix20210503\Composer\Downloader\TransportException('You must enable the openssl extension to use a secure proxy.');
                    }
                    if ($isHttpsRequest) {
                        throw new \RectorPrefix20210503\Composer\Downloader\TransportException('You must enable the curl extension to make https requests through a secure proxy.');
                    }
                } elseif ($isHttpsRequest && !\extension_loaded('openssl')) {
                    throw new \RectorPrefix20210503\Composer\Downloader\TransportException('You must enable the openssl extension to make https requests through a proxy.');
                }
        ...
        if (!isset($options['http']['header']) || \false === \stripos(\implode('', $options['http']['header']), 'user-agent')) {
            $options['http']['header'][] = \sprintf('User-Agent: Composer/%s (%s; %s; %s; %s%s)', \RectorPrefix20210503\Composer\Composer::getVersion(), \function_exists('php_uname') ? \php_uname('s') : 'Unknown', \function_exists('php_uname') ? \php_uname('r') : 'Unknown', $phpVersion, $httpVersion, \getenv('CI') ? '; CI' : '');
        }
        return $options;
    }
    /**
     ...
     * @return array
     */
    public static function getTlsDefaults(array $options, \RectorPrefix20210503\Psr\Log\LoggerInterface $logger = null)
    {
         ...
         */
        $defaults = array('ssl' => array('ciphers' => $ciphers, 'verify_peer' => \true, 'verify_depth' => 7, 'SNI_enabled' => \true, 'capture_peer_cert' => \true));
        ...
        if (!isset($defaults['ssl']['cafile']) && !isset($defaults['ssl']['capath'])) {
            $result = \RectorPrefix20210503\Composer\CaBundle\CaBundle::getSystemCaRootBundlePath($logger);
            if (\is_dir($result)) {
        ...
        if (isset($defaults['ssl']['cafile']) && (!\is_readable($defaults['ssl']['cafile']) || !\RectorPrefix20210503\Composer\CaBundle\CaBundle::validateCaFile($defaults['ssl']['cafile'], $logger))) {
            throw new \RectorPrefix20210503\Composer\Downloader\TransportException('The configured cafile was not valid or could not be read.');
        }
        if (isset($defaults['ssl']['capath']) && (!\is_dir($defaults['ssl']['capath']) || !\is_readable($defaults['ssl']['capath']))) {
            throw new \RectorPrefix20210503\Composer\Downloader\TransportException('The configured capath was not valid or could not be read.');
        }
        /**
