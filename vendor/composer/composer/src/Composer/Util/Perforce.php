...
class Perforce
{
    ...
    public function __construct($repoConfig, $port, $path, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process, $isWindows, \RectorPrefix20210503\Composer\IO\IOInterface $io)
    {
    ...
    public static function create($repoConfig, $port, $path, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process, \RectorPrefix20210503\Composer\IO\IOInterface $io)
    {
        return new \RectorPrefix20210503\Composer\Util\Perforce($repoConfig, $port, $path, $process, \RectorPrefix20210503\Composer\Util\Platform::isWindows(), $io);
    }
    public static function checkServerExists($url, \RectorPrefix20210503\Composer\Util\ProcessExecutor $processExecutor)
    {
        $output = null;
        return 0 === $processExecutor->execute('p4 -p ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url) . ' info -s', $output);
    }
    ...
    public function cleanupClientSpec()
    {
        ...
        $task = 'client -d ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($client);
        $useP4Client = \false;
    ...
    public function generateP4Command($command, $useClient = \true)
    {
        $p4Command = 'p4 ';
    ...
    public function windowsLogin($password)
    {
        ...
        if (\method_exists('RectorPrefix20210503\\Symfony\\Component\\Process\\Process', 'fromShellCommandline')) {
            $process = \RectorPrefix20210503\Symfony\Component\Process\Process::fromShellCommandline($command, null, null, $password);
        } else {
            $process = new \RectorPrefix20210503\Symfony\Component\Process\Process($command, null, null, $password);
        }
        return $process->run();
    }
    public function p4Login()
    {
        ...
        if (!$this->isLoggedIn()) {
            ...
            if ($this->windowsFlag) {
                ...
                $command = 'echo ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($password) . ' | ' . $this->generateP4Command(' login -a', \false);
                $exitCode = $this->executeCommand($command);
                if ($exitCode) {
    ...
    public function getFileContent($file, $identifier)
    {
        ...
        $command = $this->generateP4Command(' print ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($path));
        $this->executeCommand($command);
    ...
    public function getFilePath($file, $identifier)
    {
        ...
        $command = $this->generateP4Command(' files ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($path), \false);
        $this->executeCommand($command);
        $result = $this->commandResult;
    ...
    public function getBranches()
    {
        ...
        if (!$this->isStream()) {
            ...
            $command = $this->generateP4Command('streams ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape('//' . $this->p4Depot . '/...'));
            $this->executeCommand($command);
            $result = $this->commandResult;
            $resArray = \explode(\PHP_EOL, $result);
            foreach ($resArray as $line) {
        ...
        $command = $this->generateP4Command('changes ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($this->getStream() . '/...'), \false);
        $this->executeCommand($command);
        $result = $this->commandResult;
        $resArray = \explode(\PHP_EOL, $result);
    ...
    protected function getChangeList($reference)
    {
        ...
        $command = $this->generateP4Command(' changes -m1 ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($label));
        $this->executeCommand($command);
    ...
    public function getCommitLogs($fromReference, $toReference)
    {
        ...
        $command = $this->generateP4Command('filelog ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($main . '@' . $fromChangeList . ',' . $toChangeList));
        $this->executeCommand($command);
    ...
    public function getFilesystem()
    {
        if (empty($this->filesystem)) {
            $this->filesystem = new \RectorPrefix20210503\Composer\Util\Filesystem($this->process);
        }
        return $this->filesystem;
    }
    public function setFilesystem(\RectorPrefix20210503\Composer\Util\Filesystem $fs)
    {
        $this->filesystem = $fs;
    }
}
