...
class Svn
{
     ...
     */
    public function __construct($url, \RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process = null)
    {
        $this->url = $url;
        $this->io = $io;
        $this->config = $config;
    ...
    {
        $cmd = \sprintf('%s %s%s -- %s', $cmd, '--non-interactive ', $this->getCredentialString(), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($url));
        if ($path) {
            $cmd .= ' ' . \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($path);
        }
        return $cmd;
    }
    /**
    ...
    protected function getCredentialString()
    {
        ...
        return \sprintf(' %s--username %s --password %s ', $this->getAuthCache(), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($this->getUsername()), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($this->getPassword()));
    }
    /**
