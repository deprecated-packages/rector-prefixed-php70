...
class Filesystem
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\Util\ProcessExecutor $executor = null)
    {
    ...
    {
        $finder = \RectorPrefix20210503\Symfony\Component\Finder\Finder::create()->ignoreVCS(\false)->ignoreDotFiles(\false)->depth(0)->in($dir);
        return \count($finder) === 0;
    }
    public function emptyDirectory($dir, $ensureDirectoryExists = \true)
    {
    ...
    public function removeDirectory($directory)
    {
        ...
        if (\RectorPrefix20210503\Composer\Util\Platform::isWindows()) {
            $cmd = \sprintf('rmdir /S /Q %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape(\realpath($directory)));
        } else {
            $cmd = \sprintf('rm -rf %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($directory));
        }
    ...
    public function rename($source, $target)
    {
        ...
        if (\RectorPrefix20210503\Composer\Util\Platform::isWindows()) {
            ...
            $command = \sprintf('xcopy %s %s /E /I /Q /Y', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($source), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($target));
            ...
            $command = \sprintf('mv %s %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($source), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape($target));
            $result = $this->getProcess()->execute($command, $output);
            // clear stat cache because external processes aren't tracked by the php stat cache
            \clearstatcache();
            if (0 === $result) {
    ...
    public function findShortestPathCode($from, $to, $directories = \false, $staticCode = \false)
    {
        ...
        $commonPath = \rtrim($commonPath, '/') . '/';
        if (\strpos($to, $from . '/') === 0) {
            return '__DIR__ . ' . \var_export(\substr($to, \strlen($from)), \true);
        }
        ...
        return $commonPathCode . (\strlen($relTarget) ? '.' . \var_export('/' . $relTarget, \true) : '');
    }
    /**
    ...
    protected function getProcess()
    {
        if (!$this->processExecutor) {
            $this->processExecutor = new \RectorPrefix20210503\Composer\Util\ProcessExecutor();
        }
    ...
    {
        if (\RectorPrefix20210503\Composer\Util\Platform::isWindows() && \is_dir($path) && \is_link($path)) {
            return \rmdir($path);
        }
        return \unlink($path);
    }
    ...
    public function junction($target, $junction)
    {
        ...
        if (!\is_dir($target)) {
            throw new \RectorPrefix20210503\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Cannot junction to "%s" as it is not a directory.', $target), 0, null, $target);
        }
        $cmd = \sprintf('mklink /J %s %s', \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape(\str_replace('/', \DIRECTORY_SEPARATOR, $junction)), \RectorPrefix20210503\Composer\Util\ProcessExecutor::escape(\realpath($target)));
        if ($this->getProcess()->execute($cmd, $output) !== 0) {
            throw new \RectorPrefix20210503\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Failed to create junction to "%s" at "%s".', $target, $junction), 0, null, $target);
        }
    ...
    public function removeJunction($junction)
    {
        ...
        if (!$this->isJunction($junction)) {
            throw new \RectorPrefix20210503\Symfony\Component\Filesystem\Exception\IOException(\sprintf('%s is not a junction and thus cannot be removed as one', $junction));
        }
