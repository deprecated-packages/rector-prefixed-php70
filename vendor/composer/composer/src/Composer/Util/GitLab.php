...
class GitLab
{
     ...
     */
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, \RectorPrefix20210503\Composer\Util\ProcessExecutor $process = null, \RectorPrefix20210503\Composer\Util\HttpDownloader $httpDownloader = null)
    {
        $this->io = $io;
        $this->config = $config;
    ...
    public function authorizeOAuth($originUrl)
    {
        ...
        if (0 === $this->process->execute('git config gitlab.deploytoken.user', $tokenUser) && 0 === $this->process->execute('git config gitlab.deploytoken.token', $tokenPassword)) {
            $this->io->setAuthentication($originUrl, \trim($tokenUser), \trim($tokenPassword));
            return \true;
        }
    ...
    private function createToken($scheme, $originUrl)
    {
        ...
        $apiUrl = $originUrl;
        $data = \http_build_query(array('username' => $username, 'password' => $password, 'grant_type' => 'password'), null, '&');
        $options = array('retry-auth-failure' => \false, 'http' => array('method' => 'POST', 'header' => $headers, 'content' => $data));
