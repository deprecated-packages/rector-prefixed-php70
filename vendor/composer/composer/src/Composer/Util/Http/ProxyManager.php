...
use RectorPrefix20210503\Composer\Util\NoProxyPattern;
...
class ProxyManager
{
    ...
    {
        $this->fullProxy = $this->safeProxy = array('http' => null, 'https' => null);
        $this->streams['http'] = $this->streams['https'] = array('options' => null);
        $this->hasProxy = \false;
        $this->initProxyData();
    }
    /**
    ...
    {
        if ($this->error) {
            throw new \RectorPrefix20210503\Composer\Downloader\TransportException('Unable to use a proxy: ' . $this->error);
        }
        $scheme = \parse_url($requestUrl, \PHP_URL_SCHEME) ?: 'http';
        $proxyUrl = '';
        $options = array();
        $formattedProxyUrl = '';
        if ($this->hasProxy && \in_array($scheme, array('http', 'https'), \true) && $this->fullProxy[$scheme]) {
            if ($this->noProxy($requestUrl)) {
                ...
                \RectorPrefix20210503\Composer\Util\Http\ProxyHelper::setRequestFullUri($requestUrl, $options);
                $formattedProxyUrl = $this->safeProxy[$scheme];
            }
        }
        return new \RectorPrefix20210503\Composer\Util\Http\RequestProxy($proxyUrl, $options, $formattedProxyUrl);
    }
    /**
    ...
    {
        try {
            list($httpProxy, $httpsProxy, $noProxy) = \RectorPrefix20210503\Composer\Util\Http\ProxyHelper::getProxyData();
        ...
        if ($this->hasProxy) {
            ...
            if ($noProxy) {
                $this->noProxyHandler = array(new \RectorPrefix20210503\Composer\Util\NoProxyPattern($noProxy), 'test');
            }
        }
    }
    /**
    ...
    {
        $safeProxy = \RectorPrefix20210503\Composer\Util\Url::sanitize($url);
        $this->fullProxy[$scheme] = $url;
        $this->safeProxy[$scheme] = $safeProxy;
        $this->streams[$scheme]['options'] = \RectorPrefix20210503\Composer\Util\Http\ProxyHelper::getContextOptions($url);
        $this->hasProxy = \true;
