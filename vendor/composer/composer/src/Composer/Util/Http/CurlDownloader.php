 ...
 */
namespace RectorPrefix20210503\Composer\Util\Http;
...
use RectorPrefix20210503\Composer\Util\StreamContextFactory;
use RectorPrefix20210503\Composer\Util\AuthHelper;
...
class CurlDownloader
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io, \RectorPrefix20210503\Composer\Config $config, array $options = array(), $disableTls = \false)
    {
        ...
        $this->authHelper = new \RectorPrefix20210503\Composer\Util\AuthHelper($io, $config);
    ...
    {
        $attributes = \array_merge(array('retryAuthFailure' => \true, 'redirects' => 0, 'storeAuth' => \false), $attributes);
        ...
        if ($copyTo) {
            ...
            if (!$bodyHandle) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $url . '" file could not be written to ' . $copyTo . ': ' . $errorMessage);
            }
        } else {
        ...
        $options = \RectorPrefix20210503\Composer\Util\StreamContextFactory::initOptions($url, $options, \true);
        ...
        if ($proxy->isSecure()) {
            if (!$this->supportsSecureProxy) {
                throw new \RectorPrefix20210503\Composer\Downloader\TransportException('Connecting to a secure proxy using curl is not supported on PHP versions below 7.3.0.');
            }
        ...
        $this->jobs[(int) $curlHandle] = array('url' => $url, 'origin' => $origin, 'attributes' => $attributes, 'options' => $originalOptions, 'progress' => $progress, 'curlHandle' => $curlHandle, 'filename' => $copyTo, 'headerHandle' => $headerHandle, 'bodyHandle' => $bodyHandle, 'resolve' => $resolve, 'reject' => $reject);
        ...
        if ($attributes['redirects'] === 0) {
            $this->io->writeError('Downloading ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($url) . $usingProxy . $ifModified, \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
        }
        ...
        while ($progress = \curl_multi_info_read($this->multiHandle)) {
            ...
            try {
                ...
                if (\CURLE_OK !== $errno || $error || $result !== \CURLE_OK) {
                    ...
                    throw new \RectorPrefix20210503\Composer\Downloader\TransportException('curl error ' . $errno . ' while downloading ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($progress['url']) . ': ' . $error);
                }
                ...
                \fclose($job['headerHandle']);
                if ($statusCode === 0) {
                    throw new \LogicException('Received unexpected http status code 0 without error for ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($progress['url']) . ': headers ' . \var_export($headers, \true) . ' curl info ' . \var_export($progress, \true));
                }
                ...
                if ($job['filename']) {
                    ...
                    $response = new \RectorPrefix20210503\Composer\Util\Http\CurlResponse(array('url' => $progress['url']), $statusCode, $headers, $contents, $progress);
                    $this->io->writeError('[' . $statusCode . '] ' . \RectorPrefix20210503\Composer\Util\Url::sanitize($progress['url']), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
                } else {
                    \rewind($job['bodyHandle']);
                    $contents = \stream_get_contents($job['bodyHandle']);
                    $response = new \RectorPrefix20210503\Composer\Util\Http\CurlResponse(array('url' => $progress['url']), $statusCode, $headers, $contents, $progress);
                ...
                if ($response->getStatusCode() >= 400 && $response->getHeader('content-type') === 'application/json') {
                    \RectorPrefix20210503\Composer\Util\HttpDownloader::outputWarnings($this->io, $job['origin'], \json_decode($response->getBody(), \true));
                }
                ...
                if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $headers) {
                    $e->setHeaders($headers);
                    $e->setStatusCode($statusCode);
                }
                if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $response) {
                    $e->setResponse($response->getBody());
                }
                if ($e instanceof \RectorPrefix20210503\Composer\Downloader\TransportException && $progress) {
            ...
            if ($this->jobs[$i]['progress'] !== $progress) {
                ...
                if (isset($this->jobs[$i]['options']['max_file_size'])) {
                    ...
                    if ($this->jobs[$i]['options']['max_file_size'] < $progress['download_content_length']) {
                        $this->rejectJob($this->jobs[$i], new \RectorPrefix20210503\Composer\Downloader\MaxFileSizeExceededException('Maximum allowed download size reached. Content-length header indicates ' . $progress['download_content_length'] . ' bytes. Allowed ' . $this->jobs[$i]['options']['max_file_size'] . ' bytes'));
                    }
                    // Compare max_file_size with the download size in bytes
                    if ($this->jobs[$i]['options']['max_file_size'] < $progress['size_download']) {
                        $this->rejectJob($this->jobs[$i], new \RectorPrefix20210503\Composer\Downloader\MaxFileSizeExceededException('Maximum allowed download size reached. Downloaded ' . $progress['size_download'] . ' of allowed ' . $this->jobs[$i]['options']['max_file_size'] . ' bytes'));
                    }
                }
                // TODO progress
            }
    ...
    private function handleRedirect(array $job, \RectorPrefix20210503\Composer\Util\Http\Response $response)
    {
        ...
        if (!empty($targetUrl)) {
            $this->io->writeError(\sprintf('Following redirect (%u) %s', $job['attributes']['redirects'] + 1, \RectorPrefix20210503\Composer\Util\Url::sanitize($targetUrl)), \true, \RectorPrefix20210503\Composer\IO\IOInterface::DEBUG);
            return $targetUrl;
        }
        throw new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $job['url'] . '" file could not be downloaded, got redirect without Location (' . $response->getStatusMessage() . ')');
    }
    private function isAuthenticatedRetryNeeded(array $job, \RectorPrefix20210503\Composer\Util\Http\Response $response)
    {
        ...
        if ($job['origin'] === 'bitbucket.org' && !$this->authHelper->isPublicBitBucketDownload($job['url']) && \substr($job['url'], -4) === '.zip' && (!$locationHeader || \substr($locationHeader, -4) !== '.zip') && \preg_match('{^text/html\\b}i', $response->getHeader('content-type'))) {
            $needsAuthRetry = 'Bitbucket requires authentication and it was not provided';
        }
        // check for gitlab 404 when downloading archives
        if ($response->getStatusCode() === 404 && $this->config && \in_array($job['origin'], $this->config->get('gitlab-domains'), \true) && \false !== \strpos($job['url'], 'archive.zip')) {
    ...
    private function restartJob(array $job, $url, array $attributes = array())
    {
        ...
        $origin = \RectorPrefix20210503\Composer\Util\Url::getOrigin($this->config, $url);
        $this->initDownload($job['resolve'], $job['reject'], $origin, $url, $job['options'], $job['filename'], $attributes);
    }
    private function failResponse(array $job, \RectorPrefix20210503\Composer\Util\Http\Response $response, $errorMessage)
    {
        if ($job['filename']) {
            @\unlink($job['filename'] . '~');
        }
        ...
        return new \RectorPrefix20210503\Composer\Downloader\TransportException('The "' . $job['url'] . '" file could not be downloaded (' . $errorMessage . ')' . $details, $response->getStatusCode());
    }
