...
use RectorPrefix20210503\Symfony\Component\Process\ProcessUtils;
...
class ProcessExecutor
{
    ...
    public function __construct(\RectorPrefix20210503\Composer\IO\IOInterface $io = null)
    {
        $this->io = $io;
    }
    /**
    ...
    {
        if (\RectorPrefix20210503\Composer\Util\Platform::isTty()) {
    ...
    private function doExecute($command, $cwd, $tty, &$output = null)
    {
        ...
        if (null === $cwd && \RectorPrefix20210503\Composer\Util\Platform::isWindows() && \false !== \strpos($command, 'git') && \getcwd()) {
            $cwd = \realpath(\getcwd());
        }
        ...
        if (\method_exists('RectorPrefix20210503\\Symfony\\Component\\Process\\Process', 'fromShellCommandline')) {
            $process = \RectorPrefix20210503\Symfony\Component\Process\Process::fromShellCommandline($command, $cwd, null, null, static::getTimeout());
        } else {
            $process = new \RectorPrefix20210503\Symfony\Component\Process\Process($command, $cwd, null, null, static::getTimeout());
        }
        if (!\RectorPrefix20210503\Composer\Util\Platform::isWindows() && $tty) {
            try {
                $process->setTty(\true);
            } catch (\RectorPrefix20210503\Symfony\Component\Process\Exception\RuntimeException $e) {
    ...
    public function executeAsync($command, $cwd = null)
    {
        ...
        $job = array('id' => $this->idGen++, 'status' => self::STATUS_QUEUED, 'command' => $command, 'cwd' => $cwd);
        $resolver = function ($resolve, $reject) use(&$job) {
            $job['status'] = \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_QUEUED;
            $job['resolve'] = $resolve;
            $job['reject'] = $reject;
        };
        $self = $this;
        $canceler = function () use(&$job) {
            if ($job['status'] === \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_QUEUED) {
                $job['status'] = \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_ABORTED;
            }
            if ($job['status'] !== \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_STARTED) {
                return;
            }
            $job['status'] = \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_ABORTED;
            try {
        ...
        $promise = $promise->then(function () use(&$job, $self) {
            if ($job['process']->isSuccessful()) {
                $job['status'] = \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_COMPLETED;
            } else {
                $job['status'] = \RectorPrefix20210503\Composer\Util\ProcessExecutor::STATUS_FAILED;
            }
    ...
    public function outputHandler($type, $buffer)
    {
        ...
        if (\RectorPrefix20210503\Symfony\Component\Process\Process::ERR === $type) {
