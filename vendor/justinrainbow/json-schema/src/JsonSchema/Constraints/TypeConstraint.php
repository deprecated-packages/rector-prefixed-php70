 ...
 */
class TypeConstraint extends \RectorPrefix20210519\JsonSchema\Constraints\Constraint
{
    /**
     ...
     * {@inheritdoc}
     */
    public function check(&$value = null, $schema = null, \RectorPrefix20210519\JsonSchema\Entity\JsonPointer $path = null, $i = null)
    {
    ...
    protected function validateTypeNameWording($type)
    {
        if (!isset(self::$wording[$type])) {
            throw new \UnexpectedValueException(\sprintf('No wording for %s available, expected wordings are: [%s]', \var_export($type, \true), \implode(', ', \array_filter(self::$wording))));
        }
    }
    /**
    ...
    protected function validateType(&$value, $type)
    {
        ...
        $coerce = $this->factory->getConfig(\RectorPrefix20210519\JsonSchema\Constraints\Constraint::CHECK_MODE_COERCE_TYPES);
        if ('integer' === $type) {
            if ($coerce) {
        ...
        throw new \RectorPrefix20210519\JsonSchema\Exception\InvalidArgumentException((\is_object($value) ? 'object' : $value) . ' is an invalid type for ' . $type);
    }
    /**
