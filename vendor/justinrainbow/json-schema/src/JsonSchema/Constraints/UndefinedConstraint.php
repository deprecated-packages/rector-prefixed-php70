...
use RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck;
use RectorPrefix20210520\JsonSchema\Entity\JsonPointer;
use RectorPrefix20210520\JsonSchema\Exception\ValidationException;
use RectorPrefix20210520\JsonSchema\Uri\UriResolver;
/**
 ...
 */
class UndefinedConstraint extends \RectorPrefix20210520\JsonSchema\Constraints\Constraint
{
    /**
     ...
     * {@inheritdoc}
     */
    public function check(&$value, $schema = null, \RectorPrefix20210520\JsonSchema\Entity\JsonPointer $path = null, $i = null, $fromDefault = \false)
    {
        if (\is_null($schema) || !\is_object($schema)) {
            return;
        }
        $path = $this->incrementPath($path ?: new \RectorPrefix20210520\JsonSchema\Entity\JsonPointer(''), $i);
        if ($fromDefault) {
            $path->setFromDefault();
        }
     ...
     * @param string      $i
     */
    public function validateTypes(&$value, $schema, \RectorPrefix20210520\JsonSchema\Entity\JsonPointer $path, $i = null)
    {
        // check array
        ...
        // check object
        if (\RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::isObject($value)) {
     ...
     * @param string      $i
     */
    protected function validateCommonProperties(&$value, $schema, \RectorPrefix20210520\JsonSchema\Entity\JsonPointer $path, $i = '')
    {
            ...
            if (!$value instanceof self && isset($schema->required) && \is_array($schema->required)) {
                ...
                foreach ($schema->required as $required) {
                    if (!$this->getTypeCheck()->propertyExists($value, $required)) {
                        $this->addError($this->incrementPath($path ?: new \RectorPrefix20210520\JsonSchema\Entity\JsonPointer(''), $required), 'The property ' . $required . ' is required', 'required');
                    }
                }
    ...
    protected function applyDefaultValues(&$value, $schema, $path)
    {
        ...
        if (isset($schema->properties) && \RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::isObject($value)) {
            ...
            foreach ($schema->properties as $currentProperty => $propertyDefinition) {
                ...
                if (!\RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::propertyExists($value, $currentProperty) && \property_exists($propertyDefinition, 'default') && $this->shouldApplyDefaultValue($requiredOnly, $propertyDefinition, $currentProperty, $schema)) {
                    // assign default value
                    if (\is_object($propertyDefinition->default)) {
                        \RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::propertySet($value, $currentProperty, clone $propertyDefinition->default);
                    } else {
                        \RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::propertySet($value, $currentProperty, $propertyDefinition->default);
                    }
                    $this->appliedDefaults[] = $currentProperty;
                }
            }
        } elseif (isset($schema->items) && \RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::isArray($value)) {
            $items = array();
            if (\RectorPrefix20210520\JsonSchema\Constraints\TypeCheck\LooseTypeCheck::isArray($schema->items)) {
                $items = $schema->items;
     ...
     * @param string      $i
     */
    protected function validateOfProperties(&$value, $schema, \RectorPrefix20210520\JsonSchema\Entity\JsonPointer $path, $i = '')
    {
        // Verify type
        if ($value instanceof self) {
            return;
        ...
        if (isset($schema->anyOf)) {
            ...
            foreach ($schema->anyOf as $anyOf) {
                ...
                } catch (\RectorPrefix20210520\JsonSchema\Exception\ValidationException $e) {
                    $isValid = \false;
                }
            }
            if (!$isValid) {
     ...
     * @param string      $i
     */
    protected function validateDependencies($value, $dependencies, \RectorPrefix20210520\JsonSchema\Entity\JsonPointer $path, $i = '')
    {
    ...
    {
        $resolver = new \RectorPrefix20210520\JsonSchema\Uri\UriResolver();
