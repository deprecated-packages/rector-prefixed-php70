...
use RectorPrefix20210522\JsonSchema\Exception\InvalidSchemaException;
use RectorPrefix20210522\JsonSchema\Exception\RuntimeException;
 ...
 */
class SchemaConstraint extends \RectorPrefix20210522\JsonSchema\Constraints\Constraint
{
    ...
    public function check(&$element, $schema = null, \RectorPrefix20210522\JsonSchema\Entity\JsonPointer $path = null, $i = null)
    {
        if ($schema !== null) {
            ...
            throw new \RectorPrefix20210522\JsonSchema\Exception\InvalidArgumentException('no schema found to verify against');
        }
        // cast array schemas to object
        if (\is_array($validationSchema)) {
            $validationSchema = \RectorPrefix20210522\JsonSchema\Constraints\BaseConstraint::arrayToObjectRecursive($validationSchema);
        }
        ...
        if ($this->factory->getConfig(self::CHECK_MODE_VALIDATE_SCHEMA)) {
            if (!$this->getTypeCheck()->isObject($validationSchema)) {
                throw new \RectorPrefix20210522\JsonSchema\Exception\RuntimeException('Cannot validate the schema of a non-object');
            }
            ...
            $this->factory->setErrorContext(\RectorPrefix20210522\JsonSchema\Validator::ERROR_SCHEMA_VALIDATION);
            // validate schema
            try {
                $this->check($validationSchema, $schemaSpec);
            } catch (\Exception $e) {
                if ($this->factory->getConfig(self::CHECK_MODE_EXCEPTIONS)) {
                    throw new \RectorPrefix20210522\JsonSchema\Exception\InvalidSchemaException('Schema did not pass validation', 0, $e);
                }
            }
