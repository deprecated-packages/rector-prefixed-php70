...
namespace RectorPrefix20210528\JsonSchema;

use RectorPrefix20210528\JsonSchema\Constraints\BaseConstraint;
use RectorPrefix20210528\JsonSchema\Entity\JsonPointer;
use RectorPrefix20210528\JsonSchema\Exception\UnresolvableJsonPointerException;
use RectorPrefix20210528\JsonSchema\Uri\UriResolver;
use RectorPrefix20210528\JsonSchema\Uri\UriRetriever;
class SchemaStorage implements \RectorPrefix20210528\JsonSchema\SchemaStorageInterface
{
    const INTERNAL_PROVIDED_SCHEMA_URI = 'internal://provided-schema/';
    protected $uriRetriever;
    protected $uriResolver;
    protected $schemas = array();
    public function __construct(\RectorPrefix20210528\JsonSchema\UriRetrieverInterface $uriRetriever = null, \RectorPrefix20210528\JsonSchema\UriResolverInterface $uriResolver = null)
    {
        $this->uriRetriever = $uriRetriever ?: new \RectorPrefix20210528\JsonSchema\Uri\UriRetriever();
        $this->uriResolver = $uriResolver ?: new \RectorPrefix20210528\JsonSchema\Uri\UriResolver();
    }
    /**
    ...
    public function addSchema($id, $schema = null)
    {
        ...
        if (\is_array($schema)) {
            $schema = \RectorPrefix20210528\JsonSchema\Constraints\BaseConstraint::arrayToObjectRecursive($schema);
        }
    ...
    private function expandRefs(&$schema, $base = null)
    {
        ...
        if (\property_exists($schema, '$ref') && \is_string($schema->{'$ref'})) {
            $refPointer = new \RectorPrefix20210528\JsonSchema\Entity\JsonPointer($this->uriResolver->resolve($schema->{'$ref'}, $base));
    ...
    {
        $jsonPointer = new \RectorPrefix20210528\JsonSchema\Entity\JsonPointer($ref);
        // resolve filename for pointer
        $fileName = $jsonPointer->getFilename();
        if (!\strlen($fileName)) {
            throw new \RectorPrefix20210528\JsonSchema\Exception\UnresolvableJsonPointerException(\sprintf("Could not resolve fragment '%s': no file is defined", $jsonPointer->getPropertyPathAsString()));
        }
        ...
        foreach ($jsonPointer->getPropertyPaths() as $path) {
            if (\is_object($refSchema) && \property_exists($refSchema, $path)) {
                ...
                throw new \RectorPrefix20210528\JsonSchema\Exception\UnresolvableJsonPointerException(\sprintf('File: %s is found, but could not resolve fragment: %s', $jsonPointer->getFilename(), $jsonPointer->getPropertyPathAsString()));
            }
        }
        return $refSchema;
    }
