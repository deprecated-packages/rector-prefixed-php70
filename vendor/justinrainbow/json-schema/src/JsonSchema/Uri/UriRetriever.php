 ...
 */
namespace RectorPrefix20210519\JsonSchema\Uri;

use RectorPrefix20210519\JsonSchema\Exception\InvalidSchemaMediaTypeException;
use RectorPrefix20210519\JsonSchema\Exception\JsonDecodingException;
use RectorPrefix20210519\JsonSchema\Exception\ResourceNotFoundException;
use RectorPrefix20210519\JsonSchema\Uri\Retrievers\FileGetContents;
use RectorPrefix20210519\JsonSchema\Uri\Retrievers\UriRetrieverInterface;
use RectorPrefix20210519\JsonSchema\UriRetrieverInterface as BaseUriRetrieverInterface;
 ...
 */
class UriRetriever implements \RectorPrefix20210519\JsonSchema\UriRetrieverInterface
{
    /**
    ...
    public function confirmMediaType($uriRetriever, $uri)
    {
        ...
        if (\in_array($contentType, array(\RectorPrefix20210519\JsonSchema\Validator::SCHEMA_MEDIA_TYPE, 'application/json'))) {
            return;
        }
        ...
        throw new \RectorPrefix20210519\JsonSchema\Exception\InvalidSchemaMediaTypeException(\sprintf('Media type %s expected', \RectorPrefix20210519\JsonSchema\Validator::SCHEMA_MEDIA_TYPE));
    }
    /**
     * Get a URI Retriever
     *
    ...
    public function getUriRetriever()
    {
        if (\is_null($this->uriRetriever)) {
            $this->setUriRetriever(new \RectorPrefix20210519\JsonSchema\Uri\Retrievers\FileGetContents());
        }
        ...
        while ($path) {
            ...
            if (!empty($pathElement)) {
                ...
                if (!empty($jsonSchema->{$pathElement})) {
                    ...
                    throw new \RectorPrefix20210519\JsonSchema\Exception\ResourceNotFoundException('Fragment "' . $parsed['fragment'] . '" not found' . ' in ' . $uri);
                }
                if (!\is_object($jsonSchema)) {
                    throw new \RectorPrefix20210519\JsonSchema\Exception\ResourceNotFoundException('Fragment part "' . $pathElement . '" is no object ' . ' in ' . $uri);
                }
            }
        }
        return $jsonSchema;
    ...
    protected function loadSchema($fetchUri)
    {
        ...
        if (\JSON_ERROR_NONE < ($error = \json_last_error())) {
            throw new \RectorPrefix20210519\JsonSchema\Exception\JsonDecodingException($error);
        }
     ...
     */
    public function setUriRetriever(\RectorPrefix20210519\JsonSchema\Uri\Retrievers\UriRetrieverInterface $uriRetriever)
    {
    ...
    public function resolve($uri, $baseUri = null)
    {
        ...
        $baseComponents['path'] = \RectorPrefix20210519\JsonSchema\Uri\UriResolver::combineRelativePathWithBasePath($path, $basePath);
