 ...
 */
namespace RectorPrefix20210528\Seld\JsonLint;
...
class JsonParser
{
    ...
    public function lint($input, $flags = 0)
    {
        ...
        } catch (\RectorPrefix20210528\Seld\JsonLint\ParsingException $e) {
            return $e;
        }
        return null;
    }
    ...
    public function parse($input, $flags = 0)
    {
        ...
        $TERROR = 2;
        $EOF = 1;
        $this->lexer = new \RectorPrefix20210528\Seld\JsonLint\Lexer();
        ...
        while (\true) {
            ...
            if (!$action || !$action[0]) {
                ...
                if ($recovering == 3) {
                    if ($symbol == $EOF) {
                        throw new \RectorPrefix20210528\Seld\JsonLint\ParsingException($errStr ?: 'Parsing halted.');
                    }
            ...
            if (\is_array($action[0]) && \count($action) > 1) {
                throw new \RectorPrefix20210528\Seld\JsonLint\ParsingException('Parse Error: multiple actions possible at state: ' . $state . ', token: ' . $symbol);
            }
            switch ($action[0]) {
                case 1:
                    // shift
                ...
                case 2:
                    ...
                    if (!$r instanceof \RectorPrefix20210528\Seld\JsonLint\Undefined) {
                        return $r;
                    }
                    if ($len) {
                        $this->popStack($len);
    ...
    {
        throw new \RectorPrefix20210528\Seld\JsonLint\ParsingException($str, $hash);
    }
    ...
    private function performAction(\stdClass $yyval, $yytext, $yyleng, $yylineno, $yystate, &$tokens)
    {
            ...
            case 17:
                if ($this->flags & self::PARSE_TO_ASSOC) {
                    ...
                    if ($this->flags & self::DETECT_KEY_CONFLICTS && isset($tokens[$len - 2][$key])) {
                        ...
                        throw new \RectorPrefix20210528\Seld\JsonLint\DuplicateKeyException($errStr, $tokens[$len][0], array('line' => $yylineno + 1));
        ...
        return new \RectorPrefix20210528\Seld\JsonLint\Undefined();
    }
