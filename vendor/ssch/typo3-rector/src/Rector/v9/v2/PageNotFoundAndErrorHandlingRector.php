...
final class PageNotFoundAndErrorHandlingRector extends \Rector\Core\Rector\AbstractRector
{
     ...
     */
    const MAP_METHODS = ['pageNotFoundAndExit' => 'pageNotFoundAction', 'pageUnavailableAndExit' => 'unavailableAction'];
    /**
     * @var string[]
     */
    const METHODS = ['pageNotFoundAndExit', 'pageUnavailableAndExit', 'checkPageUnavailableHandler', 'pageUnavailableHandler', 'pageNotFoundHandler', 'pageErrorHandler'];
    /**
...
CODE_SAMPLE
     ...
     * @return \PhpParser\Node|null
     */
    private function createResponse(\PhpParser\Node\Expr\MethodCall $node)
    {
     ...
     * @return \PhpParser\Node|null
     */
    private function refactorPageErrorHandlerIfPossible(\PhpParser\Node\Expr\MethodCall $node)
    {
        if (!isset($node->args[0])) {
            return null;
        }
     ...
     * @return void
     */
    private function removeNodeOrParentNode(\PhpParser\Node $node)
    {
        try {
