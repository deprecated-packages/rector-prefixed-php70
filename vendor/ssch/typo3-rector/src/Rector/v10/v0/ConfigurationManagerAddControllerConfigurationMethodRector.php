...
CODE_SAMPLE
     ...
     * @return void
     */
    private function addMethodGetControllerConfiguration(\PhpParser\Node\Stmt\Class_ $node)
    {
        $methodBuilder = new \RectorPrefix20210520\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder('getControllerConfiguration');
        $methodBuilder->makeProtected();
        $methodBuilder->addParams([(new \RectorPrefix20210520\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder('extensionName'))->getNode(), (new \RectorPrefix20210520\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder('pluginName'))->getNode()]);
