...
final class RefactorTCARector extends \Rector\Core\Rector\AbstractRector
{
     ...
     */
    const MAP_WIZARD_TO_FIELD_CONTROL = ['edit' => 'editPopup', 'add' => 'addRecord', 'list' => 'listModule', 'link' => 'linkPopup', 'RTE' => 'fullScreenRichtext'];
    /**
     * @var array<string, string>
     */
    const MAP_WIZARD_TO_RENDER_TYPE = ['table' => 'textTable', 'colorChoice' => 'colorpicker', 'link' => 'inputLink'];
    /**
     * @var array<string, string>
     */
    const MAP_WIZARD_TO_CUSTOM_TYPE = ['select' => 'valuePicker', 'suggest' => 'suggestOptions', 'angle' => 'slider'];
    /**
     * @codeCoverageIgnore
     */
...
CODE_SAMPLE
     ...
     * @return void
     */
    private function addFieldControlInsteadOfWizardsAddListEdit(\PhpParser\Node\Expr\Array_ $items)
    {
            ...
            foreach ($fieldValueArray->items as $configValue) {
                ...
                foreach ($configValueArray->items as $configItemValue) {
                    ...
                    foreach ($configItemValue->value->items as $wizardItemValue) {
                        ...
                        if ($validWizard || \RectorPrefix20210525\Nette\Utils\Strings::startsWith($this->valueResolver->getValue($wizardItemValueKey), '_')) {
                            --$remainingWizards;
                        }
                        if (!$validWizard) {
                            continue;
     ...
     * @return void
     */
    private function refactorRenderTypeInputDateTime(\PhpParser\Node\Expr\ArrayItem $configValue)
    {
        if (!$configValue->value instanceof \PhpParser\Node\Expr\Array_) {
            return;
        }
