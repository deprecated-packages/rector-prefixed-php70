...
class Helpers
{
    ...
    {
        $file = \strtr($origFile = $file, \RectorPrefix20210620\Tracy\Debugger::$editorMapping);
    ...
    {
        if (\RectorPrefix20210620\Tracy\Debugger::$editor && $file && ($action === 'create' || \is_file($file))) {
            $file = \strtr($file, '/', \DIRECTORY_SEPARATOR);
            $file = \strtr($file, \RectorPrefix20210620\Tracy\Debugger::$editorMapping);
            return \strtr(\RectorPrefix20210620\Tracy\Debugger::$editor, ['%action' => $action, '%file' => \rawurlencode($file), '%line' => $line ?: 1, '%search' => \rawurlencode($search), '%replace' => \rawurlencode($replace)]);
        }
        return null;
    }
    ...
    public static function improveException(\Throwable $e)
    {
        ...
        if (!$e instanceof \Error && !$e instanceof \ErrorException || $e instanceof \RectorPrefix20210620\Nette\MemberAccessException || \strpos($e->getMessage(), 'did you mean')) {
            // do nothing
