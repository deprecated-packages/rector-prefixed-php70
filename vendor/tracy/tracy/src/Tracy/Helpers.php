...
class Helpers
{
    ...
    {
        $file = \strtr($origFile = $file, \RectorPrefix20210504\Tracy\Debugger::$editorMapping);
    ...
    {
        if (\RectorPrefix20210504\Tracy\Debugger::$editor && $file && ($action === 'create' || \is_file($file))) {
            $file = \strtr($file, '/', \DIRECTORY_SEPARATOR);
            $file = \strtr($file, \RectorPrefix20210504\Tracy\Debugger::$editorMapping);
            return \strtr(\RectorPrefix20210504\Tracy\Debugger::$editor, ['%action' => $action, '%file' => \rawurlencode($file), '%line' => $line ?: 1, '%search' => \rawurlencode($search), '%replace' => \rawurlencode($replace)]);
        }
        return null;
    }
    ...
    public static function improveException(\Throwable $e)
    {
        ...
        if (!$e instanceof \Error && !$e instanceof \ErrorException || $e instanceof \RectorPrefix20210504\Nette\MemberAccessException || \strpos($e->getMessage(), 'did you mean')) {
            // do nothing
    ...
    public static function encodeString(string $s, int $maxLength = null, &$utf = null) : string
    {
        ...
        list($utf, $table, $len) = \preg_match('##u', $s) ? [\true, $tableU, \strlen(\utf8_decode($s))] : [\false, $tableB, \strlen($s)];
