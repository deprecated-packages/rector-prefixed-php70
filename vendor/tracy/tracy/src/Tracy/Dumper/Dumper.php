...
use RectorPrefix20210504\Tracy\Dumper\Describer;
use RectorPrefix20210504\Tracy\Dumper\Exposer;
use RectorPrefix20210504\Tracy\Dumper\Renderer;
/**
 * Dumps a variable.
 */
class Dumper
{
    ...
    // deprecated
    const HIDDEN_VALUE = \RectorPrefix20210504\Tracy\Dumper\Describer::HIDDEN_VALUE;
    /** @var Dumper\Value[] */
    ...
    /** @var array */
    public static $objectExporters = [\Closure::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeClosure'], \ArrayObject::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeArrayObject'], \SplFileInfo::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeSplFileInfo'], \SplObjectStorage::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeSplObjectStorage'], \__PHP_Incomplete_Class::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposePhpIncompleteClass'], \DOMNode::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeDOMNode'], \DOMNodeList::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeDOMNodeList'], \DOMNamedNodeMap::class => [\RectorPrefix20210504\Tracy\Dumper\Exposer::class, 'exposeDOMNodeList']];
    /** @var Describer */
    private $describer;
    /** @var Renderer */
    private $renderer;
    ...
    public static function dump($var, array $options = [])
    {
        if (\PHP_SAPI === 'cli' || \PHP_SAPI === 'phpdbg') {
            $useColors = self::$terminalColors && \RectorPrefix20210504\Tracy\Helpers::detectColors();
    ...
    {
        static $sent;
        if (\RectorPrefix20210504\Tracy\Debugger::$productionMode === \true || $sent) {
            return;
        }
        $sent = \true;
        $nonce = \RectorPrefix20210504\Tracy\Helpers::getNonce();
        $nonceAttr = $nonce ? ' nonce="' . \RectorPrefix20210504\Tracy\Helpers::escapeHtml($nonce) . '"' : '';
        ...
        echo "<style{$nonceAttr}>", \str_replace('</', '<\\/', \RectorPrefix20210504\Tracy\Helpers::minifyCss($s)), "</style>\n";
        if (!\RectorPrefix20210504\Tracy\Debugger::isEnabled()) {
            $s = '(function(){' . \file_get_contents(__DIR__ . '/../Toggle/toggle.js') . '})();' . '(function(){' . \file_get_contents(__DIR__ . '/../Dumper/assets/dumper.js') . '})();';
            echo "<script{$nonceAttr}>", \str_replace(['<!--', '</s'], ['<\\!--', '<\\/s'], \RectorPrefix20210504\Tracy\Helpers::minifyJs($s)), "</script>\n";
        }
    }
    private function __construct(array $options = [])
    {
        ...
        $describer = $this->describer = new \RectorPrefix20210504\Tracy\Dumper\Describer();
        ...
        $renderer = $this->renderer = new \RectorPrefix20210504\Tracy\Dumper\Renderer();
    ...
    {
        $res = "'" . \RectorPrefix20210504\Tracy\Dumper\Renderer::jsonEncode($snapshot[0] ?? []) . "'";
        $snapshot = [];
        return $res;
    }
}
