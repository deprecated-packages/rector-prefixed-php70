...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Psr\Log\LogLevel::WARNING);
    }
    ...
    {
        $this->output($this->getLoadedMessage());
        \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    private function reportRestarted()
    {
        $loaded = $this->getLoadedMessage();
        $text = \sprintf('Restarted (%d ms). %s', $this->time, $loaded);
        $level = $this->loaded ? \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Psr\Log\LogLevel::WARNING : null;
