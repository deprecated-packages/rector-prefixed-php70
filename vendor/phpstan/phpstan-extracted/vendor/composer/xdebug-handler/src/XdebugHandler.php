 ...
 */
namespace RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler;

use RectorPrefix20210620\_HumbugBox15516bb2b566\Psr\Log\LoggerInterface;
/**
...
class XdebugHandler
{
    ...
    public function __construct($envPrefix)
    {
        ...
        $this->statusWriter = new \RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
     ...
     * @return $this
     */
    public function setLogger(\RectorPrefix20210620\_HumbugBox15516bb2b566\Psr\Log\LoggerInterface $logger)
    {
    ...
    {
        $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::CHECK, $this->loaded . '|' . $this->mode);
        $envArgs = \explode('|', (string) \getenv($this->envAllowXdebug));
        if (empty($envArgs[0]) && $this->requiresRestart(self::$xdebugActive)) {
            // Restart required
            $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && \count($envArgs) === 5) {
            ...
            $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::RESTARTED);
            \RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
            if (!$this->loaded) {
        ...
        $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::NORESTART);
    ...
    {
        $this->tryEnableSignals();
        $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::RESTARTING, \implode(' ', $command));
        if (\PHP_VERSION_ID >= 70400) {
            $cmd = $command;
        } else {
            $cmd = \RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Process::escapeShellCommand($command);
        ...
        if (!isset($exitCode)) {
            ...
            $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::ERROR, 'Unable to restart process');
            $exitCode = -1;
        } else {
            $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::INFO, 'Restarted process exited ' . $exitCode);
        }
        if ($this->debug === '2') {
            $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::INFO, 'Temp ini saved: ' . $this->tmpIni);
        } else {
            @\unlink($this->tmpIni);
        }
        exit($exitCode);
    ...
    private function prepareRestart()
    {
        ...
        if ($error) {
            $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return empty($error);
    }
    /**
    ...
    private function setEnvRestartSettings($envArgs)
    {
        ...
        \RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, \implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings)
    {
        if (\false === \getenv($this->envOriginalInis)) {
            ...
            \RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, \implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\RectorPrefix20210620\_HumbugBox15516bb2b566\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
