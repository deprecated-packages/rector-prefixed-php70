...
namespace RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Http\Middleware;
...
use RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Http\Io\HttpBodyStream;
use RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Http\Io\PauseBufferStream;
...
final class LimitConcurrentRequestsMiddleware
{
    ...
    public function __invoke(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ServerRequestInterface $request, $next)
    {
        ...
        if ($this->pending < $this->limit) {
            ...
            return $this->await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\resolve($response));
        }
        ...
        if ($body instanceof \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Stream\ReadableStreamInterface) {
            ...
            $size = $body->getSize();
            $body = new \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Http\Io\PauseBufferStream($body);
            $body->pauseImplicit();
            // replace with buffering body to ensure any readable events will be buffered
            $request = $request->withBody(new \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Http\Io\HttpBodyStream($body, $size));
        }
        ...
        \end($queue);
        $id = \key($queue);
        $deferred = new \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\Deferred(function ($_, $reject) use(&$queue, $id) {
        ...
        return $deferred->promise()->then(function () use($request, $next, $body, &$pending, $that) {
            ...
            if ($body instanceof \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Http\Io\PauseBufferStream) {
                $body->resumeImplicit();
            }
            // if the next handler returns a pending promise, we have to
            // await its resolution before invoking next queued request
            return $that->await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\resolve($response));
        });
    }
    /**
     * @internal
     ...
     */
    public function await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\PromiseInterface $promise)
    {
        $that = $this;
        return $promise->then(function ($response) use($that) {
            ...
            $that->processQueue();
            return \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\reject($error);
        });
    }
    /**
     * @internal
