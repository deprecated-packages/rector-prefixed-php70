 ...
 * @internal
 */
class RequestHeaderParser extends \RectorPrefix20210616\_HumbugBox15516bb2b566\Evenement\EventEmitter
{
    private $maxSize = 8192;
    public function handle(\RectorPrefix20210616\_HumbugBox15516bb2b566\React\Socket\ConnectionInterface $conn)
    {
        $buffer = '';
        ...
        $conn->on('data', $fn = function ($data) use(&$buffer, &$fn, $conn, $maxSize, $that) {
            ...
            if ($contentLength === 0) {
                ...
                $stream = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Io\EmptyBodyStream();
                $request = $request->withBody($stream);
            } else {
                // otherwise body is present => delimit using Content-Length or ChunkedDecoder
                $stream = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Io\CloseProtectionStream($conn);
                if ($contentLength !== null) {
                    $stream = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Io\LengthLimitedStream($stream, $contentLength);
                } else {
                    $stream = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Io\ChunkedDecoder($stream);
                }
                $request = $request->withBody(new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Io\HttpBodyStream($stream, $contentLength));
            }
    ...
    public function parseRequest($headers, $remoteSocketUri, $localSocketUri)
    {
        ...
        $request = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Message\ServerRequest($start['method'], $uri, $fields, '', $start['version'], $serverParams);
