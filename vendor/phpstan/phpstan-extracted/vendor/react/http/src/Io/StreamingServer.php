...
use RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Message\Response;
use RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Message\ServerRequest;
...
use RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ServerInterface;
 ...
 * @internal
 */
final class StreamingServer extends \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Evenement\EventEmitter
{
    private $callback;
    private $parser;
    private $loop;
     ...
     * @see self::listen()
     */
    public function __construct(\RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop, $requestHandler)
    {
        ...
        $this->parser = new \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Io\RequestHeaderParser();
        $that = $this;
        $this->parser->on('headers', function (\RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ServerRequestInterface $request, \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ConnectionInterface $conn) use($that) {
            $that->handleRequest($conn, $request);
        });
        $this->parser->on('error', function (\Exception $e, \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ConnectionInterface $conn) use($that) {
            $that->emit('error', array($e));
            // parsing failed => assume dummy request and send appropriate error
            $that->writeError($conn, $e->getCode() !== 0 ? $e->getCode() : 400, new \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Message\ServerRequest('GET', '/'));
        });
    }
    /**
     ...
     */
    public function listen(\RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ServerInterface $socket)
    {
        $socket->on('connection', array($this->parser, 'handle'));
    }
    /** @internal */
    public function handleRequest(\RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ConnectionInterface $conn, \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        try {
            ...
            $response = \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Promise\reject($error);
        ...
        if ($response instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Promise\CancellablePromiseInterface) {
            $conn->on('close', function () use($response) {
                $response->cancel();
            });
        }
        // happy path: response returned, handle and return immediately
        if ($response instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ResponseInterface) {
        ...
        if (!$response instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Promise\PromiseInterface) {
            $response = \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Promise\resolve($response);
        }
        $that = $this;
    ...
    /** @internal */
    public function writeError(\RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ConnectionInterface $conn, $code, \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ServerRequestInterface $request)
    {
        $response = new \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Message\Response($code, array('Content-Type' => 'text/plain'), 'Error ' . $code);
    ...
    /** @internal */
    public function handleResponse(\RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Socket\ConnectionInterface $connection, \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ServerRequestInterface $request, \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if (($code === 101 || $method === 'CONNECT' && $code >= 200 && $code < 300) && $body instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Io\HttpBodyStream && $body->input instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Stream\WritableStreamInterface) {
        ...
        if (!$body instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Stream\ReadableStreamInterface || !$body->isReadable()) {
            ...
            if ($body instanceof \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Stream\ReadableStreamInterface && $chunked) {
                $body = "0\r\n\r\n";
            }
        ...
        $connection->write($headers . "\r\n");
        if ($chunked) {
            $body = new \RectorPrefix20210527\_HumbugBox0b2f2d5c77b8\React\Http\Io\ChunkedEncoder($body);
        }
