...
use RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Resolver\ResolverInterface;
...
final class HappyEyeBallsConnectionBuilder
{
    ...
    public $uri;
    public $host;
    public $resolved = array(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_A => \false, \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA => \false);
    ...
    public $lastError6;
    public $lastError4;
    public function __construct(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop, \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Socket\ConnectorInterface $connector, \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Resolver\ResolverInterface $resolver, $uri, $host, $parts)
    {
        $this->loop = $loop;
    ...
    public function connect()
    {
        $timer = null;
        $that = $this;
        return new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Promise\Promise(function ($resolve, $reject) use($that, &$timer) {
            ...
            $that->resolverPromises[\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA] = $that->resolve(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA, $reject)->then($lookupResolve(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA));
            $that->resolverPromises[\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_A] = $that->resolve(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_A, $reject)->then(function (array $ips) use($that, &$timer) {
                // happy path: IPv6 has resolved already, continue with IPv4 addresses
                if ($that->resolved[\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA] === \true) {
                    return $ips;
                }
                // Otherwise delay processing IPv4 lookup until short timer passes or IPv6 resolves in the meantime
                $deferred = new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Promise\Deferred();
                $timer = $that->loop->addTimer($that::RESOLUTION_DELAY, function () use($deferred, $ips) {
                    $deferred->resolve($ips);
                });
                $that->resolverPromises[\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA]->then(function () use($that, $timer, $deferred, $ips) {
                    $that->loop->cancelTimer($timer);
                    $deferred->resolve($ips);
                });
                return $deferred->promise();
            })->then($lookupResolve(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_A));
            ...
            $_ = $reject = null;
            $that->cleanUp();
            if ($timer instanceof \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\EventLoop\TimerInterface) {
                $that->loop->cancelTimer($timer);
            }
        });
    }
    ...
    public function resolve($type, $reject)
    {
        ...
        return $that->resolver->resolveAll($that->host, $type)->then(null, function (\Exception $e) use($type, $reject, $that) {
            unset($that->resolverPromises[$type]);
            $that->resolved[$type] = \true;
            if ($type === \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_A) {
    ...
    public function check($resolve, $reject)
    {
        ...
        if ($this->nextAttemptTimer === null && (\count($this->connectQueue) > 0 || $this->resolved[\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_A] === \false || $this->resolved[\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::TYPE_AAAA] === \false)) {
    ...
    public function cleanUp()
    {
        ...
        foreach ($this->connectionPromises as $connectionPromise) {
            if ($connectionPromise instanceof \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Promise\CancellablePromiseInterface) {
                $connectionPromise->cancel();
            }
        }
        foreach ($this->resolverPromises as $resolverPromise) {
            if ($resolverPromise instanceof \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Promise\CancellablePromiseInterface) {
                $resolverPromise->cancel();
            }
        }
        if ($this->nextAttemptTimer instanceof \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\EventLoop\TimerInterface) {
