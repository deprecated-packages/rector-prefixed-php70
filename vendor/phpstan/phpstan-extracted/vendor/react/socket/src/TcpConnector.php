...
final class TcpConnector implements \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Socket\ConnectorInterface
{
    private $loop;
    private $context;
    public function __construct(\RectorPrefix20210616\_HumbugBox15516bb2b566\React\EventLoop\LoopInterface $loop, array $context = array())
    {
        $this->loop = $loop;
    ...
    public function connect($uri)
    {
        ...
        if (\false === \filter_var($ip, \FILTER_VALIDATE_IP)) {
            return \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $ip . '" does not contain a valid host IP'));
        }
        ...
        if (\false === $stream) {
            return \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\reject(new \RuntimeException(\sprintf("Connection to %s failed: %s", $uri, $errstr), $errno));
        }
        // wait for connection
        $loop = $this->loop;
        return new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\Promise(function ($resolve, $reject) use($loop, $stream, $uri) {
            $loop->addWriteStream($stream, function ($stream) use($loop, $resolve, $reject, $uri) {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    $resolve(new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Socket\Connection($stream, $loop));
                }
            });
