 ...
 */
final class UdpTransportExecutor implements \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Query\Query $query)
    {
        ...
        if (isset($queryData[512])) {
            return \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        $loop = $this->loop;
        $deferred = new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Promise\Deferred(function () use($loop, $socket, $query) {
            // cancellation should remove socket from loop and close socket
            $loop->removeReadStream($socket);
            \fclose($socket);
            throw new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\React\Dns\Query\CancellationException('DNS query for ' . $query->name . ' has been cancelled');
        });
        $parser = $this->parser;
