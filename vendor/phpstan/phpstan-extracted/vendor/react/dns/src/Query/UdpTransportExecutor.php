...
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Protocol\BinaryDumper;
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Protocol\Parser;
 ...
 */
final class UdpTransportExecutor implements \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
     ...
     */
    public function __construct($nameserver, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop)
    {
        ...
        $this->loop = $loop;
        $this->parser = new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Protocol\Parser();
        $this->dumper = new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Protocol\BinaryDumper();
    }
    public function query(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Query\Query $query)
    {
        $request = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Model\Message::createRequestForQuery($query);
        $queryData = $this->dumper->toBinary($request);
        if (isset($queryData[512])) {
            return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Unable to connect to DNS server (' . $errstr . ')', $errno));
        }
        // set socket to non-blocking and immediately try to send (fill write buffer)
        \stream_set_blocking($socket, \false);
        \fwrite($socket, $queryData);
        $loop = $this->loop;
        $deferred = new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Promise\Deferred(function () use($loop, $socket, $query) {
            // cancellation should remove socket from loop and close socket
            $loop->removeReadStream($socket);
            \fclose($socket);
            throw new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\React\Dns\Query\CancellationException('DNS query for ' . $query->name . ' has been cancelled');
        });
        $parser = $this->parser;
