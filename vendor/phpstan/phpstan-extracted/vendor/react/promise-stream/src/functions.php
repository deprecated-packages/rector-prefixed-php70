...
namespace RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\Stream;
 ...
 */
function buffer(\RectorPrefix20210620\_HumbugBox15516bb2b566\React\Stream\ReadableStreamInterface $stream, $maxLength = null)
{
    // stream already ended => resolve with empty buffer
    if (!$stream->isReadable()) {
        return \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\resolve('');
    }
    $buffer = '';
    $promise = new \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\Promise(function ($resolve, $reject) use($stream, $maxLength, &$buffer, &$bufferer) {
 ...
 */
function first(\RectorPrefix20210620\_HumbugBox15516bb2b566\Evenement\EventEmitterInterface $stream, $event = 'data')
{
    if ($stream instanceof \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Stream\ReadableStreamInterface) {
        ...
        if (!$stream->isReadable()) {
            return \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\reject(new \RuntimeException('Stream already closed'));
        }
    } elseif ($stream instanceof \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Stream\WritableStreamInterface) {
        // writable-only stream (not duplex) not writable => already closed
        if (!$stream->isWritable()) {
            return \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\reject(new \RuntimeException('Stream already closed'));
        }
    }
    return new \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\Promise(function ($resolve, $reject) use($stream, $event, &$listener) {
 ...
 */
function all(\RectorPrefix20210620\_HumbugBox15516bb2b566\Evenement\EventEmitterInterface $stream, $event = 'data')
{
    // stream already ended => resolve with empty buffer
    if ($stream instanceof \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Stream\ReadableStreamInterface) {
        // readable or duplex stream not readable => already closed
        // a half-open duplex stream is considered closed if its readable side is closed
        if (!$stream->isReadable()) {
            return \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\resolve(array());
        }
    } elseif ($stream instanceof \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Stream\WritableStreamInterface) {
        // writable-only stream (not duplex) not writable => already closed
        if (!$stream->isWritable()) {
    ...
    $promise = new \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\Promise(function ($resolve, $reject) use($stream, &$buffer) {
        $stream->on('error', function ($error) use($reject) {
            $reject(new \RuntimeException('An error occured on the underlying stream while buffering', 0, $error));
        });
        $stream->on('close', function () use($resolve, &$buffer) {
 ...
 */
function unwrapReadable(\RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\PromiseInterface $promise)
{
    return new \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\Stream\UnwrapReadableStream($promise);
}
/**
 ...
 */
function unwrapWritable(\RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\PromiseInterface $promise)
{
    return new \RectorPrefix20210620\_HumbugBox15516bb2b566\React\Promise\Stream\UnwrapWritableStream($promise);
}
