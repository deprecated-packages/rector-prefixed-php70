...
{
    const ANONYMOUS_CLASS_NAME_PREFIX = 'class@anonymous';
    /** @var Reflector */
    private $reflector;
    ...
    {
        return \PHPStan\BetterReflection\Reflection\StringCast\ReflectionClassStringCast::toString($this);
    }
    /**
    ...
    {
        return (new \PHPStan\BetterReflection\BetterReflection())->classReflector()->reflect($className);
    }
    /**
     ...
     * @return $this
     */
    public static function createFromNode(\PHPStan\BetterReflection\Reflector\Reflector $reflector, \PhpParser\Node\Stmt\ClassLike $node, \PHPStan\BetterReflection\SourceLocator\Located\LocatedSource $locatedSource, $namespace = null)
    {
        $class = new self();
    ...
    private function createMethodsFromTrait(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method) : array
    {
        ...
        $createMethod = function ($aliasMethodName) use($method, $methodAst) : ReflectionMethod {
            return \PHPStan\BetterReflection\Reflection\ReflectionMethod::createFromNode($this->reflector, $methodAst, $method->getDeclaringClass()->getDeclaringNamespaceAst(), $method->getDeclaringClass(), $this, $aliasMethodName);
        };
        $methods = [];
    ...
    {
        return \array_merge([], ...\array_map(static function (\PHPStan\BetterReflection\Reflection\ReflectionClass $ancestor) : array {
    ...
    {
        return \array_merge([], ...\array_map(function (\PHPStan\BetterReflection\Reflection\ReflectionClass $trait) : array {
            return \array_merge([], ...\array_map(function (\PHPStan\BetterReflection\Reflection\ReflectionMethod $method) : array {
    ...
    public function getMethods($filter = null) : array
    {
        ...
        return \array_values(\array_filter($this->getMethodsIndexedByName(), static function (\PHPStan\BetterReflection\Reflection\ReflectionMethod $method) use($filter) : bool {
    ...
    public function getImmediateMethods($filter = null) : array
    {
        ...
        $methods = \array_map(function (\PhpParser\Node\Stmt\ClassMethod $methodNode) : ReflectionMethod {
            return \PHPStan\BetterReflection\Reflection\ReflectionMethod::createFromNode($this->reflector, $methodNode, $this->declaringNamespace, $this, $this);
    ...
    {
        return \array_map(static function (\PHPStan\BetterReflection\Reflection\ReflectionClassConstant $classConstant) {
    ...
    public function getImmediateReflectionConstants() : array
    {
        ...
        $constants = \array_merge([], ...\array_map(function (\PhpParser\Node\Stmt\ClassConst $constNode) : array {
            ...
            foreach ($constNode->consts as $constantPositionInNode => $constantNode) {
                $constants[] = \PHPStan\BetterReflection\Reflection\ReflectionClassConstant::createFromNode($this->reflector, $constNode, $constantPositionInNode, $this);
            }
            return $constants;
        ...
        return $this->cachedReflectionConstants = \array_combine(\array_map(static function (\PHPStan\BetterReflection\Reflection\ReflectionClassConstant $constant) : string {
    ...
    public function getReflectionConstants() : array
    {
        ...
        $allReflectionConstants = \array_merge(\array_values($this->getImmediateReflectionConstants()), ...\array_map(static function (\PHPStan\BetterReflection\Reflection\ReflectionClass $ancestor) : array {
            return \array_filter(\array_values($ancestor->getReflectionConstants()), static function (\PHPStan\BetterReflection\Reflection\ReflectionClassConstant $classConstant) : bool {
                return !$classConstant->isPrivate();
            });
        }, \array_filter([$this->getParentClass()])), ...\array_map(static function (\PHPStan\BetterReflection\Reflection\ReflectionClass $interface) : array {
    ...
    public function getImmediateProperties($filter = null) : array
    {
            ...
            foreach ($this->node->stmts as $stmt) {
                if ($stmt instanceof \PhpParser\Node\Stmt\ClassMethod && $stmt->name->toLowerString() === '__construct') {
                    foreach ($stmt->params as $param) {
                        ...
                        $prop = \PHPStan\BetterReflection\Reflection\ReflectionProperty::createFromNode($this->reflector, new \PhpParser\Node\Stmt\Property($param->flags, [new \PhpParser\Node\Stmt\PropertyProperty(new \PhpParser\Node\VarLikeIdentifier($param->var->name))], $param->getAttributes(), $param->type, $param->attrGroups), 0, $this, $this, \true, \true);
                ...
                foreach ($stmt->props as $propertyPositionInNode => $propertyNode) {
                    $prop = \PHPStan\BetterReflection\Reflection\ReflectionProperty::createFromNode($this->reflector, $stmt, $propertyPositionInNode, $this, $this);
                    $properties[$prop->getName()] = $prop;
                }
            }
        ...
        return \array_filter($this->cachedImmediateProperties, static function (\PHPStan\BetterReflection\Reflection\ReflectionProperty $property) use($filter) : bool {
    ...
    public function getProperties($filter = null) : array
    {
        if ($this->cachedProperties === null) {
            ...
            $this->cachedProperties = \array_merge(\array_merge([], ...\array_map(static function (\PHPStan\BetterReflection\Reflection\ReflectionClass $ancestor) use($filter) : array {
                return \array_filter($ancestor->getProperties($filter), static function (\PHPStan\BetterReflection\Reflection\ReflectionProperty $property) : bool {
                    return !$property->isPrivate();
                });
            }, \array_filter([$this->getParentClass()])), ...\array_map(function (\PHPStan\BetterReflection\Reflection\ReflectionClass $trait) use($filter) {
                return \array_map(function (\PHPStan\BetterReflection\Reflection\ReflectionProperty $property) : ReflectionProperty {
                    return \PHPStan\BetterReflection\Reflection\ReflectionProperty::createFromNode($this->reflector, $property->getAst(), $property->getPositionInAst(), $property->getDeclaringClass(), $this, $property->isDefault(), $property->isPromoted());
        ...
        return \array_filter($this->cachedProperties, static function (\PHPStan\BetterReflection\Reflection\ReflectionProperty $property) use($filter) : bool {
            return (bool) ($filter & $property->getModifiers());
        });
    }
    /**
    ...
    public function getParentClass()
    {
        ...
        if ($this->cachedParentClass->isInterface() || $this->cachedParentClass->isTrait()) {
            throw \PHPStan\BetterReflection\Reflection\Exception\NotAClassReflection::fromReflectionClass($this->cachedParentClass);
        }
     ...
     * @return $this
     */
    private function reflectClassForNamedNode(\PhpParser\Node\Name $node)
    {
        // @TODO use actual `ClassReflector` or `FunctionReflector`?
        if ($this->isAnonymous()) {
            $class = (new \PHPStan\BetterReflection\BetterReflection())->classReflector()->reflect($node->toString());
        } else {
     ...
     * @return void
     */
    private function parseTraitUsages()
    {
    ...
    private function getCurrentClassImplementedInterfacesIndexedByName() : array
    {
        ...
        if ($node instanceof \PhpParser\Node\Stmt\Class_) {
            ...
            if (\PHPStan\BetterReflection\BetterReflection::$phpVersion >= 80000) {
    ...
    {
        if (!$this->isInterface()) {
            throw \PHPStan\BetterReflection\Reflection\Exception\NotAnInterfaceReflection::fromReflectionClass($this);
        }
        $node = $this->node;
    ...
    {
        throw \PHPStan\BetterReflection\Reflection\Exception\Uncloneable::fromClass(static::class);
    }
    /**
    ...
    public function getStaticPropertyValue(string $propertyName)
    {
        ...
        if (!$property || !$property->isStatic()) {
            throw \PHPStan\BetterReflection\Reflection\Exception\PropertyDoesNotExist::fromName($propertyName);
        }
    ...
    public function setFinal(bool $isFinal)
    {
        if (!$this->node instanceof \PhpParser\Node\Stmt\Class_) {
            throw \PHPStan\BetterReflection\Reflection\Exception\NotAClassReflection::fromReflectionClass($this);
        }
        if ($isFinal === \true) {
     ...
     * @return void
     */
    public function addProperty(string $propertyName, int $visibility = \ReflectionProperty::IS_PUBLIC, bool $static = \false)
    {
        $type = 0;
        switch ($visibility) {
    ...
    {
        $attributes = [];
        $compileNodeToValue = new \PHPStan\BetterReflection\NodeCompiler\CompileNodeToValue();
        foreach ($this->node->attrGroups as $attrGroup) {
            foreach ($attrGroup->attrs as $attr) {
                ...
                foreach ($attr->args as $i => $arg) {
                    ...
                    $arguments[$key] = $compileNodeToValue->__invoke($arg->value, new \PHPStan\BetterReflection\NodeCompiler\CompilerContext($this->reflector, $this->getFileName(), $this, $this->declaringNamespace !== null && $this->declaringNamespace->name !== null ? $this->declaringNamespace->name->toString() : null, null));
                }
                $attributes[] = new \PHPStan\BetterReflection\Reflection\ReflectionAttribute($attr->name->toString(), $arguments);
            }
        }
        return $attributes;
    }
