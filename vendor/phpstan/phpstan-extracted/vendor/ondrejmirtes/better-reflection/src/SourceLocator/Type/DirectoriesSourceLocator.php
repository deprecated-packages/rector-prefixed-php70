...
class DirectoriesSourceLocator implements \PHPStan\BetterReflection\SourceLocator\Type\SourceLocator
{
     ...
     */
    public function __construct(array $directories, \PHPStan\BetterReflection\SourceLocator\Ast\Locator $astLocator)
    {
        $this->aggregateSourceLocator = new \PHPStan\BetterReflection\SourceLocator\Type\AggregateSourceLocator(\array_values(\array_map(static function ($directory) use($astLocator) : FileIteratorSourceLocator {
            if (!\is_string($directory)) {
                throw \PHPStan\BetterReflection\SourceLocator\Exception\InvalidDirectory::fromNonStringValue($directory);
            }
            if (!\is_dir($directory)) {
                throw \PHPStan\BetterReflection\SourceLocator\Exception\InvalidDirectory::fromNonDirectory($directory);
            }
