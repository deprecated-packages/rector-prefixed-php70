...
final class MakeLocatorForComposerJsonAndInstalledJson
{
    public function __invoke(string $installationPath, \PHPStan\BetterReflection\SourceLocator\Ast\Locator $astLocator) : \PHPStan\BetterReflection\SourceLocator\Type\SourceLocator
    {
        ...
        if (!\file_exists($composerJsonPath)) {
            throw \PHPStan\BetterReflection\SourceLocator\Type\Composer\Factory\Exception\MissingComposerJson::inProjectPath($installationPath);
        }
        ...
        if (!\is_array($composer)) {
            throw \PHPStan\BetterReflection\SourceLocator\Type\Composer\Factory\Exception\FailedToParseJson::inFile($composerJsonPath);
        }
        ...
        return new \PHPStan\BetterReflection\SourceLocator\Type\AggregateSourceLocator(\array_merge([new \PHPStan\BetterReflection\SourceLocator\Type\Composer\PsrAutoloaderLocator(\PHPStan\BetterReflection\SourceLocator\Type\Composer\Psr\Psr4Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr4AutoloadNamespaces($composer), $realInstallationPath), ...\array_map(function (array $package) use($realInstallationPath) : array {
            return $this->prefixWithPackagePath($this->packageToPsr4AutoloadNamespaces($package), $realInstallationPath, $package);
        }, $installed))), $astLocator), new \PHPStan\BetterReflection\SourceLocator\Type\Composer\PsrAutoloaderLocator(\PHPStan\BetterReflection\SourceLocator\Type\Composer\Psr\Psr0Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr0AutoloadNamespaces($composer), $realInstallationPath), ...\array_map(function (array $package) use($realInstallationPath) : array {
