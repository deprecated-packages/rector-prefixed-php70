...
final class MakeLocatorForComposerJson
{
    public function __invoke(string $installationPath, \PHPStan\BetterReflection\SourceLocator\Ast\Locator $astLocator) : \PHPStan\BetterReflection\SourceLocator\Type\SourceLocator
    {
        ...
        return new \PHPStan\BetterReflection\SourceLocator\Type\AggregateSourceLocator(\array_merge([new \PHPStan\BetterReflection\SourceLocator\Type\Composer\PsrAutoloaderLocator(\PHPStan\BetterReflection\SourceLocator\Type\Composer\Psr\Psr4Mapping::fromArrayMappings($this->prefixWithInstallationPath($this->packageToPsr4AutoloadNamespaces($composer), $pathPrefix)), $astLocator), new \PHPStan\BetterReflection\SourceLocator\Type\Composer\PsrAutoloaderLocator(\PHPStan\BetterReflection\SourceLocator\Type\Composer\Psr\Psr0Mapping::fromArrayMappings($this->prefixWithInstallationPath($this->packageToPsr0AutoloadNamespaces($composer), $pathPrefix)), $astLocator), new \PHPStan\BetterReflection\SourceLocator\Type\DirectoriesSourceLocator($classMapDirectories, $astLocator)], ...\array_map(static function (string $file) use($astLocator) : array {
