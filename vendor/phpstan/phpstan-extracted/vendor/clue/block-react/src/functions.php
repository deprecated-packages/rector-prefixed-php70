...
namespace RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Clue\React\Block;
...
use RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\Timer;
use RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\Timer\TimeoutException;
/**
 ...
 * @return void
 */
function sleep($time, \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop)
{
    await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\Timer\resolve($time, $loop), $loop);
}
/**
 ...
 */
function await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\PromiseInterface $promise, \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop, $timeout = null)
{
    $wait = \true;
    $resolved = null;
    $exception = null;
    $rejected = \false;
    if ($timeout !== null) {
        $promise = \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\Timer\timeout($promise, $timeout, $loop);
    }
 ...
 * @throws TimeoutException if the $timeout is given and triggers
 */
function awaitAny(array $promises, \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop, $timeout = null)
{
    // Explicitly overwrite argument with null value. This ensure that this
    // argument does not show up in the stack trace in PHP 7+ only.
    $all = $promises;
    ...
    try {
        ...
        $ret = await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\any($all)->then(null, function () {
            // rejects with an array of rejection reasons => reject with Exception instead
            throw new \Exception('All promises rejected');
        }), $loop, $timeout);
    } catch (\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\Timer\TimeoutException $e) {
        // the timeout fired
 ...
 * @throws TimeoutException if the $timeout is given and triggers
 */
function awaitAll(array $promises, \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\EventLoop\LoopInterface $loop, $timeout = null)
{
    // Explicitly overwrite argument with null value. This ensure that this
    // argument does not show up in the stack trace in PHP 7+ only.
    $all = $promises;
    $promises = null;
    try {
        return await(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\all($all), $loop, $timeout);
    } catch (\Exception $e) {
...
function _cancelAllPromises(array $promises)
{
    foreach ($promises as $promise) {
        if ($promise instanceof \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\React\Promise\CancellablePromiseInterface) {
            $promise->cancel();
        }
    }
}
