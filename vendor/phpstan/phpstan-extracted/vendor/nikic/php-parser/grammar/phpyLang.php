...
{
    $code = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\resolveNodes($code);
    $code = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\resolveMacros($code);
    $code = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\resolveStackAccess($code);
    return $code;
}
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\resolveNodes($matches['params']);
        $params = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
