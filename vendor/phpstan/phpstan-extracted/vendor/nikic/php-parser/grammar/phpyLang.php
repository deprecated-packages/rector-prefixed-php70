...
{
    $code = \RectorPrefix20210616\_HumbugBox15516bb2b566\resolveNodes($code);
    $code = \RectorPrefix20210616\_HumbugBox15516bb2b566\resolveMacros($code);
    $code = \RectorPrefix20210616\_HumbugBox15516bb2b566\resolveStackAccess($code);
    return $code;
}
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \RectorPrefix20210616\_HumbugBox15516bb2b566\resolveNodes($matches['params']);
        $params = \RectorPrefix20210616\_HumbugBox15516bb2b566\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \RectorPrefix20210616\_HumbugBox15516bb2b566\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \RectorPrefix20210616\_HumbugBox15516bb2b566\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \RectorPrefix20210616\_HumbugBox15516bb2b566\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \RectorPrefix20210616\_HumbugBox15516bb2b566\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \RectorPrefix20210616\_HumbugBox15516bb2b566\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \RectorPrefix20210616\_HumbugBox15516bb2b566\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\RectorPrefix20210616\_HumbugBox15516bb2b566\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
