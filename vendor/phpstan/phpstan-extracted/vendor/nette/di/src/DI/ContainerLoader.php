...
class ContainerLoader
{
    ...
    private function loadFile(string $class, callable $generator)
    {
        ...
        \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\Utils\FileSystem::createDir($this->tempDirectory);
        $handle = @\fopen("{$file}.lock", 'c+');
        // @ is escalated to exception
        if (!$handle) {
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\IOException("Unable to create file '{$file}.lock'. " . \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\Utils\Helpers::getLastError());
        } elseif (!@\flock($handle, \LOCK_EX)) {
            // @ is escalated to exception
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\IOException("Unable to acquire exclusive lock on '{$file}.lock'. " . \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\Utils\Helpers::getLastError());
        }
        if (!\is_file($file) || $this->isExpired($file, $updatedMeta)) {
            ...
            foreach ($toWrite as $name => $content) {
                if (\file_put_contents("{$name}.tmp", $content) !== \strlen($content) || !\rename("{$name}.tmp", $name)) {
                    @\unlink("{$name}.tmp");
                    // @ - file may not exist
                    throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\IOException("Unable to create file '{$name}'.");
        ...
        if (@(include $file) === \false) {
            ...
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\IOException("Unable to include '{$file}'.");
        }
    ...
    private function isExpired(string $file, string &$updatedMeta = null) : bool
    {
        if ($this->autoRebuild) {
            ...
            // @ - file may not exist
            $orig = $meta[2] ?? null;
            return empty($meta[0]) || \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\DI\DependencyChecker::isExpired(...$meta) || $orig !== $meta[2] && ($updatedMeta = \serialize($meta));
        }
        return \false;
    }
    ...
    {
        $compiler = new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\DI\Compiler();
