...
abstract class CompilerExtension
{
    ...
    /** @return static */
    public function setCompiler(\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\DI\Compiler $compiler, string $name)
    {
        $this->initialization = new \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Closure();
    ...
    public function setConfig($config)
    {
        if (!\is_array($config) && !\is_object($config)) {
            throw new \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException();
        }
        $this->config = $config;
        return $this;
    }
    ...
    {
        return \is_object($this->config) ? \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Schema\Expect::from($this->config) : \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Schema\Expect::array();
    }
    /**
    ...
    public function validateConfig(array $expected, array $config = null, string $name = null) : array
    {
        ...
        if ($extra = \array_diff_key((array) $config, $expected)) {
            ...
            $hint = \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Utils\Helpers::getSuggestion(\array_keys($expected), \key($extra));
            $extra = $hint ? \key($extra) : \implode("', '{$name} › ", \array_keys($extra));
            throw new \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\DI\InvalidConfigurationException("Unknown configuration option '{$name} › {$extra}'" . ($hint ? ", did you mean '{$name} › {$hint}'?" : '.'));
        }
        return \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Schema\Helpers::merge($config, $expected);
    }
    ...
    public function loadDefinitionsFromConfig(array $configList)
    {
        ...
        foreach ($configList as $key => $config) {
            ...
            $res[$key] = \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers::prefixServiceName($config, $this->name);
        }
    ...
    public function getInitialization() : \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Closure
    {
