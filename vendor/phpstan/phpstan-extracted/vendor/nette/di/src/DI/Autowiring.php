...
class Autowiring
{
    ...
    public function getByType(string $type, bool $throw = \false)
    {
        ...
        if (empty($types[$type])) {
            if ($throw) {
                throw new \RectorPrefix20210518\_HumbugBox0b2f2d5c77b8\Nette\DI\MissingServiceException("Service of type '{$type}' not found.");
            }
            return null;
            ...
            throw new \RectorPrefix20210518\_HumbugBox0b2f2d5c77b8\Nette\DI\ServiceCreationException("Multiple services of type {$type} found: " . \implode(', ', $list) . $hint);
        }
    }
    /**
        ...
        foreach ($this->builder->getDefinitions() as $name => $def) {
            ...
            if (\is_array($autowired)) {
                foreach ($autowired as $k => $autowiredType) {
                    if ($autowiredType === \RectorPrefix20210518\_HumbugBox0b2f2d5c77b8\Nette\DI\ContainerBuilder::THIS_SERVICE) {
                        $autowired[$k] = $type;
                    } elseif (!\is_a($type, $autowiredType, \true)) {
                        throw new \RectorPrefix20210518\_HumbugBox0b2f2d5c77b8\Nette\DI\ServiceCreationException("Incompatible class {$autowiredType} in autowiring definition of service '{$name}'.");
                    }
                }
            }
