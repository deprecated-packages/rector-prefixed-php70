...
class DependencyChecker
{
    ...
    public function export() : array
    {
        ...
        foreach ($this->dependencies as $dep) {
            if (\is_string($dep)) {
                ...
                $functions[] = \RectorPrefix20210525\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::toString($dep);
            } else {
                throw new \RectorPrefix20210525\_HumbugBox0b2f2d5c77b8\Nette\InvalidStateException('Unexpected dependency ' . \gettype($dep));
            }
        }
    ...
    private static function calculateHash(array $classes, array $functions) : string
    {
        ...
        foreach ($classes as $name) {
            ...
            $hash[] = [$name, \RectorPrefix20210525\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getUseStatements($class), $class->isAbstract(), \get_parent_class($name), \class_implements($name), \class_uses($name)];
        ...
        foreach ($functions as $name) {
            if (\strpos($name, '::')) {
                ...
                $uses = \RectorPrefix20210525\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getUseStatements($class);
            } else {
    ...
    private static function hashParameters(\ReflectionFunctionAbstract $method) : array
    {
        ...
        foreach ($method->getParameters() as $param) {
            $res[] = [$param->name, \RectorPrefix20210525\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getParameterType($param), $param->allowsNull(), $param->isVariadic(), $param->isDefaultValueAvailable() ? [\RectorPrefix20210525\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getParameterDefaultValue($param)] : null];
        }
        return $res;
    }
}
