 ...
 * Accessor definition.
 */
final class AccessorDefinition extends \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\Definitions\Definition
{
    ...
    public function setImplement(string $type)
    {
        ...
        if (!$method || $method->isStatic() || $method->getName() !== self::METHOD_GET || \count($rc->getMethods()) > 1) {
            throw new \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException("Service '{$this->getName()}': Interface {$type} must have just one non-static method get().");
        } elseif ($method->getNumberOfParameters()) {
            throw new \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException("Service '{$this->getName()}': Method {$type}::get() must have no parameters.");
        }
    ...
    {
        if ($reference instanceof \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\Definitions\Reference) {
            $this->reference = $reference;
        } else {
            $this->reference = \substr($reference, 0, 1) === '@' ? new \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\Definitions\Reference(\substr($reference, 1)) : \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\Definitions\Reference::fromType($reference);
        }
        return $this;
    }
    /**
    ...
    public function complete(\RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\Resolver $resolver)
    {
        if (!$this->reference) {
            ...
            if (!$returnType) {
                throw new \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\ServiceCreationException("Method {$interface}::get() has not return type hint or annotation @return.");
            } elseif (!\class_exists($returnType) && !\interface_exists($returnType)) {
                throw new \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\ServiceCreationException("Check a type hint or annotation @return of the {$interface}::get() method, class '{$returnType}' cannot be found.");
            }
    ...
    public function generateMethod(\RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Method $method, \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\PhpGenerator $generator)
    {
        ...
        $class->addMethod(self::METHOD_GET)->setBody('return $this->container->getService(?);', [$this->reference->getValue()])->setReturnType(\RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getReturnType($rm));
