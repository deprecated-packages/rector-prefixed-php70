...
namespace RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\DI\Config\Adapters;
...
use RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon;
/**
 * Reading and generating NEON files.
 */
final class NeonAdapter implements \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\DI\Config\Adapter
{
    ...
    {
        return $this->process((array) \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::decode(\RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Utils\FileSystem::read($file)));
    }
        ...
        foreach ($arr as $key => $val) {
            if (\is_string($key) && \substr($key, -1) === self::PREVENT_MERGING_SUFFIX) {
                if (!\is_array($val) && $val !== null) {
                    throw new \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\DI\InvalidConfigurationException("Replacing operator is available only for arrays, item '{$key}' is not array.");
                }
                $key = \substr($key, 0, -1);
            ...
            if (\is_array($val)) {
                ...
                if ($val->value === \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::CHAIN) {
                    ...
                    $val = new \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\DI\Definitions\Statement($tmp[0], $this->process($val->attributes));
                }
            }
            $res[$key] = $val;
        }
