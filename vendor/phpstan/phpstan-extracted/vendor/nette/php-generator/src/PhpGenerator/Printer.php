...
class Printer
{
    ...
    public function printFunction(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\GlobalFunction $function, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::formatDocComment($function->getComment() . "\n") . self::printAttributes($function->getAttributes(), $namespace) . 'function ' . ($function->getReturnReference() ? '&' : '') . $function->getName() . $this->printParameters($function, $namespace) . $this->printReturnType($function, $namespace) . "\n{\n" . $this->indent(\ltrim(\rtrim($function->getBody()) . "\n")) . "}\n";
    }
    public function printClosure(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Closure $closure) : string
    {
        $uses = [];
        foreach ($closure->getUses() as $param) {
            $uses[] = ($param->isReference() ? '&' : '') . '$' . $param->getName();
        }
        $useStr = \strlen($tmp = \implode(', ', $uses)) > (new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Dumper())->wrapLength && \count($uses) > 1 ? "\n" . $this->indentation . \implode(",\n" . $this->indentation, $uses) . "\n" : $tmp;
    ...
    public function printArrowFunction(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Closure $closure) : string
    {
        foreach ($closure->getUses() as $use) {
            if ($use->isReference()) {
                throw new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Arrow function cannot bind variables by-reference.');
            }
        }
    ...
    public function printMethod(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Method $method, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        $method->validate();
        return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::formatDocComment($method->getComment() . "\n") . self::printAttributes($method->getAttributes(), $namespace) . ($method->isAbstract() ? 'abstract ' : '') . ($method->isFinal() ? 'final ' : '') . ($method->getVisibility() ? $method->getVisibility() . ' ' : '') . ($method->isStatic() ? 'static ' : '') . 'function ' . ($method->getReturnReference() ? '&' : '') . $method->getName() . ($params = $this->printParameters($method, $namespace)) . $this->printReturnType($method, $namespace) . ($method->isAbstract() || $method->getBody() === null ? ";\n" : (\strpos($params, "\n") === \false ? "\n" : ' ') . "{\n" . $this->indent(\ltrim(\rtrim($method->getBody()) . "\n")) . "}\n");
    }
    public function printClass(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\ClassType $class, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        $class->validate();
        ...
        foreach ($class->getConstants() as $const) {
            ...
            $consts[] = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::formatDocComment((string) $const->getComment()) . self::printAttributes($const->getAttributes(), $namespace) . $def . $this->dump($const->getValue(), \strlen($def)) . ";\n";
        }
        $properties = [];
        foreach ($class->getProperties() as $property) {
            ...
            $properties[] = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::formatDocComment((string) $property->getComment()) . self::printAttributes($property->getAttributes(), $namespace) . $def . ($property->getValue() === null && !$property->isInitialized() ? '' : ' = ' . $this->dump($property->getValue(), \strlen($def) + 3)) . ";\n";
        }
        $methods = [];
    ...
    public function printFile(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpFile $file) : string
    {
        $namespaces = [];
        foreach ($file->getNamespaces() as $namespace) {
            $namespaces[] = $this->printNamespace($namespace);
        }
        return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::normalize("<?php\n" . ($file->getComment() ? "\n" . \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::formatDocComment($file->getComment() . "\n") : '') . "\n" . ($file->hasStrictTypes() ? "declare(strict_types=1);\n\n" : '') . \implode("\n\n", $namespaces)) . "\n";
    }
    /** @return static */
    ...
    protected function indent(string $s) : string
    {
        ...
        return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::indent($s, 1, $this->indentation);
    }
    protected function dump($var, int $column = 0) : string
    {
        return (new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Dumper())->dump($var, $column);
    }
    protected function printUses(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpNamespace $namespace) : string
    {
        $name = $namespace->getName();
        $uses = [];
     ...
     */
    public function printParameters($function, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        $params = [];
        ...
        foreach ($list as $param) {
            ...
            $type = $param->getType();
            $promoted = $param instanceof \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PromotedParameter ? $param : null;
            $params[] = ($promoted ? \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::formatDocComment((string) $promoted->getComment()) : '') . ($attrs = self::printAttributes($param->getAttributes(), $namespace, \true)) . ($promoted ? ($param->getVisibility() ?: 'public') . ' ' : '') . \ltrim($this->printType($type, $param->isNullable(), $namespace) . ' ') . ($param->isReference() ? '&' : '') . ($variadic ? '...' : '') . '$' . $param->getName() . ($param->hasDefaultValue() && !$variadic ? ' = ' . $this->dump($param->getDefaultValue()) : '');
            $special = $special || $promoted || $attrs;
        }
        $line = \implode(', ', $params);
        return \count($params) > 1 && ($special || \strlen($line) > (new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Dumper())->wrapLength) ? "(\n" . $this->indent(\implode(",\n", $params)) . ($special ? ',' : '') . "\n)" : "({$line})";
    }
    /**
     * @param string|null $type
     */
    public function printType($type, bool $nullable = \false, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
    ...
    private function printAttributes(array $attrs, $namespace, bool $inline = \false) : string
    {
        ...
        foreach ($attrs as $attr) {
            $args = (new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Dumper())->format('...?', $attr->getArguments());
