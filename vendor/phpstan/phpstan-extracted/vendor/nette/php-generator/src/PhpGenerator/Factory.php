...
use RectorPrefix20210620\_HumbugBox15516bb2b566\PhpParser;
use PhpParser\Node;
...
final class Factory
{
    ...
    public function fromClassReflection(\ReflectionClass $from, bool $withBodies = \false) : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType
    {
        $class = $from->isAnonymous() ? new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType() : new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType($from->getShortName(), new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\PhpNamespace($from->getNamespaceName()));
        ...
        $class->setComment(\RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
        ...
        foreach ($from->getMethods() as $method) {
            if ($method->getDeclaringClass()->name === $from->name) {
                ...
                if ($withBodies) {
                    $srcMethod = \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\Utils\Reflection::getMethodDeclaringMethod($method);
    ...
    public function fromMethodReflection(\ReflectionMethod $from) : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Method
    {
        $method = new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Method($from->name);
        $method->setParameters(\array_map([$this, 'fromParameterReflection'], $from->getParameters()));
        $method->setStatic($from->isStatic());
        $isInterface = $from->getDeclaringClass()->isInterface();
        $method->setVisibility($from->isPrivate() ? \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PRIVATE : ($from->isProtected() ? \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PROTECTED : ($isInterface ? null : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PUBLIC)));
        $method->setFinal($from->isFinal());
        $method->setAbstract($from->isAbstract() && !$isInterface);
        $method->setBody($from->isAbstract() ? null : '');
        $method->setReturnReference($from->returnsReference());
        $method->setVariadic($from->isVariadic());
        $method->setComment(\RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
    ...
    {
        $function = $from->isClosure() ? new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Closure() : new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\GlobalFunction($from->name);
        ...
        if (!$from->isClosure()) {
            $function->setComment(\RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
        }
    ...
    {
        $ref = \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\Utils\Callback::toReflection($from);
        return $ref instanceof \ReflectionMethod ? self::fromMethodReflection($ref) : self::fromFunctionReflection($ref);
    }
    public function fromParameterReflection(\ReflectionParameter $from) : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Parameter
    {
        $param = \PHP_VERSION_ID >= 80000 && $from->isPromoted() ? new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\PromotedParameter($from->name) : new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Parameter($from->name);
        ...
        if ($from->isDefaultValueAvailable()) {
            $param->setDefaultValue($from->isDefaultValueConstant() ? new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Literal($from->getDefaultValueConstantName()) : $from->getDefaultValue());
            $param->setNullable($param->isNullable() && $param->getDefaultValue() !== null);
        }
        $param->setAttributes(self::getAttributes($from));
        return $param;
    }
    public function fromConstantReflection(\ReflectionClassConstant $from) : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Constant
    {
        $const = new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Constant($from->name);
        $const->setValue($from->getValue());
        $const->setVisibility($from->isPrivate() ? \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PRIVATE : ($from->isProtected() ? \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PROTECTED : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PUBLIC));
        $const->setComment(\RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
        $const->setAttributes(self::getAttributes($from));
        return $const;
    }
    public function fromPropertyReflection(\ReflectionProperty $from) : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Property
    {
        $defaults = $from->getDeclaringClass()->getDefaultProperties();
        $prop = new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Property($from->name);
        $prop->setValue($defaults[$prop->getName()] ?? null);
        $prop->setStatic($from->isStatic());
        $prop->setVisibility($from->isPrivate() ? \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PRIVATE : ($from->isProtected() ? \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PROTECTED : \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\ClassType::VISIBILITY_PUBLIC));
        ...
        $prop->setComment(\RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
    ...
    {
        if ($from->isAnonymous()) {
            throw new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\NotSupportedException('Anonymous classes are not supported.');
        }
        ...
        foreach ($nodeFinder->findInstanceOf($class, \PhpParser\Node\Stmt\ClassMethod::class) as $method) {
            ...
            if ($method->stmts) {
                ...
                $bodies[$method->name->toString()] = \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unindent($body, 2);
            }
        }
        return $bodies;
    }
    ...
    {
        if ($from->isClosure()) {
            throw new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\NotSupportedException('Closures are not supported.');
        }
        list($code, $stmts) = $this->parse($from);
        $nodeFinder = new \PhpParser\NodeFinder();
        ...
        return \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Helpers::unindent($body, 1);
    }
    /**
    ...
    private function parse($from) : array
    {
        ...
        if (!\class_exists(\PhpParser\ParserFactory::class)) {
            throw new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\NotSupportedException("PHP-Parser is required to load method bodies, install package 'nikic/php-parser'.");
        } elseif (!$file) {
            throw new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\InvalidStateException("Source code of {$from->name} not found.");
        }
    ...
    private function getAttributes($from) : array
    {
        ...
        foreach ($from->getAttributes() as $attr) {
            $res[] = new \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\PhpGenerator\Attribute($attr->getName(), $attr->getArguments());
        }
        return $res;
    }
}
