...
final class Dumper
{
    ...
    {
        if ($var instanceof \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Literal) {
            return \ltrim(\RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::indent(\trim((string) $var), $level), "\t");
        } elseif ($var === null) {
            return 'null';
            ...
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Cannot dump resource.');
        } else {
    ...
    private function dumpArray(array &$var, array $parents, int $level, int $column) : string
    {
        if (empty($var)) {
            ...
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Nesting level too deep or recursive dependency.');
        }
    ...
    private function dumpObject(&$var, array $parents, int $level) : string
    {
        if ($var instanceof \Serializable) {
            ...
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Cannot dump closure.');
        }
        $class = \get_class($var);
        if ((new \ReflectionObject($var))->isAnonymous()) {
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Cannot dump anonymous class.');
    ...
    public function format(string $statement, ...$args) : string
    {
        ...
        foreach ($tokens as $n => $token) {
            if ($n % 2 === 0) {
                ...
                throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Insufficient number of arguments.');
            } elseif ($token === '?') {
                $res .= $this->dump(\array_shift($args), \strlen($res) - \strrpos($res, "\n"));
            } elseif ($token === '...?' || $token === '?*') {
                $arg = \array_shift($args);
                if (!\is_array($arg)) {
                    throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Argument must be an array.');
                }
                $res .= $this->dumpArguments($arg, \strlen($res) - \strrpos($res, "\n"));
            } else {
                // $  ->  ::
                $arg = \array_shift($args);
                if ($arg instanceof \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Literal || !\RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\PhpGenerator\Helpers::isIdentifier($arg)) {
        ...
        if ($args) {
            throw new \RectorPrefix20210520\_HumbugBox0b2f2d5c77b8\Nette\InvalidArgumentException('Insufficient number of placeholders.');
        }
        return $res;
    }
