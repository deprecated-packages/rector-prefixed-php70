...
namespace RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
    /**
     * Register autoloader.
        ...
        foreach ($this->scanPaths as $path) {
            ...
            foreach ($iterator as $file) {
                ...
                foreach ($classes as $class) {
                    ...
                    if (isset($info['file'])) {
                        throw new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$info['file']} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    private function loadCache()
    {
        ...
        // @ file may not exist
        if (\is_array($data)) {
            list($this->classes, $this->missing) = $data;
            return;
        }
        if ($lock) {
            \flock($lock, \LOCK_UN);
