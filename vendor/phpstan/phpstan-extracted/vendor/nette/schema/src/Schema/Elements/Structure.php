...
final class Structure implements \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Schema
{
    use Base;
    ...
    {
        (function (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Schema ...$items) {
        })(...\array_values($items));
        $this->items = $items;
        $this->castTo = 'object';
    }
    ...
    {
        throw new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\InvalidStateException('Structure cannot have default value.');
    }
    /**
     * @return $this
    ...
    {
        $this->otherItems = $type instanceof \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Schema ? $type : new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Elements\Type($type);
        return $this;
    }
    /********************* processing ****************d*g**/
        ...
        if (\is_array($value) && \is_array($base)) {
            ...
            foreach ($value as $key => $val) {
                if ($key === $index) {
                    ...
                    $itemSchema = $this->items[$key] ?? $this->otherItems;
                    $base[$key] = $itemSchema ? $itemSchema->merge($val, $base[$key]) : \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Helpers::merge($val, $base[$key]);
                } else {
                    $base[$key] = $val;
                }
            }
    ...
    public function complete($value, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Context $context)
    {
        ...
        if ($extraKeys = \array_keys(\array_diff_key($value, $items))) {
            if ($this->otherItems) {
                ...
                $hint = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Helpers::getSuggestion(\array_map('strval', \array_keys($items)), (string) $extraKeys[0]);
