...
final class Helpers
{
    ...
    {
        if ($type = \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getPropertyType($prop)) {
            return ($prop->getType()->allowsNull() ? '?' : '') . $type;
        } elseif ($type = \preg_replace('#\\s.*#', '', (string) self::parseAnnotation($prop, 'var'))) {
            $class = \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::getPropertyDeclaringClass($prop);
            return \preg_replace_callback('#[\\w\\\\]+#', function ($m) use($class) {
                return \RectorPrefix20210526\_HumbugBox0b2f2d5c77b8\Nette\Utils\Reflection::expandClassName($m[0], $class);
            }, $type);
        }
        return null;
    }
