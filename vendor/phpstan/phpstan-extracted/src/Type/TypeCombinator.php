...
class TypeCombinator
{
    ...
    public static function remove(\PHPStan\Type\Type $fromType, \PHPStan\Type\Type $typeToRemove) : \PHPStan\Type\Type
    {
        ...
        if ($fromType instanceof \PHPStan\Type\BooleanType) {
            ...
            if ($typeToRemove->isSuperTypeOf($arrayType)->yes()) {
                return new \PHPStan\Type\Generic\GenericObjectType(\Traversable::class, [$fromType->getIterableKeyType(), $fromType->getIterableValueType()]);
            }
            $traversableType = new \PHPStan\Type\ObjectType(\Traversable::class);
            if ($typeToRemove->isSuperTypeOf($traversableType)->yes()) {
                return new \PHPStan\Type\ArrayType($fromType->getIterableKeyType(), $fromType->getIterableValueType());
            }
        } elseif ($fromType instanceof \PHPStan\Type\IntegerRangeType) {
            $type = $fromType->tryRemove($typeToRemove);
            if ($type !== null) {
                return $type;
            }
        } elseif ($fromType instanceof \PHPStan\Type\IntegerType) {
     ...
     */
    private static function unionWithSubtractedType(\PHPStan\Type\Type $type, $subtractedType) : \PHPStan\Type\Type
    {
     ...
     * @param \PHPStan\Type\Type|null $subtractedType
     */
    private static function intersectWithSubtractedType(\PHPStan\Type\SubtractableType $subtractableType, $subtractedType) : \PHPStan\Type\Type
    {
