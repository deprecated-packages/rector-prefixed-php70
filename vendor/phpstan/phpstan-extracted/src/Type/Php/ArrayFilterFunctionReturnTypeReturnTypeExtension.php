    ...
    public function getTypeFromFunctionCall(\PHPStan\Reflection\FunctionReflection $functionReflection, \PhpParser\Node\Expr\FuncCall $functionCall, \PHPStan\Analyser\Scope $scope) : \PHPStan\Type\Type
    {
        ...
        if ($arrayArg !== null) {
            ...
            if ($flagArg === null) {
                $var = null;
                $expr = null;
                if ($callbackArg instanceof \PhpParser\Node\Expr\Closure && \count($callbackArg->stmts) === 1 && \count($callbackArg->params) > 0) {
                    $statement = $callbackArg->stmts[0];
                    if ($statement instanceof \PhpParser\Node\Stmt\Return_ && $statement->expr !== null) {
                        $var = $callbackArg->params[0]->var;
                        $expr = $statement->expr;
                    }
                } elseif ($callbackArg instanceof \PhpParser\Node\Expr\ArrowFunction && \count($callbackArg->params) > 0) {
                    $var = $callbackArg->params[0]->var;
