...
class ConstantStringType extends \PHPStan\Type\StringType implements \PHPStan\Type\ConstantScalarType
{
    ...
    public function describe(\PHPStan\Type\VerbosityLevel $level) : string
    {
        return $level->handle(static function () : string {
            ...
            try {
                $truncatedValue = \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::truncate($this->value, self::DESCRIBE_LIMIT);
            } catch (\RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Utils\RegexpException $e) {
    ...
    public function isCallable() : \PHPStan\TrinaryLogic
    {
        ...
        $matches = \RectorPrefix20210517\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($this->value, '#^([a-zA-Z_\\x7f-\\xff\\\\][a-zA-Z0-9_\\x7f-\\xff\\\\]*)::([a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)\\z#');
        if ($matches !== null) {
    ...
    {
        $subtractedTypes = [new \PHPStan\Type\Constant\ConstantBooleanType(\true), \PHPStan\Type\IntegerRangeType::createAllGreaterThanOrEqualTo((float) $this->value)];
        if ($this->value === '') {
            $subtractedTypes[] = new \PHPStan\Type\NullType();
            $subtractedTypes[] = new \PHPStan\Type\StringType();
        }
        if (!(bool) $this->value) {
    ...
    {
        $subtractedTypes = [\PHPStan\Type\IntegerRangeType::createAllGreaterThan((float) $this->value)];
        if (!(bool) $this->value) {
    ...
    {
        $subtractedTypes = [new \PHPStan\Type\Constant\ConstantBooleanType(\false), \PHPStan\Type\IntegerRangeType::createAllSmallerThanOrEqualTo((float) $this->value)];
        if ((bool) $this->value) {
            $subtractedTypes[] = new \PHPStan\Type\Constant\ConstantBooleanType(\true);
        }
        return \PHPStan\Type\TypeCombinator::remove(new \PHPStan\Type\MixedType(), \PHPStan\Type\TypeCombinator::union(...$subtractedTypes));
    ...
    {
        $subtractedTypes = [\PHPStan\Type\IntegerRangeType::createAllSmallerThan((float) $this->value)];
        if ((bool) $this->value) {
            $subtractedTypes[] = new \PHPStan\Type\Constant\ConstantBooleanType(\false);
        }
        return \PHPStan\Type\TypeCombinator::remove(new \PHPStan\Type\MixedType(), \PHPStan\Type\TypeCombinator::union(...$subtractedTypes));
