...
class TypehintHelper
{
     ...
     * @param string|null $selfClass
     */
    public static function decideTypeFromReflection($reflectionType, $phpDocType = null, $selfClass = null, bool $isVariadic = \false) : \PHPStan\Type\Type
    {
        ...
        if (\RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::endsWith(\strtolower($reflectionTypeString), '\\object')) {
            $reflectionTypeString = 'object';
        }
        if (\RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::endsWith(\strtolower($reflectionTypeString), '\\mixed')) {
            $reflectionTypeString = 'mixed';
        }
        if (\RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::endsWith(\strtolower($reflectionTypeString), '\\false')) {
            $reflectionTypeString = 'false';
        }
        if (\RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::endsWith(\strtolower($reflectionTypeString), '\\null')) {
     ...
     * @param \PHPStan\Type\Type|null $phpDocType
     */
    public static function decideType(\PHPStan\Type\Type $type, $phpDocType = null) : \PHPStan\Type\Type
    {
        if ($phpDocType !== null && !$phpDocType instanceof \PHPStan\Type\ErrorType) {
            ...
            if ((!$phpDocType instanceof \PHPStan\Type\NeverType || $type instanceof \PHPStan\Type\MixedType && !$type->isExplicitMixed()) && $type->isSuperTypeOf(\PHPStan\Type\Generic\TemplateTypeHelper::resolveToBounds($phpDocType))->yes()) {
