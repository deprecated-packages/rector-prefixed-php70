            ...
            try {
                if (\is_array($ignoreError)) {
                    if (!isset($ignoreError['message'])) {
                        $errors[] = \sprintf('Ignored error %s is missing a message.', \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Utils\Json::encode($ignoreError));
                        continue;
                    }
                    if (!isset($ignoreError['path'])) {
                        if (!isset($ignoreError['paths'])) {
                            $errors[] = \sprintf('Ignored error %s is missing a path.', \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Utils\Json::encode($ignoreError));
                        }
                    ...
                    \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match('', $ignoreMessage);
