            ...
            try {
                if (\is_array($ignoreError)) {
                    if (!isset($ignoreError['message'])) {
                        $errors[] = \sprintf('Ignored error %s is missing a message.', \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::encode($ignoreError));
                        continue;
                    }
                    if (!isset($ignoreError['path'])) {
                        if (!isset($ignoreError['paths'])) {
                            $errors[] = \sprintf('Ignored error %s is missing a path.', \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::encode($ignoreError));
                        }
                    ...
                    \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Strings::match('', $ignoreMessage);
