...
class IgnoredError
{
    ...
    public static function shouldIgnore(\PHPStan\File\FileHelper $fileHelper, \PHPStan\Analyser\Error $error, string $ignoredErrorPattern, $path) : bool
    {
        ...
        $ignoredErrorPattern = \str_replace([\preg_quote('RectorPrefix20210531\\r\\n'), \preg_quote('\\r')], \preg_quote('\\n'), $ignoredErrorPattern);
        if ($path !== null) {
            $fileExcluder = new \PHPStan\File\FileExcluder($fileHelper, [$path], []);
            if (\RectorPrefix20210531\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($errorMessage, $ignoredErrorPattern) === null) {
                return \false;
            }
        ...
        return \RectorPrefix20210531\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($errorMessage, $ignoredErrorPattern) !== null;
    }
}
