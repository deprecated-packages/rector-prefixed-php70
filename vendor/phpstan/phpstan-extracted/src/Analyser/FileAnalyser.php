...
class FileAnalyser
{
     ...
     */
    public function analyseFile(string $file, array $analysedFiles, \PHPStan\Rules\Registry $registry, $outerNodeCallback) : \PHPStan\Analyser\FileAnalyserResult
    {
        $fileErrors = [];
        $fileDependencies = [];
        $exportedNodes = [];
        if (\is_file($file)) {
            try {
                ...
                $linesToIgnore = [];
                $temporaryFileErrors = [];
                $nodeCallback = function (\PhpParser\Node $node, \PHPStan\Analyser\Scope $scope) use(&$fileErrors, &$fileDependencies, &$exportedNodes, $file, $registry, $outerNodeCallback, $analysedFiles, &$linesToIgnore, &$temporaryFileErrors) {
     ...
     * @return int|null
     */
    private function findLineToIgnoreComment(\PhpParser\Comment $comment)
    {
