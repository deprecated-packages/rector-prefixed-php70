...
use RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon;
...
{
    const CACHE_VERSION = 'v9-project-extensions';
     ...
     */
    public function __construct(\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher, \PHPStan\File\FileFinder $scanFileFinder, \PHPStan\Reflection\ReflectionProvider $reflectionProvider, string $cacheFilePath, string $tempResultCachePath, array $analysedPaths, array $composerAutoloaderProjectPaths, array $stubFiles, string $usedLevel, $cliAutoloadFile, array $bootstrapFiles, array $scanFiles, array $scanDirectories, array $fileReplacements)
    {
     ...
     */
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, $projectConfigArray, \PHPStan\Command\Output $output, $resultCacheName = null) : \PHPStan\Analyser\ResultCache\ResultCache
    {
        if ($debug) {
            if ($output->isDebug()) {
                $output->writeLineFormatted('Result cache not used because of debug mode.');
            }
            return new \PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), $this->getMeta($allAnalysedFiles, $projectConfigArray), [], [], []);
        }
        if ($onlyFiles) {
            if ($output->isDebug()) {
        ...
        if ($this->isMetaDifferent($data['meta'], $meta)) {
            ...
            return new \PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), $meta, [], [], []);
        }
        ...
        return new \PHPStan\Analyser\ResultCache\ResultCache(\array_unique($filesToAnalyse), \false, $data['lastFullAnalysisTime'], $meta, $filteredErrors, $invertedDependenciesToReturn, $filteredExportedNodes);
    }
    /**
    ...
    private function isMetaDifferent(array $cachedMeta, array $currentMeta) : bool
    {
        ...
        if ($projectConfig !== null) {
            $currentMeta['projectConfig'] = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::encode($currentMeta['projectConfig']);
        }
     ...
     */
    public function process(\PHPStan\Analyser\AnalyserResult $analyserResult, \PHPStan\Analyser\ResultCache\ResultCache $resultCache, \PHPStan\Command\Output $output, bool $onlyFiles, $save) : \PHPStan\Analyser\ResultCache\ResultCacheProcessResult
    {
        ...
        $doSave = function (array $errorsByFile, $dependencies, array $exportedNodes, $resultCacheName) use($internalErrors, $resultCache, $output, $onlyFiles, $meta) : bool {
            if ($onlyFiles) {
                if ($output->isDebug()) {
     ...
     * @return mixed[]|null
     */
    private function mergeDependencies(\PHPStan\Analyser\ResultCache\ResultCache $resultCache, $freshDependencies)
    {
     ...
     * @return void
     */
    private function save(int $lastFullAnalysisTime, $resultCacheName, array $errors, array $dependencies, array $exportedNodes, array $meta)
    {
...
php;
        ...
        if ($projectConfigArray !== null) {
            $meta['projectConfig'] = \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::encode($projectConfigArray);
        }
     ...
     */
    private function getProjectExtensionFiles($projectConfig, array $dependencies) : array
    {
    ...
    private function getClassesFromConfigDefinition($definition) : array
    {
        ...
        if ($definition instanceof \RectorPrefix20210528\_HumbugBox0b2f2d5c77b8\Nette\DI\Definitions\Statement) {
     ...
     * @return mixed[]
     */
    private function getMeta(array $allAnalysedFiles, $projectConfigArray) : array
    {
        ...
        return ['cacheVersion' => self::CACHE_VERSION, 'phpstanVersion' => $this->getPhpStanVersion(), 'phpVersion' => \PHP_VERSION_ID, 'projectConfig' => $projectConfigArray, 'analysedPaths' => $this->analysedPaths, 'scannedFiles' => $this->getScannedFiles($allAnalysedFiles), 'composerLocks' => $this->getComposerLocks(), 'composerInstalled' => $this->getComposerInstalled(), 'executedFilesHashes' => $this->getExecutedFileHashes(), 'phpExtensions' => $extensions, 'stubFiles' => $this->getStubFiles(), 'level' => $this->usedLevel];
    }
