 ...
 */
class AutoloadSourceLocator implements \PHPStan\BetterReflection\SourceLocator\Type\SourceLocator
{
    ...
    public function locateIdentifier(\PHPStan\BetterReflection\Reflector\Reflector $reflector, \PHPStan\BetterReflection\Identifier\Identifier $identifier)
    {
        ...
        if ($identifier->isConstant()) {
            ...
            $reflection = \PHPStan\BetterReflection\Reflection\ReflectionConstant::createFromNode($reflector, new \PhpParser\Node\Expr\FuncCall(new \PhpParser\Node\Name('define'), [new \PhpParser\Node\Arg(new \PhpParser\Node\Scalar\String_($constantName)), new \PhpParser\Node\Arg(new \PhpParser\Node\Scalar\String_(''))]), new \PHPStan\BetterReflection\SourceLocator\Located\LocatedSource('', null), null, null);
        ...
        list($potentiallyLocatedFile, $className, $startLine) = $locateResult;
        return $this->findReflection($reflector, $potentiallyLocatedFile, new \PHPStan\BetterReflection\Identifier\Identifier($className, $identifier->getType()), $startLine);
    }
    /**
     * @param int|null $startLine
     * @return \PHPStan\BetterReflection\Reflection\Reflection|null
     */
    private function findReflection(\PHPStan\BetterReflection\Reflector\Reflector $reflector, string $file, \PHPStan\BetterReflection\Identifier\Identifier $identifier, $startLine)
    {
    ...
    private function locateClassByName(string $className)
    {
        ...
        try {
            ...
            return \PHPStan\Reflection\BetterReflection\SourceLocator\FileReadTrapStreamWrapper::withStreamWrapperOverride(static function () use($className) {
