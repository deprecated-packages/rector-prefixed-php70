...
class ClassBlacklistSourceLocator implements \PHPStan\BetterReflection\SourceLocator\Type\SourceLocator
{
    /** @var SourceLocator */
    ...
    public function locateIdentifier(\PHPStan\BetterReflection\Reflector\Reflector $reflector, \PHPStan\BetterReflection\Identifier\Identifier $identifier)
    {
        if ($identifier->isClass()) {
            foreach ($this->patterns as $pattern) {
                if (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($identifier->getName(), $pattern) !== null) {
                    return null;
                }
            }
        }
