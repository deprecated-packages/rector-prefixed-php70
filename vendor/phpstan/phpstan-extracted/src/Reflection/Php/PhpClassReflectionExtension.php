...
class PhpClassReflectionExtension implements \PHPStan\Reflection\PropertiesClassReflectionExtension, \PHPStan\Reflection\MethodsClassReflectionExtension
{
    ...
    private function createMethod(\PHPStan\Reflection\ClassReflection $classReflection, \PHPStan\Reflection\Php\BuiltinMethodReflection $methodReflection, bool $includingAnnotations) : \PHPStan\Reflection\MethodReflection
    {
        ...
        if ($this->signatureMapProvider->hasMethodSignature($declaringClassName, $methodReflection->getName())) {
            ...
            foreach ($variantNumbers as $variantNumber) {
                ...
                if (\count($variantNumbers) === 1) {
                    ...
                    if ($stubPhpDocPair !== null) {
                        list($stubPhpDoc, $stubDeclaringClass) = $stubPhpDocPair;
            ...
            return new \PHPStan\Reflection\Native\NativeMethodReflection($this->reflectionProvider, $declaringClass, $methodReflection, $variants, $hasSideEffects, $stubPhpDocString, $throwType);
        }
        ...
        if ($stubPhpDocPair !== null) {
            list($resolvedPhpDoc, $phpDocBlockClassReflection) = $stubPhpDocPair;
        }
        $stubPhpDocString = null;
        if ($resolvedPhpDoc === null) {
     ...
     */
    private function createNativeMethodVariant(\PHPStan\Reflection\SignatureMap\FunctionSignature $methodSignature, array $stubPhpDocParameterTypes, array $stubPhpDocParameterVariadicity, $stubPhpDocReturnType, array $phpDocParameterTypes, $phpDocReturnType, array $phpDocParameterNameMapping) : \PHPStan\Reflection\FunctionVariantWithPhpDocs
    {
        $parameters = [];
        foreach ($methodSignature->getParameters() as $parameterSignature) {
            ...
            $parameters[] = new \PHPStan\Reflection\Native\NativeParameterWithPhpDocsReflection($phpDocParameterName, $parameterSignature->isOptional(), $type ?? $parameterSignature->getType(), $phpDocType ?? new \PHPStan\Type\MixedType(), $parameterSignature->getNativeType(), $parameterSignature->passedByReference(), $stubPhpDocParameterVariadicity[$parameterSignature->getName()] ?? $parameterSignature->isVariadic(), null);
        }
        $returnType = null;
     ...
     * @return string|null
     */
    private function findPropertyTrait(\ReflectionProperty $propertyReflection)
    {
        if ($propertyReflection instanceof \PHPStan\BetterReflection\Reflection\Adapter\ReflectionProperty) {
     ...
     * @return string|null
     */
    private function deepScanTraitsForProperty(array $traits, \ReflectionProperty $propertyReflection)
    {
     ...
     * @return string|null
     */
    private function findMethodTrait(\PHPStan\Reflection\Php\BuiltinMethodReflection $methodReflection)
    {
        if ($methodReflection->getReflection() instanceof \PHPStan\BetterReflection\Reflection\Adapter\ReflectionMethod) {
     ...
     */
    private function inferPrivatePropertyType(string $propertyName, \PHPStan\Reflection\MethodReflection $constructor)
    {
    ...
    private function inferAndCachePropertyTypes(\PHPStan\Reflection\MethodReflection $constructor) : array
    {
        ...
        list($templateTypeMap, $phpDocParameterTypes, $phpDocReturnType, $phpDocThrowType, $deprecatedDescription, $isDeprecated, $isInternal, $isFinal, $isPure) = $this->nodeScopeResolver->getPhpDocs($classScope, $methodNode);
     ...
     */
    private function findClassNode(string $className, array $nodes)
    {
     ...
     */
    private function findConstructorNode(string $methodName, array $classStatements)
    {
     ...
     * @return \PHPStan\Type\Type|null
     */
    private function getPhpDocReturnType(\PHPStan\Reflection\ClassReflection $phpDocBlockClassReflection, \PHPStan\PhpDoc\ResolvedPhpDocBlock $resolvedPhpDoc, \PHPStan\Type\Type $nativeReturnType)
    {
     ...
     * @return mixed[]|null
     */
    private function findMethodPhpDocIncludingAncestors(\PHPStan\Reflection\ClassReflection $declaringClass, string $methodName, array $positionalParameterNames)
    {
