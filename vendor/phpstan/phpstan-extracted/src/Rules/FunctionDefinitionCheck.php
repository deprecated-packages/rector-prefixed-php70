...
class FunctionDefinitionCheck
{
    ...
    /** @var bool */
    private $checkMissingTemplateTypeInParameter;
    public function __construct(\PHPStan\Reflection\ReflectionProvider $reflectionProvider, \PHPStan\Rules\ClassCaseSensitivityCheck $classCaseSensitivityCheck, \PHPStan\Php\PhpVersion $phpVersion, bool $checkClassCaseSensitivity, bool $checkThisOnly, bool $checkMissingTemplateTypeInParameter)
    {
     ...
     * @return RuleError[]
     */
    public function checkFunction(\PhpParser\Node\Stmt\Function_ $function, \PHPStan\Reflection\FunctionReflection $functionReflection, string $parameterMessage, string $returnMessage, string $unionTypesMessage, string $templateTypeMissingInParameterMessage) : array
    {
     ...
     * @return RuleError[]
     */
    public function checkClassMethod(\PHPStan\Reflection\Php\PhpMethodFromParserNodeReflection $methodReflection, \PhpParser\Node\Stmt\ClassMethod $methodNode, string $parameterMessage, string $returnMessage, string $unionTypesMessage, string $templateTypeMissingInParameterMessage) : array
    {
     ...
     * @return RuleError[]
     */
    private function checkParametersAcceptor(\PHPStan\Reflection\ParametersAcceptor $parametersAcceptor, \PhpParser\Node\FunctionLike $functionNode, string $parameterMessage, string $returnMessage, string $unionTypesMessage, string $templateTypeMissingInParameterMessage) : array
    {
        $errors = [];
        ...
        if ($this->checkMissingTemplateTypeInParameter) {
            ...
            if (\count($templateTypes) > 0) {
                foreach ($parametersAcceptor->getParameters() as $parameter) {
                    \PHPStan\Type\TypeTraverser::map($parameter->getType(), static function (\PHPStan\Type\Type $type, callable $traverse) use(&$templateTypes) : Type {
