...
class InstantiationRule implements \PHPStan\Rules\Rule
{
    ...
    {
        $errors = [];
        foreach ($this->getClassNames($node, $scope) as list($class, $isName)) {
     ...
     * @return RuleError[]
     */
    private function checkClassName(string $class, bool $isName, \PhpParser\Node $node, \PHPStan\Analyser\Scope $scope) : array
    {
    ...
    private function getClassNames(\PhpParser\Node $node, \PHPStan\Analyser\Scope $scope) : array
    {
        ...
        return \array_merge(\array_map(static function (\PHPStan\Type\Constant\ConstantStringType $type) : array {
            return [$type->getValue(), \true];
        }, \PHPStan\Type\TypeUtils::getConstantStrings($type)), \array_map(static function (string $name) : array {
            return [$name, \false];
