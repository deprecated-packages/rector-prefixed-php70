...
class FunctionCallParametersCheck
{
    ...
    public function check(\PHPStan\Reflection\ParametersAcceptor $parametersAcceptor, \PHPStan\Analyser\Scope $scope, bool $isBuiltin, $funcCall, array $messages) : array
    {
        ...
        if (!$hasNamedArguments) {
            ...
            foreach ($arguments as $i => list($argumentValue, $argumentValueType, $unpack, $argumentName)) {
                if ($unpack) {
        ...
        list($addedErrors, $argumentsWithParameters) = $this->processArguments($parametersAcceptor, $funcCall->getLine(), $isBuiltin, $arguments, $hasNamedArguments, $messages[10], $messages[11]);
        ...
        foreach ($argumentsWithParameters as $i => list($argumentValue, $argumentValueType, $unpack, $argumentName, $argumentLine, $parameter)) {
            ...
            if ($this->checkArgumentTypes && !$parameter->passedByReference()->createsNewVariable() && !$this->ruleLevelHelper->accepts($parameterType, $argumentValueType, $scope->isDeclareStrictTypes())) {
                $verbosityLevel = \PHPStan\Type\VerbosityLevel::getRecommendedLevelByType($parameterType, $argumentValueType);
        ...
        if ($this->checkMissingTypehints && $parametersAcceptor instanceof \PHPStan\Reflection\ResolvedFunctionVariant) {
            ...
            if (\count($resolvedTypes) > 0) {
                $returnTemplateTypes = [];
                \PHPStan\Type\TypeTraverser::map($originalParametersAcceptor->getReturnType(), static function (\PHPStan\Type\Type $type, callable $traverse) use(&$returnTemplateTypes) : Type {
                ...
                foreach ($originalParametersAcceptor->getParameters() as $parameter) {
                    \PHPStan\Type\TypeTraverser::map($parameter->getType(), static function (\PHPStan\Type\Type $type, callable $traverse) use(&$parameterTemplateTypes) : Type {
                        if ($type instanceof \PHPStan\Type\Generic\TemplateType) {
    ...
    private function processArguments(\PHPStan\Reflection\ParametersAcceptor $parametersAcceptor, int $line, bool $isBuiltin, array $arguments, bool $hasNamedArguments, string $missingParameterMessage, string $unknownParameterMessage) : array
    {
        ...
        foreach ($arguments as $i => list($argumentValue, $argumentValueType, $unpack, $argumentName, $argumentLine)) {
