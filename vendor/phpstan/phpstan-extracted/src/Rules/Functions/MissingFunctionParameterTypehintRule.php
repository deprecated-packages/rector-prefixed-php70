...
final class MissingFunctionParameterTypehintRule implements \PHPStan\Rules\Rule
{
    ...
    private function checkFunctionParameter(\PHPStan\Reflection\FunctionReflection $functionReflection, \PHPStan\Reflection\ParameterReflection $parameterReflection) : array
    {
        ...
        foreach ($this->missingTypehintCheck->getIterableTypesWithMissingValueTypehint($parameterType) as $iterableType) {
            ...
            $messages[] = \PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Function %s() has parameter $%s with no value type specified in iterable type %s.', $functionReflection->getName(), $parameterReflection->getName(), $iterableTypeDescription))->tip(\PHPStan\Rules\MissingTypehintCheck::TURN_OFF_MISSING_ITERABLE_VALUE_TYPE_TIP)->build();
        }
        foreach ($this->missingTypehintCheck->getNonGenericObjectTypesWithGenericClass($parameterType) as list($name, $genericTypeNames)) {
        ...
        foreach ($this->missingTypehintCheck->getCallablesWithMissingSignature($parameterType) as $callableType) {
            $messages[] = \PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Function %s() has parameter $%s with no signature specified for %s.', $functionReflection->getName(), $parameterReflection->getName(), $callableType->describe(\PHPStan\Type\VerbosityLevel::typeOnly())))->build();
        }
        return $messages;
    }
}
