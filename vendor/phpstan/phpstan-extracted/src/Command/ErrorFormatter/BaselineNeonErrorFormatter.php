...
class BaselineNeonErrorFormatter implements \PHPStan\Command\ErrorFormatter\ErrorFormatter
{
    ...
    public function formatErrors(\PHPStan\Command\AnalysisResult $analysisResult, \PHPStan\Command\Output $output) : int
    {
        if (!$analysisResult->hasErrors()) {
            $output->writeRaw(\RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::encode(['parameters' => ['ignoreErrors' => []]], \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::BLOCK));
            return 0;
        }
        $fileErrors = [];
        ...
        foreach ($fileErrors as $file => $errorMessages) {
            ...
            foreach ($fileErrorsCounts as $message => $count) {
                $errorsToOutput[] = ['message' => \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers::escape('#^' . \preg_quote($message, '#') . '$#'), 'count' => $count, 'path' => \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers::escape($this->relativePathHelper->getRelativePath($file))];
            }
        }
        $output->writeRaw(\RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::encode(['parameters' => ['ignoreErrors' => $errorsToOutput]], \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Neon\Neon::BLOCK));
        return 1;
    }
}
