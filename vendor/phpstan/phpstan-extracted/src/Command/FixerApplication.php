...
use RectorPrefix20210616\_HumbugBox15516bb2b566\Composer\CaBundle\CaBundle;
...
use RectorPrefix20210616\_HumbugBox15516bb2b566\React\ChildProcess\Process;
...
use RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Browser;
use RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\CancellablePromiseInterface;
use RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\ExtendedPromiseInterface;
...
use RectorPrefix20210616\_HumbugBox15516bb2b566\React\Socket\Connector;
use RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Helper\ProgressBar;
...
use function RectorPrefix20210616\_HumbugBox15516bb2b566\Clue\React\Block\await;
use function escapeshellarg;
use function file_exists;
use function RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\resolve;
class FixerApplication
{
     ...
     */
    public function run($projectConfigFile, \PHPStan\Command\InceptionResult $inceptionResult, \RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Output\OutputInterface $output, array $fileSpecificErrors, array $notFileSpecificErrors, int $filesCount, string $mainScript) : int
    {
        ...
        $server->on('connection', function (\RectorPrefix20210616\_HumbugBox15516bb2b566\React\Socket\ConnectionInterface $connection) use($loop, $projectConfigFile, $input, $output, $fileSpecificErrors, $notFileSpecificErrors, $mainScript, $filesCount, $reanalyseProcessQueue, $inceptionResult) {
            $decoder = new \RectorPrefix20210616\_HumbugBox15516bb2b566\Clue\React\NDJson\Decoder($connection, \true, 512, \defined('JSON_INVALID_UTF8_IGNORE') ? \JSON_INVALID_UTF8_IGNORE : 0, 128 * 1024 * 1024);
            ...
            $decoder->on('data', function (array $data) use($loop, $encoder, $projectConfigFile, $input, $output, $mainScript, $reanalyseProcessQueue, $inceptionResult) {
                ...
                $this->reanalyseWithTmpFile($loop, $inceptionResult, $mainScript, $reanalyseProcessQueue, $projectConfigFile, $data['data']['tmpFile'], $data['data']['insteadOfFile'], $data['data']['fixerSuggestionId'], $input)->done(static function (string $output) use($encoder, $id) {
                    $encoder->write(['id' => $id, 'response' => \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::decode($output, \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::FORCE_ARRAY)]);
     ...
     */
    private function getFixerProcess(\RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Output\OutputInterface $output, int $serverPort) : \RectorPrefix20210616\_HumbugBox15516bb2b566\React\ChildProcess\Process
    {
        ...
        return new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\ChildProcess\Process(\sprintf('%s -d memory_limit=%s %s --port %d', \PHP_BINARY, \escapeshellarg(\ini_get('memory_limit')), \escapeshellarg($pharPath), $serverPort), null, $env, []);
    }
    /**
     * @return void
     */
    private function downloadPhar(\RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Output\OutputInterface $output, string $pharPath, string $infoPath)
    {
        $currentVersion = null;
        if (\file_exists($pharPath) && \file_exists($infoPath)) {
            /** @var array{version: string, date: string} $currentInfo */
            $currentInfo = \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::decode(\PHPStan\File\FileReader::read($infoPath), \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::FORCE_ARRAY);
        ...
        $loop = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\EventLoop\StreamSelectLoop();
        $client = new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Http\Browser($loop, new \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Socket\Connector($loop, ['timeout' => 5, 'tls' => ['cafile' => \RectorPrefix20210616\_HumbugBox15516bb2b566\Composer\CaBundle\CaBundle::getBundledCaBundlePath()], 'dns' => '1.1.1.1']));
        /** @var array{url: string, version: string} $latestInfo */
        $latestInfo = \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::decode((string) \RectorPrefix20210616\_HumbugBox15516bb2b566\Clue\React\Block\await($client->get('https://fixer-download-api.phpstan.com/latest'), $loop, 5.0)->getBody(), \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::FORCE_ARRAY);
        // @phpstan-ignore-line
        ...
        $progressBar = new \RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Helper\ProgressBar($output);
        $client->requestStreaming('GET', $latestInfo['url'])->done(static function (\RectorPrefix20210616\_HumbugBox15516bb2b566\Psr\Http\Message\ResponseInterface $response) use($progressBar, $pharPathResource) {
    ...
    {
        \PHPStan\File\FileWriter::write($infoPath, \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::encode(['version' => $version, 'date' => (new \DateTimeImmutable('', new \DateTimeZone('UTC')))->format(\DateTime::ATOM)]));
    }
    /**
     ...
     * @return void
     */
    private function monitorFileChanges(\RectorPrefix20210616\_HumbugBox15516bb2b566\React\EventLoop\LoopInterface $loop, callable $hasChangesCallback)
    {
     ...
     * @param string|null $projectConfigFile
     */
    private function reanalyseWithTmpFile(\RectorPrefix20210616\_HumbugBox15516bb2b566\React\EventLoop\LoopInterface $loop, \PHPStan\Command\InceptionResult $inceptionResult, string $mainScript, \PHPStan\Process\Runnable\RunnableQueue $runnableQueue, $projectConfigFile, string $tmpFile, string $insteadOfFile, string $fixerSuggestionId, \RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Input\InputInterface $input) : \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\PromiseInterface
    {
     ...
     */
    private function reanalyseAfterFileChanges(\RectorPrefix20210616\_HumbugBox15516bb2b566\React\EventLoop\LoopInterface $loop, \PHPStan\Command\InceptionResult $inceptionResult, string $mainScript, $projectConfigFile, $fixerSuggestionId, \RectorPrefix20210616\_HumbugBox15516bb2b566\Symfony\Component\Console\Input\InputInterface $input) : \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\PromiseInterface
    {
        ...
        if (\count($resultCache->getFilesToAnalyse()) === 0) {
            ...
            return \RectorPrefix20210616\_HumbugBox15516bb2b566\React\Promise\resolve(['fileSpecificErrors' => $finalFileSpecificErrors, 'notFileSpecificErrors' => $finalNotFileSpecificErrors]);
        }
        ...
        return $this->processInProgress->then(static function (string $output) : array {
            return \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::decode($output, \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Utils\Json::FORCE_ARRAY);
        });
    }
