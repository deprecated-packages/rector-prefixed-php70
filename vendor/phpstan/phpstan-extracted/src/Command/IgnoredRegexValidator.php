...
use RectorPrefix20210518\Hoa\Compiler\Llk\Parser;
use RectorPrefix20210518\Hoa\Compiler\Llk\TreeNode;
...
class IgnoredRegexValidator
{
    ...
    public function __construct(\RectorPrefix20210518\Hoa\Compiler\Llk\Parser $parser, \PHPStan\PhpDoc\TypeStringResolver $typeStringResolver)
    {
        $this->parser = $parser;
    ...
    public function validate(string $regex) : \PHPStan\Command\IgnoredRegexValidatorResult
    {
        ...
        } catch (\RectorPrefix20210518\Hoa\Exception\Exception $e) {
     ...
     */
    private function getIgnoredTypes(\RectorPrefix20210518\Hoa\Compiler\Llk\TreeNode $ast) : array
    {
        ...
        foreach ($alternation->getChildren() as $child) {
            ...
            $matches = \RectorPrefix20210518\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($text, '#^([a-zA-Z0-9]+)[,]?\\s*#');
            if ($matches === null) {
                continue;
            }
            try {
     ...
     * @return string|null
     */
    private function getText(\RectorPrefix20210518\Hoa\Compiler\Llk\TreeNode $treeNode)
    {
    ...
    private function hasAnchorsInTheMiddle(\RectorPrefix20210518\Hoa\Compiler\Llk\TreeNode $ast) : bool
    {
        if ($ast->getId() === 'token') {
