...
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Composer\XdebugHandler\XdebugHandler;
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Config\Adapters\PhpAdapter;
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers;
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Context as SchemaContext;
use RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Processor;
...
class CommandHelper
{
     ...
     */
    public static function begin(\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Symfony\Component\Console\Output\OutputInterface $output, array $paths, $pathsFile, $memoryLimit, $autoloadFile, array $composerAutoloaderProjectPaths, $projectConfigFile, $generateBaselineFile, $level, bool $allowXdebug, bool $manageMemoryLimitFile = \true, bool $debugEnabled = \false, $singleReflectionFile = null, $singleReflectionInsteadOfFile = null) : \PHPStan\Command\InceptionResult
    {
        if (!$allowXdebug) {
            $xdebug = new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Composer\XdebugHandler\XdebugHandler('phpstan', '--ansi');
            $xdebug->check();
            unset($xdebug);
        }
        ...
        $errorOutput = (static function () use($input, $output) : Output {
            $symfonyErrorOutput = $output instanceof \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Symfony\Component\Console\Output\ConsoleOutputInterface ? $output->getErrorOutput() : $output;
        ...
        if ($memoryLimit !== null) {
            if (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($memoryLimit, '#^-?\\d+[kMG]?$#i') === null) {
        ...
        if ($projectConfigFile !== null) {
            ...
            } catch (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\InvalidStateException $e) {
                $errorOutput->writeLineFormatted($e->getMessage());
                throw new \PHPStan\Command\InceptionNotSuccessfulException();
            } catch (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\FileNotFoundException $e) {
                $errorOutput->writeLineFormatted($e->getMessage());
                throw new \PHPStan\Command\InceptionNotSuccessfulException();
            }
            ...
            if (isset($projectConfig['parameters']['tmpDir'])) {
                $tmpDir = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers::expand($projectConfig['parameters']['tmpDir'], $defaultParameters);
            }
            ...
            if (\count($paths) === 0 && isset($projectConfig['parameters']['paths'])) {
                $analysedPathsFromConfig = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers::expand($projectConfig['parameters']['paths'], $defaultParameters);
        ...
        } catch (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\InvalidConfigurationException $e) {
            $errorOutput->writeLineFormatted('<error>Invalid configuration:</error>');
            $errorOutput->writeLineFormatted($e->getMessage());
            throw new \PHPStan\Command\InceptionNotSuccessfulException();
        } catch (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\AssertionException $e) {
            $errorOutput->writeLineFormatted('<error>Invalid configuration:</error>');
            $errorOutput->writeLineFormatted($e->getMessage());
            throw new \PHPStan\Command\InceptionNotSuccessfulException();
        }
        ...
        $processor->onNewContext[] = static function (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Schema\Context $context) {
    ...
    private static function detectDuplicateIncludedFiles(\PHPStan\Command\Output $output, \PHPStan\File\FileHelper $fileHelper, array $configFiles, array $loaderParameters)
    {
        ...
        $phpAdapter = new \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Config\Adapters\PhpAdapter();
        $allConfigFiles = [];
     ...
     * @return string[]
     */
    private static function getConfigFiles(\PHPStan\File\FileHelper $fileHelper, \PHPStan\DependencyInjection\NeonAdapter $neonAdapter, \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Config\Adapters\PhpAdapter $phpAdapter, string $configFile, array $loaderParameters, $generateBaselineFile) : array
    {
        ...
        if (\RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::endsWith($configFile, '.php')) {
        ...
        if (isset($data['includes'])) {
            \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Validators::assert($data['includes'], 'list', \sprintf("section 'includes' in file '%s'", $configFile));
            $includes = \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\DI\Helpers::expand($data['includes'], $loaderParameters);
    ...
    {
        return \RectorPrefix20210523\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($includedFile, '#([a-z]+:)?[/\\\\]#Ai') !== null ? $includedFile : \dirname($mainFile) . '/' . $includedFile;
    }
}
