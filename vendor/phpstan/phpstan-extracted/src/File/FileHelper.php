...
class FileHelper
{
    ...
    public function absolutizePath(string $path) : string
    {
        ...
        if (\RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::startsWith($path, 'phar://')) {
            return $path;
        }
    ...
    {
        $matches = \RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::match($originalPath, '~^([a-z]+)\\:\\/\\/(.+)~');
        if ($matches !== null) {
        ...
        $path = \RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::replace($path, '~/{2,}~', '/');
