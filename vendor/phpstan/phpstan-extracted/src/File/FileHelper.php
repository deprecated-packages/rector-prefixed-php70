...
class FileHelper
{
    ...
    public function absolutizePath(string $path) : string
    {
        ...
        if (\RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\Utils\Strings::startsWith($path, 'phar://')) {
            return $path;
        }
    ...
    {
        $matches = \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\Utils\Strings::match($originalPath, '~^([a-z]+)\\:\\/\\/(.+)~');
        if ($matches !== null) {
        ...
        $path = \RectorPrefix20210620\_HumbugBox15516bb2b566\Nette\Utils\Strings::replace($path, '~/{2,}~', '/');
