...
class ExportedNodeResolver
{
     ...
     */
    public function resolve(string $fileName, \PhpParser\Node $node)
    {
        ...
        if ($node instanceof \PhpParser\Node\Stmt\PropertyProperty) {
            ...
            if (!$parentNode instanceof \PhpParser\Node\Stmt\Property) {
                throw new \PHPStan\ShouldNotHappenException(\sprintf('Expected node type %s, %s occurred.', \PhpParser\Node\Stmt\Property::class, \is_object($parentNode) ? \get_class($parentNode) : \gettype($parentNode)));
            }
     ...
     * @return \PHPStan\Dependency\ExportedNode\ExportedPhpDocNode|null
     */
    private function exportPhpDocNode(string $file, $className, $functionName, $text)
    {
        if ($text === null) {
            return null;
        }
