...
class RunnableQueue
{
    ...
    public function queue(\PHPStan\Process\Runnable\Runnable $runnable, int $size) : \RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\React\Promise\CancellablePromiseInterface
    {
        if ($size > $this->maxSize) {
            throw new \PHPStan\ShouldNotHappenException('Runnable size exceeds queue maxSize.');
        }
        $deferred = new \RectorPrefix20210522\_HumbugBox0b2f2d5c77b8\React\Promise\Deferred(static function () use($runnable) {
            $runnable->cancel();
        });
