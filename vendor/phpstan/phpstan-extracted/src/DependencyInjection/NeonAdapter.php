...
use RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\DI\Config\Adapter;
...
use RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Neon\Entity;
use RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Neon\Neon;
use PHPStan\File\FileHelper;
use PHPStan\File\FileReader;
class NeonAdapter implements \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\DI\Config\Adapter
{
    ...
    public function load(string $file) : array
    {
        ...
        try {
            return $this->process((array) \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Neon\Neon::decode($contents), '', $file);
        } catch (\RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Neon\Exception $e) {
            throw new \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Neon\Exception(\sprintf('Error while loading %s: %s', $file, $e->getMessage()));
        }
    }
    /**
     * @param mixed[] $arr
        ...
        foreach ($arr as $key => $val) {
            if (\is_string($key) && \substr($key, -1) === self::PREVENT_MERGING_SUFFIX) {
                if (!\is_array($val) && $val !== null) {
                    throw new \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\DI\InvalidConfigurationException(\sprintf('Replacing operator is available only for arrays, item \'%s\' is not array.', $key));
                }
                $key = \substr($key, 0, -1);
            ...
            if (\is_array($val)) {
                ...
                if ($val->value === \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\Neon\Neon::CHAIN) {
                    ...
                    $val = new \RectorPrefix20210616\_HumbugBox15516bb2b566\Nette\DI\Definitions\Statement($tmp[0], $this->process($val->attributes, $fileKeyToPass, $file));
                }
            }
