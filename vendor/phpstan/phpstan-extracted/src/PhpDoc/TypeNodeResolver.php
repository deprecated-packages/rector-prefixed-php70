...
class TypeNodeResolver
{
    ...
    private function resolveConstTypeNode(\PHPStan\PhpDocParser\Ast\Type\ConstTypeNode $typeNode, \PHPStan\Analyser\NameScope $nameScope) : \PHPStan\Type\Type
    {
        ...
        if ($constExpr instanceof \PHPStan\PhpDocParser\Ast\ConstExpr\ConstFetchNode) {
            ...
            if (\RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::endsWith($constantName, '*')) {
                $constantNameStartsWith = \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::substring($constantName, 0, \RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::length($constantName) - 1);
                $constantTypes = [];
                foreach ($classReflection->getNativeReflection()->getConstants() as $classConstantName => $constantValue) {
                    if (!\RectorPrefix20210519\_HumbugBox0b2f2d5c77b8\Nette\Utils\Strings::startsWith($classConstantName, $constantNameStartsWith)) {
                        continue;
                    }
