...
class PhpDocNodeResolver
{
        ...
        foreach (['@var', '@psalm-var', '@phpstan-var'] as $tagName) {
            ...
            foreach ($phpDocNode->getVarTagValues($tagName) as $tagValue) {
                ...
                if ($tagValue->variableName !== '') {
                    ...
                    $varTag = new \PHPStan\PhpDoc\Tag\VarTag($type);
                    $tagResolved[] = $varTag;
                }
            }
    ...
    public function resolveTemplateTags(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PHPStan\Analyser\NameScope $nameScope) : array
    {
        ...
        foreach ($phpDocNode->getTags() as $phpDocTagNode) {
            ...
            $resolved[$valueNode->name] = new \PHPStan\PhpDoc\Tag\TemplateTag($valueNode->name, $valueNode->bound !== null ? $this->typeNodeResolver->resolve($valueNode->bound, $nameScope->unsetTemplateType($valueNode->name)) : new \PHPStan\Type\MixedType(), $variance);
     ...
     */
    public function resolveReturnTag(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PHPStan\Analyser\NameScope $nameScope)
    {
        $resolved = null;
     ...
     */
    public function resolveThrowsTags(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PHPStan\Analyser\NameScope $nameScope)
    {
     ...
     */
    public function resolveTypeAliasTags(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PHPStan\Analyser\NameScope $nameScope) : array
    {
        $resolved = [];
        foreach (['@psalm-type', '@phpstan-type'] as $tagName) {
            foreach ($phpDocNode->getTypeAliasTagValues($tagName) as $typeAliasTagValue) {
                ...
                $resolved[$alias] = new \PHPStan\PhpDoc\Tag\TypeAliasTag($alias, $typeNode, $nameScope);
            }
        }
        return $resolved;
    }
     ...
     */
    public function resolveTypeAliasImportTags(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PHPStan\Analyser\NameScope $nameScope) : array
    {
        $resolved = [];
        foreach (['@psalm-import-type', '@phpstan-import-type'] as $tagName) {
            foreach ($phpDocNode->getTypeAliasImportTagValues($tagName) as $typeAliasImportTagValue) {
                ...
                $resolved[$importedAs ?? $importedAlias] = new \PHPStan\PhpDoc\Tag\TypeAliasImportTag($importedAlias, $importedFrom, $importedAs);
            }
        }
        return $resolved;
    }
     ...
     */
    public function resolveDeprecatedTag(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PHPStan\Analyser\NameScope $nameScope)
    {
    ...
    public function resolveIsPure(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode) : bool
    {
        foreach ($phpDocNode->getTags() as $phpDocTagNode) {
    ...
    public function resolveIsImpure(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode) : bool
    {
        foreach ($phpDocNode->getTags() as $phpDocTagNode) {
