...
final class EditorConfigFile
{
    ...
    {
        $this->declarationFactory = $declarationFactory ?? new \RectorPrefix20210620\Idiosyncratic\EditorConfig\Declaration\Factory();
    ...
    private function parse(string $content)
    {
        ...
        foreach ($parsedContent as $glob => $declarations) {
            ...
            $this->sections[] = new \RectorPrefix20210620\Idiosyncratic\EditorConfig\Section($this->getGlobPrefix($glob), $glob, $declarations, $this->declarationFactory);
        }
    }
    /**
     * @return void
    ...
    private function setIsRoot(string $isRoot)
    {
        if (\in_array($isRoot, ['true', 'false']) === \false) {
            throw new \RectorPrefix20210620\Idiosyncratic\EditorConfig\Exception\InvalidValue('root', $isRoot);
        }
