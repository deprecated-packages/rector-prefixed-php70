...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node)
    {
        ...
        if (!\RectorPrefix20210523\Nette\Utils\Strings::endsWith($oldRealPath, '.phpt')) {
            return null;
        }
    ...
    {
        // file suffix
        $newRealPath = \RectorPrefix20210523\Nette\Utils\Strings::replace($oldRealPath, self::PHPT_SUFFIX_REGEX, '.php');
        // cleanup tests prefix
        $newRealPath = $this->fileInfoDeletionAnalyzer->clearNameFromTestingPrefix($newRealPath);
        // Test suffix
        if (!\RectorPrefix20210523\Nette\Utils\Strings::endsWith($newRealPath, 'Test.php')) {
            return \RectorPrefix20210523\Nette\Utils\Strings::replace($newRealPath, self::PHP_SUFFIX_REGEX, 'Test.php');
        }
        return $newRealPath;
    }
}
