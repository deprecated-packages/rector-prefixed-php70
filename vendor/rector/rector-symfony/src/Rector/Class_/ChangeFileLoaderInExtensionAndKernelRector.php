...
, [self::FROM => 'xml', self::TO => 'yaml'])]);
    ...
    private function replaceSuffix(\PhpParser\Node\Expr\MethodCall $methodCall, string $from, string $to)
    {
        ...
        $this->traverseNodesWithCallable([$fileArgument], function (\PhpParser\Node $node) use($from, $to) {
            if (!$node instanceof \PhpParser\Node\Scalar\String_) {
                return null;
            }
            $node->value = \RectorPrefix20210525\Nette\Utils\Strings::replace($node->value, '#\\.' . $from . '$#', '.' . $to);
            return $node;
        });
    }
}
