...
use RectorPrefix20210522\Symfony\Component\Console\Input\StringInput;
...
CODE_SAMPLE
    ...
    {
        $privatesCaller = new \RectorPrefix20210522\Symplify\PackageBuilder\Reflection\PrivatesCaller();
        return $privatesCaller->callPrivateMethod(new \RectorPrefix20210522\Symfony\Component\Console\Input\StringInput(''), 'tokenize', [$process]);
    }
    /**
     * @return void
     */
    ...
    private function findPreviousNodeAssign(\PhpParser\Node $node, \PhpParser\Node\Expr $firstArgumentExpr)
    {
        ...
        $assign = $this->betterNodeFinder->findFirstPrevious($node, function (\PhpParser\Node $checkedNode) use($firstArgumentExpr) {
