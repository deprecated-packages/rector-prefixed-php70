 ...
 */
class ArrayNode extends \RectorPrefix20210527\Symfony\Component\Config\Definition\BaseNode implements \RectorPrefix20210527\Symfony\Component\Config\Definition\PrototypeNodeInterface
{
     ...
     */
    public function addChild(\RectorPrefix20210527\Symfony\Component\Config\Definition\NodeInterface $node)
    {
        $name = $node->getName();
        if (!\strlen($name)) {
    ...
    {
        if (\false === $value) {
            throw new \RectorPrefix20210527\Symfony\Component\Config\Definition\Exception\UnsetKeyException(\sprintf('Unsetting key for path "%s", value: %s.', $this->getPath(), \json_encode($value)));
        }
        foreach ($this->children as $name => $child) {
            if (!\array_key_exists($name, $value)) {
                if ($child->isRequired()) {
                    ...
                    $ex = new \RectorPrefix20210527\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($message);
            ...
            } catch (\RectorPrefix20210527\Symfony\Component\Config\Definition\Exception\UnsetKeyException $e) {
                unset($value[$name]);
            }
        }
        return $value;
    ...
    protected function validateType($value)
    {
        if (!\is_array($value) && (!$this->allowFalse || \false !== $value)) {
            $ex = new \RectorPrefix20210527\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected "array", but got "%s"', $this->getPath(), \get_debug_type($value)));
    ...
    protected function normalizeValue($value)
    {
        ...
        if (\count($value) && !$this->ignoreExtraKeys) {
            ...
            $ex = new \RectorPrefix20210527\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($msg);
            $ex->setPath($this->getPath());
            throw $ex;
        }
        return $normalized;
    ...
    protected function remapXml(array $value)
    {
        foreach ($this->xmlRemappings as list($singular, $plural)) {
            ...
            $value[$plural] = \RectorPrefix20210527\Symfony\Component\Config\Definition\Processor::normalizeConfig($value, $singular, $plural);
