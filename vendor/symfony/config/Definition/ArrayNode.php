 ...
 */
class ArrayNode extends \RectorPrefix20210504\Symfony\Component\Config\Definition\BaseNode implements \RectorPrefix20210504\Symfony\Component\Config\Definition\PrototypeNodeInterface
{
     ...
     */
    public function addChild(\RectorPrefix20210504\Symfony\Component\Config\Definition\NodeInterface $node)
    {
        $name = $node->getName();
        if (!\strlen($name)) {
    ...
    {
        if (\false === $value) {
            throw new \RectorPrefix20210504\Symfony\Component\Config\Definition\Exception\UnsetKeyException(\sprintf('Unsetting key for path "%s", value: %s.', $this->getPath(), \json_encode($value)));
        }
        foreach ($this->children as $name => $child) {
            if (!\array_key_exists($name, $value)) {
                if ($child->isRequired()) {
                    ...
                    $ex = new \RectorPrefix20210504\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($message);
            ...
            } catch (\RectorPrefix20210504\Symfony\Component\Config\Definition\Exception\UnsetKeyException $e) {
                unset($value[$name]);
            }
        }
        return $value;
    ...
    protected function validateType($value)
    {
        if (!\is_array($value) && (!$this->allowFalse || \false !== $value)) {
            $ex = new \RectorPrefix20210504\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected "array", but got "%s"', $this->getPath(), \get_debug_type($value)));
    ...
    protected function normalizeValue($value)
    {
        ...
        if (\count($value) && !$this->ignoreExtraKeys) {
            ...
            $ex = new \RectorPrefix20210504\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($msg);
            $ex->setPath($this->getPath());
            throw $ex;
        }
        return $normalized;
    ...
    protected function remapXml(array $value)
    {
        foreach ($this->xmlRemappings as list($singular, $plural)) {
            ...
            $value[$plural] = \RectorPrefix20210504\Symfony\Component\Config\Definition\Processor::normalizeConfig($value, $singular, $plural);
        ...
        foreach ($rightSide as $k => $v) {
            ...
            if (!\array_key_exists($k, $leftSide)) {
                if (!$this->allowNewKeys) {
                    $ex = new \RectorPrefix20210504\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('You are not allowed to define new elements for path "%s". Please define all elements for this path in one config file. If you are trying to overwrite an element, make sure you redefine it with the same name.', $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
                $leftSide[$k] = $v;
