 ...
 */
namespace RectorPrefix20210504\Symfony\Component\Config\Definition\Dumper;
...
use RectorPrefix20210504\Symfony\Component\Config\Definition\EnumNode;
use RectorPrefix20210504\Symfony\Component\Config\Definition\NodeInterface;
...
{
    private $reference;
    public function dump(\RectorPrefix20210504\Symfony\Component\Config\Definition\ConfigurationInterface $configuration, string $namespace = null)
    {
        return $this->dumpNode($configuration->getConfigTreeBuilder()->buildTree(), $namespace);
    }
    public function dumpNode(\RectorPrefix20210504\Symfony\Component\Config\Definition\NodeInterface $node, string $namespace = null)
    {
        $this->reference = '';
    ...
    private function writeNode(\RectorPrefix20210504\Symfony\Component\Config\Definition\NodeInterface $node, int $depth = 0, bool $root = \false, string $namespace = null)
    {
        ...
        $rootChildren = [];
        $rootComments = [];
        if ($node instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\ArrayNode) {
            ...
            if ($node instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\PrototypedArrayNode) {
                ...
                if ($prototype instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\PrototypedArrayNode) {
                    $prototype->setName($key ?? '');
                    $children = [$key => $prototype];
                } elseif ($prototype instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\ArrayNode) {
            ...
            foreach ($children as $child) {
                if ($child instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\ArrayNode) {
                    // get elements
                    $rootChildren[] = $child;
                    continue;
                }
                ...
                // comments
                $comments = [];
                if ($child instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\BaseNode && ($info = $child->getInfo())) {
                    $comments[] = $info;
                }
                if ($child instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\BaseNode && ($example = $child->getExample())) {
                    $comments[] = 'Example: ' . $example;
                }
                if ($child->isRequired()) {
                    $comments[] = 'Required';
                }
                if ($child instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\BaseNode && $child->isDeprecated()) {
                    $deprecation = $child->getDeprecation($child->getName(), $node->getPath());
                    $comments[] = \sprintf('Deprecated (%s)', ($deprecation['package'] || $deprecation['version'] ? "Since {$deprecation['package']} {$deprecation['version']}: " : '') . $deprecation['message']);
                }
                if ($child instanceof \RectorPrefix20210504\Symfony\Component\Config\Definition\EnumNode) {
