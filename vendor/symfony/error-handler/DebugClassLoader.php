...
use RectorPrefix20210528\Composer\InstalledVersions;
use RectorPrefix20210528\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use RectorPrefix20210528\Doctrine\Persistence\Proxy;
use RectorPrefix20210528\Mockery\MockInterface;
use RectorPrefix20210528\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use RectorPrefix20210528\PHPUnit\Framework\MockObject\MockObject;
use RectorPrefix20210528\Prophecy\Prophecy\ProphecySubjectInterface;
use RectorPrefix20210528\ProxyManager\Proxy\ProxyInterface;
/**
...
class DebugClassLoader
{
    ...
    public static function enable()
    {
        ...
        \class_exists(\RectorPrefix20210528\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\RectorPrefix20210528\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \RectorPrefix20210528\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210528\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210528\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210528\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210528\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210528\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, \false) as $use) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!\strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && 0 === \strpos($className, 'Symfony\\') && (!\class_exists(\RectorPrefix20210528\Composer\InstalledVersions::class) || 'symfony/symfony' !== \RectorPrefix20210528\Composer\InstalledVersions::getRootPackage()['name'])) {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \RectorPrefix20210528\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
