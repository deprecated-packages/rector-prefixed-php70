 ...
 * Cache file storage.
 */
class FileStorage implements \RectorPrefix20210518\Nette\Caching\Storage
{
    ...
    /** @var array */
    private $locks;
    public function __construct(string $dir, \RectorPrefix20210518\Nette\Caching\Storages\Journal $journal = null)
    {
        if (!\is_dir($dir)) {
            throw new \RectorPrefix20210518\Nette\DirectoryNotFoundException("Directory '{$dir}' not found.");
        }
        $this->dir = $dir;
    ...
    public function write(string $key, $data, array $dp)
    {
        ...
        if (isset($dp[\RectorPrefix20210518\Nette\Caching\Cache::EXPIRATION])) {
            if (empty($dp[\RectorPrefix20210518\Nette\Caching\Cache::SLIDING])) {
                $meta[self::META_EXPIRE] = $dp[\RectorPrefix20210518\Nette\Caching\Cache::EXPIRATION] + \time();
                // absolute time
            } else {
                $meta[self::META_DELTA] = (int) $dp[\RectorPrefix20210518\Nette\Caching\Cache::EXPIRATION];
                // sliding time
            }
        }
        if (isset($dp[\RectorPrefix20210518\Nette\Caching\Cache::ITEMS])) {
            foreach ($dp[\RectorPrefix20210518\Nette\Caching\Cache::ITEMS] as $item) {
    ...
    {
        $all = !empty($conditions[\RectorPrefix20210518\Nette\Caching\Cache::ALL]);
        $collector = empty($conditions);
        $namespaces = $conditions[\RectorPrefix20210518\Nette\Caching\Cache::NAMESPACES] ?? null;
        // cleaning using file iterator
        if ($all || $collector) {
            $now = \time();
            foreach (\RectorPrefix20210518\Nette\Utils\Finder::find('_*')->from($this->dir)->childFirst() as $entry) {
                $path = (string) $entry;
                if ($entry->isDir()) {
            ...
            foreach ($namespaces as $namespace) {
                ...
                foreach (\RectorPrefix20210518\Nette\Utils\Finder::findFiles('_*')->in($dir) as $entry) {
