...
namespace RectorPrefix20210504\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \RectorPrefix20210504\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
            ...
            foreach ($iterator as $fileInfo) {
                ...
                foreach ($foundClasses as $class) {
                    if (isset($this->classes[$class])) {
                        throw new \RectorPrefix20210504\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$this->classes[$class][0]} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \RectorPrefix20210504\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \RectorPrefix20210504\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \RectorPrefix20210504\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    private function updateFile(string $file)
    {
        ...
        foreach ($foundClasses as $class) {
            ...
            if (isset($prevFile)) {
                throw new \RectorPrefix20210504\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$prevFile} and in {$file}.");
            }
    ...
    {
        \RectorPrefix20210504\Nette\Utils\FileSystem::createDir($dir);
