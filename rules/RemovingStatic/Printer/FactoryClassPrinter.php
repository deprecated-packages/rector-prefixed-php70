...
final class FactoryClassPrinter
{
    ...
    private function createFactoryClassFilePath(\PhpParser\Node\Stmt\Class_ $oldClass) : string
    {
        ...
        $directoryPath = \RectorPrefix20210531\Nette\Utils\Strings::before($smartFileInfo->getRealPath(), \DIRECTORY_SEPARATOR, -1);
        $resolvedOldClass = $this->nodeNameResolver->getName($oldClass);
        if ($resolvedOldClass === null) {
            throw new \Rector\Core\Exception\ShouldNotHappenException();
        }
        $bareClassName = \RectorPrefix20210531\Nette\Utils\Strings::after($resolvedOldClass, '\\', -1) . 'Factory.php';
