...
final class AnonymousFunctionNodeFactory
{
    ...
    public function __construct(\RectorPrefix20210503\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \PhpParser\Parser $parser)
    {
        $this->parser = $parser;
    ...
    public function createAnonymousFunctionFromString(\PhpParser\Node\Expr $expr)
    {
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmt, function (\PhpParser\Node $node) : Node {
            ...
            $match = \RectorPrefix20210503\Nette\Utils\Strings::match($node->value, self::DIM_FETCH_REGEX);
            if (!$match) {
                return $node;
            }
