...
final class AnonymousFunctionNodeFactory
{
     ...
     */
    private $parser;
    public function __construct(\RectorPrefix20210519\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \PhpParser\Parser $parser)
    {
    ...
    public function createAnonymousFunctionFromString(\PhpParser\Node\Expr $expr)
    {
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmt, function (\PhpParser\Node $node) : Node {
            ...
            $match = \RectorPrefix20210519\Nette\Utils\Strings::match($node->value, self::DIM_FETCH_REGEX);
            if (!$match) {
                return $node;
            }
