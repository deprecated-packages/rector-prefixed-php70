...
final class RegexMatcher
{
    ...
    public function resolvePatternExpressionWithoutEIfFound(\PhpParser\Node\Expr $expr)
    {
        if ($expr instanceof \PhpParser\Node\Scalar\String_) {
            ...
            $modifiers = \RectorPrefix20210531\Nette\Utils\Strings::after($pattern, $delimiter, -1);
    ...
    {
        $modifiersWithoutE = \RectorPrefix20210531\Nette\Utils\Strings::replace($modifiers, '#e#', '');
        return \RectorPrefix20210531\Nette\Utils\Strings::before($pattern, $delimiter, -1) . $delimiter . $modifiersWithoutE;
    }
    /**
     * @return \PhpParser\Node\Expr|null
     */
    private function matchConcat(\PhpParser\Node\Expr\BinaryOp\Concat $concat)
    {
        ...
        $matches = \RectorPrefix20210531\Nette\Utils\Strings::match($lastItem->value, self::LETTER_SUFFIX_REGEX);
        ...
        $lastItem->value = \RectorPrefix20210531\Nette\Utils\Strings::replace($lastItem->value, self::LAST_E_REGEX, '$1$2');
        return $concat;
    }
}
