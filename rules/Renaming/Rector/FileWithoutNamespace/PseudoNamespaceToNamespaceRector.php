...
, [self::NAMESPACE_PREFIXES_WITH_EXCLUDED_CLASSES => [new \Rector\Renaming\ValueObject\PseudoNamespaceToNamespace('Some_', ['Some_Class_To_Keep'])]])]);
    ...
    public function configure(array $configuration)
    {
        ...
        \RectorPrefix20210527\Webmozart\Assert\Assert::allIsInstanceOf($namespacePrefixesWithExcludedClasses, \Rector\Renaming\ValueObject\PseudoNamespaceToNamespace::class);
    ...
    {
        $this->traverseNodesWithCallable($stmts, function (\PhpParser\Node $node) {
    ...
    private function processIdentifier(\PhpParser\Node\Identifier $identifier)
    {
        ...
        $namespaceName = \RectorPrefix20210527\Nette\Utils\Strings::before($name, '_', -1);
        /** @var string $lastNewNamePart */
        $lastNewNamePart = \RectorPrefix20210527\Nette\Utils\Strings::after($name, '_', -1);
        $newNamespace = \RectorPrefix20210527\Nette\Utils\Strings::replace($namespaceName, self::SPLIT_BY_UNDERSCORE_REGEX, '$1$2\\\\$4');
