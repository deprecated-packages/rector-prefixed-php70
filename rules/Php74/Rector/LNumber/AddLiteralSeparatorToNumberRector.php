...
final class AddLiteralSeparatorToNumberRector extends \Rector\Core\Rector\AbstractRector implements \Rector\Core\Contract\Rector\ConfigurableRectorInterface
{
    ...
    public function configure(array $configuration)
    {
        ...
        \RectorPrefix20210504\Webmozart\Assert\Assert::integer($limitValue);
...
, [self::LIMIT_VALUE => 1000000])]);
    ...
    public function refactor(\PhpParser\Node $node)
    {
        ...
        if (\RectorPrefix20210504\Nette\Utils\Strings::contains($numericValueAsString, '.')) {
    ...
    private function shouldSkip(\PhpParser\Node $node, string $numericValueAsString) : bool
    {
        ...
        // already separated
        if (\RectorPrefix20210504\Nette\Utils\Strings::contains($numericValueAsString, '_')) {
            return \true;
        }
        ...
        // e+/e-
        if (\RectorPrefix20210504\Nette\Utils\Strings::match($numericValueAsString, '#e#i')) {
            return \true;
        }
        // too short
        return \RectorPrefix20210504\Nette\Utils\Strings::length($numericValueAsString) <= self::GROUP_SIZE;
    }
    /**
     * @return string[]
     */
