...
use RectorPrefix20210519\Symplify\Astral\ValueObject\NodeBuilder\UseBuilder;
...
, [self::USE_IMPORTS_TO_RESTORE => [new \Rector\Restoration\ValueObject\CompleteImportForPartialAnnotation('Doctrine\\ORM\\Mapping', 'ORM')]])]);
    ...
    public function refactor(\PhpParser\Node $node)
    {
        ...
        foreach ($this->useImportsToRestore as $useImportToRestore) {
            ...
            if (!\RectorPrefix20210519\Nette\Utils\Strings::match($this->print($class), $annotationToSeek)) {
                continue;
            }
    ...
    {
        $useBuilder = new \RectorPrefix20210519\Symplify\Astral\ValueObject\NodeBuilder\UseBuilder($completeImportForPartialAnnotation->getUse());
