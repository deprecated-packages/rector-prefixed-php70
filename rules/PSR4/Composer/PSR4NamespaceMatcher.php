...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\Rector\Core\ValueObject\Application\File $file, \PhpParser\Node $node)
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $path) {
                $path = \rtrim($path, '/');
                if (!\RectorPrefix20210504\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $path)) {
                    continue;
                }
    ...
    {
        $extraNamespace = \RectorPrefix20210504\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \RectorPrefix20210504\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \RectorPrefix20210504\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
