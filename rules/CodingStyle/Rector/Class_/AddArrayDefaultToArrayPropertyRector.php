...
CODE_SAMPLE
    ...
    {
        $this->traverseNodesWithCallable($class, function (\PhpParser\Node $class) use($propertyNames) {
    ...
    private function clearNotNullBeforeCount(\PhpParser\Node\Stmt\Class_ $class, array $propertyNames)
    {
        $this->traverseNodesWithCallable($class, function (\PhpParser\Node $node) use($propertyNames) {
            ...
            $isNextNodeCountingProperty = (bool) $this->betterNodeFinder->findFirst($node->right, function (\PhpParser\Node $node) use($propertyNames) {
