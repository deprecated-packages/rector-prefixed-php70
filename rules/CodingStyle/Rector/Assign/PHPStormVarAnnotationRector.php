...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node)
    {
        ...
        if (!\RectorPrefix20210531\Nette\Utils\Strings::match($docContent, $varPattern)) {
            return null;
        }
        // switch docs
    ...
    private function createDocComment(\PhpParser\Node $node) : \PhpParser\Comment\Doc
    {
        ...
        if (\strncmp($docContent, '/* ', \strlen('/* ')) === 0) {
            $docContent = \RectorPrefix20210531\Nette\Utils\Strings::replace($docContent, self::SINGLE_ASTERISK_COMMENT_START_REGEX, '/** ');
        }
        // $value is first, instead of type is first
        if (\RectorPrefix20210531\Nette\Utils\Strings::match($docContent, self::VAR_ANNOTATION_REGEX)) {
            $docContent = \RectorPrefix20210531\Nette\Utils\Strings::replace($docContent, self::VARIABLE_NAME_AND_TYPE_MATCH_REGEX, '$3$2$1');
        }
