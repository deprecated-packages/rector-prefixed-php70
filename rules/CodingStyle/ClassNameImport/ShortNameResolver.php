...
final class ShortNameResolver
{
    ...
    public function __construct(\RectorPrefix20210531\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \Rector\NodeNameResolver\NodeNameResolver $nodeNameResolver, \PhpParser\NodeFinder $nodeFinder, \PHPStan\Reflection\ReflectionProvider $reflectionProvider, \Rector\Core\PhpParser\Node\BetterNodeFinder $betterNodeFinder)
    {
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmts, function (\PhpParser\Node $node) use(&$shortNamesToFullyQualifiedNames, $reflectionClass) {
            ...
            foreach ($phpDocInfo->getPhpDocNode()->children as $phpDocChildNode) {
                ...
                if ($reflectionClass !== null) {
                    $fullyQualifiedTagName = \RectorPrefix20210531\Nette\Utils\Reflection::expandClassName($shortTagName, $reflectionClass);
                } else {
    ...
    private function resolveShortTagNameFromPhpDocChildNode(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocChildNode $phpDocChildNode)
    {
        ...
        if (\RectorPrefix20210531\Nette\Utils\Strings::match($tagName, self::BIG_LETTER_START_REGEX)) {
            return $tagName;
        }
