...
final class ShortNameResolver
{
    ...
    public function __construct(\RectorPrefix20210520\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \Rector\NodeNameResolver\NodeNameResolver $nodeNameResolver, \PhpParser\NodeFinder $nodeFinder, \PHPStan\Reflection\ReflectionProvider $reflectionProvider, \Rector\Core\PhpParser\Node\BetterNodeFinder $betterNodeFinder)
    {
    ...
    private function resolveForStmts(array $stmts) : array
    {
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmts, function (\PhpParser\Node $node) use(&$shortNamesToFullyQualifiedNames) {
            ...
            // already short
            if (\RectorPrefix20210520\Nette\Utils\Strings::contains($originalName->toString(), '\\')) {
                return;
            }
            $fullyQualifiedName = $this->nodeNameResolver->getName($node);
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmts, function (\PhpParser\Node $node) use(&$shortNamesToFullyQualifiedNames, $reflectionClass) {
            ...
            foreach ($phpDocInfo->getPhpDocNode()->children as $phpDocChildNode) {
                ...
                if ($reflectionClass !== null) {
                    $fullyQualifiedTagName = \RectorPrefix20210520\Nette\Utils\Reflection::expandClassName($shortTagName, $reflectionClass);
                } else {
    ...
    private function resolveShortTagNameFromPhpDocChildNode(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocChildNode $phpDocChildNode)
    {
        ...
        if (\RectorPrefix20210520\Nette\Utils\Strings::match($tagName, self::BIG_LETTER_START_REGEX)) {
            return $tagName;
        }
        ...
        if (\RectorPrefix20210520\Nette\Utils\Strings::contains($typeNode->name, '\\')) {
            return null;
        }
