...
final class ConstructorAssignDetector
{
    ...
    public function isPropertyAssigned(\PhpParser\Node\Stmt\ClassLike $classLike, string $propertyName) : bool
    {
        ...
        foreach ($initializeClassMethods as $initializeClassMethod) {
            $this->simpleCallableNodeTraverser->traverseNodesWithCallable((array) $initializeClassMethod->stmts, function (\PhpParser\Node $node) use($propertyName, &$isAssignedInConstructor) {
     ...
     * @return void
     */
    private function decorateFirstLevelStatementAttribute(array $classMethods)
    {
