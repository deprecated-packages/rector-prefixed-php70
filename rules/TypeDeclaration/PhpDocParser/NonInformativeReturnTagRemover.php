...
final class NonInformativeReturnTagRemover
{
     ...
     */
    private $typeChecker;
    public function __construct(\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfoFactory $phpDocInfoFactory, \RectorPrefix20210525\Symplify\PackageBuilder\Php\TypeChecker $typeChecker)
    {
    ...
    private function removeNullableType(\PHPStan\Type\Type $returnType, \PHPStan\PhpDocParser\Ast\PhpDoc\ReturnTagValueNode $returnTagValueNode, \Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo)
    {
        ...
        if (!\RectorPrefix20210525\Nette\Utils\Strings::endsWith($nullabledReturnType->getClassName(), $nullabledReturnTagValueNode->name)) {
            return;
        }
        $phpDocInfo->removeByType(\PHPStan\PhpDocParser\Ast\PhpDoc\ReturnTagValueNode::class);
    }
    ...
    private function isClassNameAndPartMatch(string $className, string $returnTagValueNodeType) : bool
    {
        ...
        return \RectorPrefix20210525\Nette\Utils\Strings::endsWith($className, '\\' . $returnTagValueNodeType);
    }
}
