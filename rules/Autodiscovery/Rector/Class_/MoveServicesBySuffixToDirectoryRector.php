...
, [self::GROUP_NAMES_BY_SUFFIX => ['Repository']])]);
    ...
    public function configure(array $configuration)
    {
        ...
        \RectorPrefix20210531\Webmozart\Assert\Assert::allString($groupNamesBySuffix);
    ...
    private function processGroupNamesBySuffix(\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, \Rector\Core\ValueObject\Application\File $file, array $groupNamesBySuffix)
    {
        foreach ($groupNamesBySuffix as $groupNames) {
            ...
            if (!\RectorPrefix20210531\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), '#' . $suffixPattern . '#')) {
                continue;
            }
            ...
            if (\RectorPrefix20210531\Nette\Utils\Strings::match($smartFileInfo->getPath(), '#' . $groupNames . '$#')) {
                continue;
            }
    ...
    private function isLocatedInExpectedLocation(string $groupName, string $suffixPattern, \Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : bool
    {
        ...
        return (bool) \RectorPrefix20210531\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), $expectedLocationFilePattern);
    }
    /**
     * @return void
     */
